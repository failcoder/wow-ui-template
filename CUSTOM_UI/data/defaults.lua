...
UI.defaults.Filters["BlackList"] = {};
UI.defaults.Filters["WhiteList"] = {};
UI.defaults.Filters["Defense"] = {};
UI.defaults.Filters["Player"] = {};
UI.defaults.Filters["AuraBars"] = {};
UI.defaults.Filters["CC"] = {};
UI.defaults.Filters["Raid"] = {};
UI.defaults.Filters["Custom"] = {};
...
for id in InitAuraBars:gmatch("([^,]+)") do
    if(id) then
        ...
        if(safename(spellID)) then
            UI.defaults.Filters["AuraBars"][id] = {0.98, 0.57, 0.11}
        end
    end
end

local function SanitizeFilters()
    for filterType, filterData in pairs(UI.db.Filters) do
        if(filterType == 'AuraBars') then
          for id, params in pairs(UI.db.Filters[filterType]) do
            if(type(id) ~= 'string') then
                UI.db.Filters.AuraBars[id] = nil;
                local newID = tostring(id);
                if(newID) then
                    UI.db.Filters.AuraBars[newID] = {0.98, 0.57, 0.11}
                end
            end
          end
        elseif(filterType == 'Custom') then
          for customType, customData in pairs(UI.db.Filters.Custom) do
            for id, params in pairs(customData) do
                if((not params.id) or (tonumber(id) ~= params.id)) then
                    UI.db.Filters.Custom[customType][id] = nil;
                end
            end
          end
        else
          for id, params in pairs(UI.db.Filters[filterType]) do
              if((not params.id) or (tonumber(id) ~= params.id)) then
                  UI.db.Filters[filterType][id] = nil;
              end
          end
        end
    end
