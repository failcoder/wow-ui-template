...
local Layout = _G["CUSTOM_UI_Layout"];
Layout.Frames = {};
Layout.Sections = {
	['ALL'] = {},
	['GENERAL'] = {}
...
local function SnapStickyFrame(frameA, frameB, left, top, right, bottom)
	...
	if (bA  <= tB and bB  <= tA) then
		...
		if lA  <= (lB  +  Sticky.rangeX) and lA  >= (lB - Sticky.rangeX) then
			newX = lB  +  wA
			if frameB == UIParent or frameB == WorldFrame or frameB == CUSTOM_UIParent then
				newX = newX  +  4
			end
			snap = true
		end
...
local function GrabUsableRegions(frame)
	local parent = frame or UI.Screen
...
TheHand.bg:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\MENTALO-HAND-OFF]])
TheHand.energy = TheHand:CreateTexture(nil, "OVERLAY")
TheHand.energy:SetAllPoints(TheHand)
TheHand.energy:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\MENTALO-ENERGY]])
UI.Animate:Orbit(TheHand.energy, 10)
...
local TheHand_OnUpdate = function(self, elapsed)
	...
	if self.elapsedTime > 0.1 then
		self.elapsedTime = 0
		local x, y = GetCursorPosition()
		local scale = UI.Screen:GetEffectiveScale()
		self:SetPoint("CENTER", UI.Screen, "BOTTOMLEFT", (x  /  scale)  +  50, (y  /  scale)  +  50)
	end
end

function TheHand:Enable()
	self:Show()
	self.bg:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\MENTALO-HAND-ON]])
	self.energy:Show()
	self.flash:Play()
...
function TheHand:Disable()
	self.flash:Stop()
	self.energy:Hide()
	self.bg:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\MENTALO-HAND-OFF]])
...
local function SetPrecisionSizes()
	...
	if(not CurrentFrameTarget.postsize) then
		CUSTOM_UI_LayoutPrecision:SetHeight(70)
		CUSTOM_UI_LayoutPrecisionWidthAdjust:Hide()
		CUSTOM_UI_LayoutPrecisionHeightAdjust:Hide()
	else
		local x,y = CUSTOM_UI_LayoutPrecision:GetCenter()
		CUSTOM_UI_LayoutPrecision:ClearAllPoints()
		CUSTOM_UI_LayoutPrecision:SetPoint("BOTTOMLEFT", CUSTOM_UIParent, "BOTTOMLEFT", x,y)

		CUSTOM_UI_LayoutPrecision:SetHeight(144)
		...
		CUSTOM_UI_LayoutPrecisionWidthAdjust.rangeLow:SetText(minRange);
		CUSTOM_UI_LayoutPrecisionWidthAdjust.rangeHigh:SetText(maxRange);
		CUSTOM_UI_LayoutPrecisionWidthAdjust:SetMinMaxValues(minRange, maxRange);
		CUSTOM_UI_LayoutPrecisionWidthAdjust:SetValue(curWidth);
		CUSTOM_UI_LayoutPrecisionWidthAdjust.rangeValue:SetText(curWidth);
		CUSTOM_UI_LayoutPrecisionWidthAdjust:Show()
		CUSTOM_UI_LayoutPrecisionHeightAdjust.rangeLow:SetText(min2Range);
		CUSTOM_UI_LayoutPrecisionHeightAdjust.rangeHigh:SetText(max2Range);
		CUSTOM_UI_LayoutPrecisionHeightAdjust:SetMinMaxValues(minRange, maxRange);
		CUSTOM_UI_LayoutPrecisionHeightAdjust:SetValue(curHeight);
		CUSTOM_UI_LayoutPrecisionHeightAdjust.rangeValue:SetText(curHeight);
		CUSTOM_UI_LayoutPrecisionHeightAdjust:Show()
	end
end

function Layout:Movable_OnMouseUp()
	if(not CUSTOM_UI_LayoutPrecision) then return end;
	CurrentFrameTarget = self;
	local xOffset, yOffset, anchor = CalculateOffsets()

	CUSTOM_UI_LayoutPrecisionSetX.CurrentValue = xOffset;
	CUSTOM_UI_LayoutPrecisionSetX:SetText(xOffset)

	CUSTOM_UI_LayoutPrecisionSetY.CurrentValue = yOffset;
	CUSTOM_UI_LayoutPrecisionSetY:SetText(yOffset)

	CUSTOM_UI_LayoutPrecision.Title:SetText(self.textString)
end
...
function Layout:Movable_OnUpdate()
	...
	if CurrentFrameTarget ~= frame then
		CUSTOM_UI_LayoutPrecision:Hide()
		frame:GetScript("OnMouseUp")(frame)
	else
		CUSTOM_UI_LayoutPrecision:ClearAllPoints()
		CUSTOM_UI_LayoutPrecision:SetPoint(anchor1, frame, anchor2, xOffset, yOffset)
		SetPrecisionSizes()
	end
...
function Layout:Movable_OnDragStart()
	if InCombatLockdown() then UI:AddonMessage(ERR_NOT_IN_COMBAT)return end
	if UI.db.general.stickyFrames then
...
function Layout:Movable_OnDragStop()
	...
	self:ClearAllPoints()
	self:SetPoint(newAnchor, UI.Screen, newAnchor, cX, cY)

	SaveAnchor(self.name)

	if CUSTOM_UI_LayoutPrecision then
...
function Layout:Movable_OnEnter()
	...
	UpdateFrameTarget = self;
	Layout.Portrait:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\MENTALO-ON]])
...
function Layout:Movable_OnLeave()
	...
	Layout.Portrait:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\MENTALO-OFF]])
	TheHand:Hide()
	if(not CUSTOM_UI_LayoutPrecision) then return end;
	if(CurrentFrameTarget ~= self and not CUSTOM_UI_LayoutPrecision:IsShown()) then
		self:SetAlpha(0.4)
	end
end
...
function Layout:Movable_OnMouseDown(button)
	if button == "RightButton" then
		...
		CUSTOM_UI_LayoutPrecision:Show()
	end
end
...
local function SetNewAnchor(frame, moveName, title, postDragFunc)
	...
	local currentCategory = UI_Registry.CURRENT_SCHEMA;
	...
	local grip = CreateFrame("Button", moveName, UI.Screen)
	...
	mtext:SetFontObject(CUSTOM_UI_Font_Default)
...
function Layout:Toggle(arg)
	...
	if(aceConfig and UI.OptionsLoaded) then
		aceConfig:Close(UI.NameID)
		GameTooltip:Hide()
	end
	CUSTOM_UI_LayoutPrecision:Hide()
	...
	if(self:IsShown()) then
		UI:AddonMessage('Frames are now locked!')
		self:Hide()
	else
		--print(arg)
		arg = arg or 'ALL';
		...
		if(category == 'HELP') then
			UI:AddonMessage('You can use the following commands to specify which frame-groups you want to move at a time.')
			for section,_ in pairs(self.Sections) do
		        print('/sv move |cff00FF00' .. lower(section) .. '|r')
			end
		else
			UI:AddonMessage('Frames are now unlocked!')
			UI:AddonMessage('To see move commands type |cff00FF00/sv move help|r.')
...
function Graph:Toggle(enabled)
	if((not self.Grid) or (self.CellSize ~= UI.db.general.graphSize)) then
...
function Graph:UpdateAllReports()
	...
	local cellSize = UI.db.general.graphSize
	self.CellSize = cellSize

	self.Grid = CreateFrame('Frame', nil, UIParent)
	self.Grid:SetAllPoints(UI.Screen)
	self.Grid:SetFrameStrata('BACKGROUND')
	self.Grid:SetFrameLevel(1)

	local width = UI.Screen:GetWidth();
	local height = UI.Screen:GetHeight();
...
local XML_LayoutLockButton_OnClick = function(self)
	Graph:Toggle()
	Layout:Toggle()
	if(UI.OptionsLoaded and UI.OptionsStandby) then
		UI.OptionsStandby = nil
		LibStub("AceConfigDialog-1.0"):Open(UI.NameID)
	end
end

local CUSTOM_UI_LayoutPrecisionResetButton_OnClick = function(self)
	if(not CurrentFrameTarget) then return end
...
local XML_LayoutPrecisionInputX_EnterPressed = function(self)
	...
	if(current) then
		if(CurrentFrameTarget) then
			local xOffset, yOffset, anchor = CalculateOffsets()
			yOffset = tonumber(CUSTOM_UI_LayoutPrecisionSetY.CurrentValue)
			CurrentFrameTarget:ClearAllPoints()
			CurrentFrameTarget:SetPoint(anchor, CUSTOM_UIParent, anchor, current, yOffset)
...
local XML_LayoutPrecisionInputY_EnterPressed = function(self)
	...
	if(current) then
		if(CurrentFrameTarget) then
			local xOffset, yOffset, anchor = CalculateOffsets()
			xOffset = tonumber(CUSTOM_UI_LayoutPrecisionSetX.CurrentValue)
			CurrentFrameTarget:ClearAllPoints()
			CurrentFrameTarget:SetPoint(anchor, CUSTOM_UIParent, anchor, xOffset, current)
			SaveAnchor(CurrentFrameTarget.name)
		end
		self.CurrentValue = current
	end
...
local XML_LayoutPrecisionWidthAdjust_OnValueChanged = function(self, widthValue)
	...
	if(CurrentFrameTarget and CurrentFrameTarget.postsize) then
		...
		local heightValue = CUSTOM_UI_LayoutPrecisionHeightAdjust:GetValue();
...
local XML_LayoutPrecisionHeightAdjust_OnValueChanged = function(self, heightValue)
	...
	if(CurrentFrameTarget and CurrentFrameTarget.postsize) then
		local frame = CurrentFrameTarget.parent;
		local widthValue = CUSTOM_UI_LayoutPrecisionWidthAdjust:GetValue();
		CurrentFrameTarget.postsize(frame, widthValue, heightValue);
	end
end
--[[
...
local function SaveCurrentPosition(frame)
	...
	if(UI.db.general.saveDraggable) then
		UI.private.Draggables[frameName] = result
		Dragger.Frames = UI.private.Draggables
	else
...
local DraggerEventHandler = function(self, event, ...)
	...
	local allCentered = UI.db.screen.multiMonitor
	...
	for frameName, data in pairs(UIPanels) do
		...
		if(not data.initialized) then
			...
			if(frame) then
				...
				if(allCentered) then
					frame:ClearAllPoints()
					frame:SetPoint('TOP', UI.Screen, 'TOP', 0, -180)
					data.centered = true
				end
...
function Dragger:Reset()
	if(UI.db.general.saveDraggable) then
		for frameName, data in pairs(UIPanels) do
			if(UI.private.Draggables[frameName]) then
				UI.private.Draggables[frameName] = nil
			end
			data.initialized = nil
		end
		self.Frames = UI.private.Draggables
	else
		for frameName, data in pairs(UIPanels) do
...
local function InitializeMovables()
	CLOAKED_BG:SetAllPoints(UI.Screen);
	...
	Layout.Anchors = UI.db.LAYOUT or {}
	...
	CUSTOM_UI_LayoutLockButton:SetSize(110, 25)
	CUSTOM_UI_LayoutLockButton.Left:SetAlpha(0)
	CUSTOM_UI_LayoutLockButton.Middle:SetAlpha(0)
	CUSTOM_UI_LayoutLockButton.Right:SetAlpha(0)
	CUSTOM_UI_LayoutLockButton:SetNormalTexture("")
	CUSTOM_UI_LayoutLockButton:SetPushedTexture("")
	CUSTOM_UI_LayoutLockButton:SetPushedTexture("")
	CUSTOM_UI_LayoutLockButton:SetDisabledTexture("")
	CUSTOM_UI_LayoutLockButton:RemoveTextures()
	CUSTOM_UI_LayoutLockButton:SetFrameLevel(CUSTOM_UI_LayoutLockButton:GetFrameLevel() + 1)
	CUSTOM_UI_LayoutLockButton.texture = CUSTOM_UI_LayoutLockButton:CreateTexture(nil, "BORDER")
	CUSTOM_UI_LayoutLockButton.texture:SetSize(110, 50)
	CUSTOM_UI_LayoutLockButton.texture:SetPoint("CENTER", CUSTOM_UI_LayoutLockButton, "CENTER", 0, -4)
	CUSTOM_UI_LayoutLockButton.texture:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\QUESTION]])
	CUSTOM_UI_LayoutLockButton.texture:SetVertexColor(1, 1, 1)
	CUSTOM_UI_LayoutLockButton.texture:SetTexCoord(1, 0, 1, 1, 0, 0, 0, 1)
	CUSTOM_UI_LayoutLockButton.text = CUSTOM_UI_LayoutLockButton:CreateFontString(nil, "OVERLAY")
	CUSTOM_UI_LayoutLockButton.text:SetFont(UI.Media.font.caps, 18, "OUTLINE")
	CUSTOM_UI_LayoutLockButton.text:SetTextColor(1, 0.5, 0)
	CUSTOM_UI_LayoutLockButton.text:SetPoint("CENTER")
	CUSTOM_UI_LayoutLockButton.text:SetText("Lock")
	CUSTOM_UI_LayoutLockButton:SetScript("OnEnter", function(this)
		this.texture:SetVertexColor(0.1, 0.1, 0.1)
		this.text:SetTextColor(1, 1, 0)
	end)
	CUSTOM_UI_LayoutLockButton:SetScript("OnLeave", function(this)
		this.texture:SetVertexColor(1, 1, 1)
		this.text:SetTextColor(1, 0.5, 0)
	end)
	CUSTOM_UI_LayoutLockButton:SetScript("OnClick", XML_LayoutLockButton_OnClick)

	CUSTOM_UI_LayoutGridButton:SetSize(110, 25)
	CUSTOM_UI_LayoutGridButton.Left:SetAlpha(0)
	CUSTOM_UI_LayoutGridButton.Middle:SetAlpha(0)
	CUSTOM_UI_LayoutGridButton.Right:SetAlpha(0)
	CUSTOM_UI_LayoutGridButton:SetNormalTexture("")
	CUSTOM_UI_LayoutGridButton:SetPushedTexture("")
	CUSTOM_UI_LayoutGridButton:SetPushedTexture("")
	CUSTOM_UI_LayoutGridButton:SetDisabledTexture("")
	CUSTOM_UI_LayoutGridButton:RemoveTextures()
	CUSTOM_UI_LayoutGridButton:SetFrameLevel(CUSTOM_UI_LayoutGridButton:GetFrameLevel() + 1)
	CUSTOM_UI_LayoutGridButton.texture = CUSTOM_UI_LayoutGridButton:CreateTexture(nil, "BORDER")
	CUSTOM_UI_LayoutGridButton.texture:SetSize(110, 50)
	CUSTOM_UI_LayoutGridButton.texture:SetPoint("CENTER", CUSTOM_UI_LayoutGridButton, "CENTER", 0, -4)
	CUSTOM_UI_LayoutGridButton.texture:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Doodads\QUESTION]])
	CUSTOM_UI_LayoutGridButton.texture:SetVertexColor(1, 1, 1)
	CUSTOM_UI_LayoutGridButton.text = CUSTOM_UI_LayoutGridButton:CreateFontString(nil, "OVERLAY")
	CUSTOM_UI_LayoutGridButton.text:SetFont(UI.Media.font.caps, 18, "OUTLINE")
	CUSTOM_UI_LayoutGridButton.text:SetTextColor(1, 0.5, 0)
	CUSTOM_UI_LayoutGridButton.text:SetPoint("CENTER")
	CUSTOM_UI_LayoutGridButton.text:SetText("Grid")
	CUSTOM_UI_LayoutGridButton:SetScript("OnEnter", function(this)
		this.texture:SetVertexColor(0.1, 0.1, 0.1)
		this.text:SetTextColor(1, 1, 0)
	end)
	CUSTOM_UI_LayoutGridButton:SetScript("OnLeave", function(this)
		this.texture:SetVertexColor(1, 1, 1)
		this.text:SetTextColor(1, 0.5, 0)
	end)
	CUSTOM_UI_LayoutGridButton:SetScript("OnClick", XML_LayoutGridButton_OnClick)

	CUSTOM_UI_LayoutPrecision:SetFrameLevel(999)
	CUSTOM_UI_LayoutPrecision:SetStyle("Frame", "Pattern")
	CUSTOM_UI_LayoutPrecision:EnableMouse(true)
	CUSTOM_UI_LayoutPrecision:RegisterForDrag("LeftButton")

	UI.API:Set("CloseButton", CUSTOM_UI_LayoutPrecisionCloseButton)

	CUSTOM_UI_LayoutPrecisionSetX:SetStyle("Editbox")
	CUSTOM_UI_LayoutPrecisionSetX.CurrentValue = 0;
	CUSTOM_UI_LayoutPrecisionSetX:SetScript("OnEnterPressed", XML_LayoutPrecisionInputX_EnterPressed)
	CUSTOM_UI_LayoutPrecisionSetY:SetStyle("Editbox")
	CUSTOM_UI_LayoutPrecisionSetY.CurrentValue = 0;
	CUSTOM_UI_LayoutPrecisionSetY:SetScript("OnEnterPressed", XML_LayoutPrecisionInputY_EnterPressed)
	CUSTOM_UI_LayoutPrecisionResetButton:SetStyle("Button")
	CUSTOM_UI_LayoutPrecisionUpButton:SetStyle("Button")
	CUSTOM_UI_LayoutPrecisionDownButton:SetStyle("Button")
	CUSTOM_UI_LayoutPrecisionLeftButton:SetStyle("Button")
	CUSTOM_UI_LayoutPrecisionRightButton:SetStyle("Button")
	CUSTOM_UI_LayoutPrecisionResetButton:SetScript("OnClick", CUSTOM_UI_LayoutPrecisionResetButton_OnClick)

	CUSTOM_UI_LayoutPrecisionWidthAdjust:SetValueStep(1);
	CUSTOM_UI_LayoutPrecisionHeightAdjust:SetValueStep(1);
	CUSTOM_UI_LayoutPrecisionWidthAdjust:SetScript("OnValueChanged", XML_LayoutPrecisionWidthAdjust_OnValueChanged)
	CUSTOM_UI_LayoutPrecisionHeightAdjust:SetScript("OnValueChanged", XML_LayoutPrecisionHeightAdjust_OnValueChanged)

	CUSTOM_UI_LayoutPrecision:SetScript("OnHide", function()
		if(not CurrentFrameTarget) then return end
	...
	if(UI.db.general.useDraggable) then
		if(not UI.private.Draggables) then UI.private.Draggables = {} end
...
UI.Events:On("LOAD_ALL_WIDGETS", InitializeMovables);
--[[
##########################################################
CORE FUNCTIONS
##########################################################
]]--
function UI:NewAnchor(frame, title, postDragFunc)
...
function UI:SetAnchorResizing(frame, postSizeFunc, minRange, maxRange, min2Range, max2Range)
...
function UI:MoveAnchors(...)
	Layout:Toggle(...)
end

function UI:UpdateAnchors()
	Layout:Update()
end

function UI:ResetAnchors(...)
	Layout:Reset(...)
end

function UI:ForceAnchors(forced)
...
UI.SystemAlert["RESETBLIZZARD_CHECK"] = {
...
UI.SystemAlert["RESETLAYOUT_CHECK"] = {
