...
local UI = select(2, ...);
local L = UI.L;
local MOD = UI:NewPackage("Dock", L["Docks"]);
MOD.Border = {};
--[[ COMMON CONSTANTS ]]--
...
-- UI.SpecialFX:Register("dragging_highlight", [[Spells\Warlock_bodyofflames_medium_state_shoulder_right_purple.m2]], -20, 50, 20, -50, 0.35, 0, 0.5)
UI.SpecialFX:Register("dragging_highlight_top", [[Spells\Creature_spellportal_blue_clickable.m2]], 0, 0, 0, -80, 0.8, 0, 2.5)
UI.SpecialFX:Register("dragging_highlight_bottom", [[Spells\Creature_spellportal_blue_clickable.m2]], 0, 80, 0, 0, 0.9, 0, -0.25)
...
MOD.ButtonSound = UI.Sounds:Blend("DockButton", "Buttons", "Levers");
MOD.ErrorSound = UI.Sounds:Blend("Malfunction", "Sparks", "Wired");
...
local function GetDockDimensions(location)
	...
	if(isTop) then
		if(isLeft) then
			width = UI.db.Dock.dockTopLeftWidth;
			height = UI.db.Dock.dockTopLeftHeight;
		else
			width = UI.db.Dock.dockTopRightWidth;
			height = UI.db.Dock.dockTopRightHeight;
		end
	else
		if(isLeft) then
			width = UI.db.Dock.dockLeftWidth;
			height = UI.db.Dock.dockLeftHeight;
			if(MOD.private.LeftExpanded) then
				height = height + 300
			end
		else
			width = UI.db.Dock.dockRightWidth;
			height = UI.db.Dock.dockRightHeight;
...
local function SetDockDimensions(location, width, height, buttonSize)
	...
	if(isTop) then
		if(isLeft) then
			UI.db.Dock.dockTopLeftWidth = width;
			if(not buttonSize) then
				UI.db.Dock.dockTopLeftHeight = height;
			end
		else
			UI.db.Dock.dockTopRightWidth = width;
			if(not buttonSize) then
				UI.db.Dock.dockTopRightHeight = height;
			end
		end
	else
		if(isLeft) then
			UI.db.Dock.dockLeftWidth = width;
			if(not buttonSize) then
				UI.db.Dock.dockLeftHeight = height;
			end
		else
			UI.db.Dock.dockRightWidth = width;
			if(not buttonSize) then
				UI.db.Dock.dockRightHeight = height;
			end
		end
	end

	if(buttonSize) then
		UI.db.Dock.buttonSize = height;
	end
end
...
local function ScreenBorderVisibility()
	if UI.db.Dock.bottomPanel then
		CUSTOM_UIDock_BottomBorder:Show()
	else
		CUSTOM_UIDock_BottomBorder:Hide()
	end

	if UI.db.Dock.topPanel then
		CUSTOM_UIDock_TopBorder:Show()
	else
		CUSTOM_UIDock_TopBorder:Hide()
	end
end
...
local UpdateBackdrop = function(self)
	...
	if(UI.db.Dock.backdrop and (not MOD.private.Disabled[self:GetName() .. 'Button'])) then
...
function MOD:SetBorderTheme()
	self.Border.Top:SetPoint("TOPLEFT", UI.Screen, "TOPLEFT", -1, 1)
	self.Border.Top:SetPoint("TOPRIGHT", UI.Screen, "TOPRIGHT", 1, 1)
	self.Border.Top:SetHeight(10)
	self.Border.Top:SetBackdrop({
		bgFile = UI.Media.background.button,
	...
	self.Border.Top:SetBackdropColor(unpack(UI.Media.color.dark))
	...
	self.Border.Bottom:SetPoint("BOTTOMLEFT", UI.Screen, "BOTTOMLEFT", -1, -1)
	self.Border.Bottom:SetPoint("BOTTOMRIGHT", UI.Screen, "BOTTOMRIGHT", 1, -1)
	...
	self.Border.Bottom:SetBackdropColor(unpack(UI.Media.color.dark))
...
function MOD:SetButtonTheme(button, size)
	...
	sparks:SetTexture(UI.Media.dock.sparks[1])
	sparks:SetVertexColor(0.7, 0.6, 0.5)
	sparks:SetBlendMode("ADD")
	sparks:SetAlpha(0)

	UI.Animate:Sprite8(sparks, 0.08, 2, false, true)

	button.Sparks = sparks;

	button.ClickTheme = function(self)
		self.Sparks:SetTexture(UI.Media.dock.sparks[random(1,3)])
...
_G.ToggleSuperDockLeft = function(self, button)
	...
	if(button and IsAltKeyDown()) then
		UI:StaticPopup_Show('RESETDOCKS_CHECK')
	elseif(button and button == 'RightButton') then
		...
		MOD.ButtonSound()
		local userSize = UI.db.Dock.dockLeftHeight
		...
		UI.Events:Trigger("DOCK_EXPANDED", "BottomLeft", activeDock);
	else
		if MOD.private.LeftFaded then
			...
			UI.Events:Trigger("DOCK_FADE_IN", "BottomLeft", activeDock);
			...
			UI.Events:Trigger("DOCK_FADE_OUT", "BottomLeft", activeDock);
			PlaySoundFile([[sound\doodad\be_scryingorb_explode.ogg]])
		end
	end
end
...
_G.ToggleSuperDockRight = function(self, button)
	...
	elseif(button and button == 'RightButton') then
		...
		MOD.ButtonSound()
		local userSize = UI.db.Dock.dockRightHeight
		...
		MOD:UpdateDockBackdrops()
		UI.Events:Trigger("DOCK_EXPANDED", "BottomRight", activeDock);
	else
		if MOD.private.RightFaded then
			...
			UI.Events:Trigger("DOCK_FADE_IN", "BottomRight", activeDock);
			PlaySoundFile([[sound\doodad\be_scryingorb_explode.ogg]])
		else
			...
			UI.Events:Trigger("DOCK_FADE_OUT", "BottomRight", activeDock);
			PlaySoundFile([[sound\doodad\be_scryingorb_explode.ogg]])
		end
	end
end
...
_G.ToggleSuperDocks = function()
	if(MOD.private.AllFaded) then
		...
		MOD.BottomLeft:FadeIn(0.2, MOD.BottomLeft:GetAlpha(), 1)
		MOD.BottomLeft.Bar:FadeIn(0.2, MOD.BottomLeft.Bar:GetAlpha(), 1)
		UI.Events:Trigger("DOCK_FADE_IN", "BottomLeft", MOD.private.Active.BottomLeft);
		MOD.BottomRight:FadeIn(0.2, MOD.BottomRight:GetAlpha(), 1)
		MOD.BottomRight.Bar:FadeIn(0.2, MOD.BottomRight.Bar:GetAlpha(), 1)
		UI.Events:Trigger("DOCK_FADE_IN", "BottomRight", MOD.private.Active.BottomRight);
		PlaySoundFile([[sound\doodad\be_scryingorb_explode.ogg]])
	else
		...
		MOD.BottomLeft:FadeOut(0.2, MOD.BottomLeft:GetAlpha(), 0)
		MOD.BottomLeft.Bar:FadeOut(0.2, MOD.BottomLeft.Bar:GetAlpha(), 0)
		UI.Events:Trigger("DOCK_FADE_OUT", "BottomLeft");
		MOD.BottomRight:FadeOut(0.2, MOD.BottomRight:GetAlpha(), 0)
		MOD.BottomRight.Bar:FadeOut(0.2, MOD.BottomRight.Bar:GetAlpha(), 0)
		UI.Events:Trigger("DOCK_FADE_OUT", "BottomRight");
		PlaySoundFile([[sound\doodad\be_scryingorb_explode.ogg]])
	end
end
...
local function DeactivateDockletButton(button)
	...
	if(button.Icon) then
		button.Icon:SetGradient(unpack(UI.Media.gradient.icon));
	end
end
...
local function ActivateDockletButton(button)
	...
	button:SetPanelColor("default");
	if(button.Icon) then
		button.Icon:SetGradient(unpack(UI.Media.gradient.checked));
	end
end
...
local function ShowDockletWindow(button, location)
	...
	if(window.PostShowCallback) then
		...
		UI.Events:Trigger("DOCKLET_SHOWN", location, button.LinkKey);
	end
	return true;
end
...
local function HideDockletWindow(button, location)
	...
	if(window.PostHideCallback) then
		...
		UI.Events:Trigger("DOCKLET_HIDDEN", location, button.LinkKey);
	end
	return true;
end
...
local function ResetAllDockletWindows(dockbar, button)
	...
	UI.Events:Trigger("DOCKLET_RESET", location);
end
...
local DockBar_NextDefault = function(self)
	...
	UI.Events:Trigger("DOCKLET_LIST_EMPTY", location);
end
...
local DockButton_OnDragStop = function(self)
	...
	if((not DRAG_TARGETBAR) and self.CanFloat) then
		...
		UI.Events:Trigger("DOCKLET_MOVED", self.LocationKey);
	end
...
local DockButton_OnEnter = function(self, ...)
	...
	self.Icon:SetGradient(unpack(UI.Media.gradient.highlight))
...
local DockButton_OnLeave = function(self, ...)
	...
	if(self.ActiveDocklet) then
		self.Icon:SetGradient(unpack(UI.Media.gradient.checked))
	else
		self.Icon:SetGradient(unpack(UI.Media.gradient.icon))
	end
	GameTooltip:Hide()
end
...
local DockButton_ShowDockOptions = function(self)
	...
	UI.Dropdown:Open(self, list, menuTitle);
end
--[[
##########################################################
...
local DockBar_CreateButton = function(self, displayName, globalName, texture, tipFunction, primaryTemplate, frameLink)
	local dockIcon = texture or [[Interface\AddOns\CUSTOM_UI\media\textures\Dock\DOCK-ICON-ADDON]];
	local size = self.ToolBar:GetHeight();
	local template = "CUSTOM_UI_DockletButtonTemplate"
	local isAction = false;

	if(primaryTemplate) then
		template = primaryTemplate .. ", CUSTOM_UI_DockletButtonTemplate";
		isAction = true;
	end
...
MOD.TopCenter = _G["CUSTOM_UI_DockTopCenter"];
MOD.BottomCenter = _G["CUSTOM_UI_DockBottomCenter"];
...
local DockAlert_Activate = function(self, child)
	...
	local size = UI.db.Dock.buttonSize or 22;
	self:SetHeight(size)
	self.backdrop:Show()
...
for location, settings in pairs(DOCK_LOCATIONS) do
	MOD[location] = _G["CUSTOM_UI_Dock" .. location];
	MOD[location].Bar = _G["CUSTOM_UI_DockBar" .. location];
...
local Docklet_Relocate = function(self, location)
	...
	if(self.Bar) then
		...
		local barReverse = UI:GetReversePoint(barAnchor);
		local spacing = UI.db.Dock.buttonSpacing;
...
local CornerButton2_OnClick = function(self, button)
	if(IsAltKeyDown() and button and button == 'RightButton') then
		...
		UI.Dropdown:Open(self, menu, titleText);
	end
end

-- function MOD:UpdateLocals()
-- 	DOCK_WIDTH = UI.db.Dock.dockWidth;
-- 	DOCK_HEIGHT = UI.db.Dock.dockHeight;
-- 	DOCK_ALPHA = UI.db.Dock.dockOpacity;
-- end
...
function MOD:ResetAllButtons()
	...
	UI.Events:Trigger("DOCKLETS_RESET", location);
	ReloadUI()
end
...
function MOD:Refresh()
	local buttonsize = UI.db.Dock.buttonSize;
	local spacing = UI.db.Dock.buttonSpacing;
	...
	local centerWidth = UI.db.Dock.dockCenterWidth;
	local dockWidth = centerWidth * 0.5;
	local dockHeight = UI.db.Dock.dockCenterHeight;
	...
	UI.Events:Trigger("DOCKS_UPDATED");
end
...
function MOD:Load()
	if(not UI.private.Docks) then
		UI.private.Docks = {}
	end

	self.private = UI.private.Docks;
	...
	self.Border.Top = CreateFrame("Frame", "CUSTOM_UIDock_TopBorder", UI.Screen);
	self.Border.Bottom = CreateFrame("Frame", "CUSTOM_UIDock_BottomBorder", UI.Screen);
	self:SetBorderTheme();
	ScreenBorderVisibility();
	...
	for location, settings in pairs(DOCK_LOCATIONS) do
		...
		local anchor = upper(location);
		local reverse = UI:GetReversePoint(anchor);
		local barAnchor = settings[2];
		local barReverse = UI:GetReversePoint(barAnchor);
		local isBottom = settings[3];
		local vertMod = isBottom and 1 or -1

		dock.Bar:SetParent(UI.Screen)
		dock.Bar:ClearAllPoints()
		dock.Bar:SetSize(width, buttonsize)
		dock.Bar:SetPoint(anchor, UI.Screen, anchor, (2 * mod), (2 * vertMod))
		...
		if(location:find('Top')) then
			...
			UI.SpecialFX:SetFXFrame(highlight, "dragging_highlight_top")
			highlight.texture = highlight:CreateTexture(nil, "BACKGROUND")
			highlight.texture:SetAllPoints()
			highlight.texture:SetTexture(UI.Media.statusbar.default);
			...
			highlight:SetHeight(buttonsize * 2)
			UI.SpecialFX:SetFXFrame(highlight, "dragging_highlight_bottom")
			highlight.texture = highlight:CreateTexture(nil, "BACKGROUND")
			highlight.texture:SetAllPoints()
			highlight.texture:SetTexture(UI.Media.statusbar.default);
		...
		if(dock.Bar.Button) then
	    	...
	    	dock.Bar.Button.Icon:SetTexture(UI.Media.dock.sizeIcon)
			...
			dock.Bar.Button.Icon:SetGradient(unpack(UI.Media.gradient.icon))
			...
			if(dock.Bar.Button2) then
		    	...
		    	dock.Bar.Button2.Icon:SetTexture(UI.Media.dock.optionsIcon)
		    	dock.Bar.ToolBar:SetSize(1, buttonsize)
		    	dock.Bar.ToolBar:SetPoint(barAnchor, dock.Bar.Button2, barReverse, (spacing * mod), 0)
				dock.Bar.Button2:SetPanelColor("default")
				dock.Bar.Button2.Icon:SetGradient(unpack(UI.Media.gradient.icon))
				if(location:find('Left')) then
		...
		UI:NewAnchor(dock.Bar, location .. " Dock ToolBar");
		UI:SetAnchorResizing(dock.Bar, dockBarPostSizeFunc, 10, 500, 10, 80);
		UI:NewAnchor(dock, location .. " Dock Window");
		UI:SetAnchorResizing(dock, dockPostSizeFunc, 10, 500);
	end

	if MOD.private.LeftFaded then MOD.BottomLeft:Hide() end
	if MOD.private.RightFaded then MOD.BottomRight:Hide() end

	UI:ManageVisibility(self.BottomRight.Window)
	UI:ManageVisibility(self.TopLeft)
	UI:ManageVisibility(self.TopRight)
	--UI:ManageVisibility(self.BottomCenter)
	UI:ManageVisibility(self.TopCenter)

	local centerWidth = UI.db.Dock.dockCenterWidth;
	local dockHeight = UI.db.Dock.dockCenterHeight;

	self.TopCenter:SetParent(UI.Screen)
	self.TopCenter:ClearAllPoints()
	self.TopCenter:SetSize(centerWidth, dockHeight)
	self.TopCenter:SetPoint("TOP", UI.Screen, "TOP", 0, 0)

	self.BottomCenter:SetParent(UI.Screen)
	self.BottomCenter:ClearAllPoints()
	self.BottomCenter:SetSize(centerWidth, dockHeight)
	self.BottomCenter:SetPoint("BOTTOM", UI.Screen, "BOTTOM", 0, 0)
