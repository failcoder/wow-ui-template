...
local lib = Librarian:NewLibrary("API");
...
lib.Templates = {
    ["Default"]     = "CUSTOM_UI_CoreStyle_Default",
    ["Transparent"] = "CUSTOM_UI_CoreStyle_Transparent",
    ["Button"]      = "CUSTOM_UI_CoreStyle_Button",
    ["CheckButton"] = "CUSTOM_UI_CoreStyle_CheckButton",
    ["DockButton"]  = "CUSTOM_UI_CoreStyle_DockButton",
    ["ActionSlot"]  = "CUSTOM_UI_CoreStyle_ActionSlot",
    ["Lite"]        = "CUSTOM_UI_CoreStyle_Lite",
    ["Icon"]        = "CUSTOM_UI_CoreStyle_Icon",
    ["Bar"]         = "CUSTOM_UI_CoreStyle_Bar",
    ["Inset"]       = "CUSTOM_UI_CoreStyle_Inset",
    ["Blackout"]    = "CUSTOM_UI_CoreStyle_Blackout",
    ["Component"]   = "CUSTOM_UI_CoreStyle_Component",
    ["Paper"]       = "CUSTOM_UI_CoreStyle_Paper",
    ["Container"]   = "CUSTOM_UI_CoreStyle_Container",
    ["Pattern"]     = "CUSTOM_UI_CoreStyle_Pattern",
    ["Premium"]     = "CUSTOM_UI_CoreStyle_Premium",
    ["Model"]       = "CUSTOM_UI_CoreStyle_Model",
    ["UnitLarge"]   = "CUSTOM_UI_CoreStyle_UnitLarge",
    ["UnitSmall"]   = "CUSTOM_UI_CoreStyle_UnitSmall",
    ["Window"]      = "CUSTOM_UI_CoreStyle_Window",
    ["Window2"]     = "CUSTOM_UI_CoreStyle_Window2",
};
lib.Methods = {};
lib.Concepts = {};
--[[
...
local function ScreenUpdate()
    ...
    if(IsMacClient()) then
        if(not MAC_DISPLAY) then
            MAC_DISPLAY = UI_Registry:NewGlobal("Display");
    ...
    local gxScale;
    if(UI.db.screen.advanced) then
        BASE_MOD = 0.64
        local ADJUSTED_SCALE = UI.db.screen.scaleAdjust;
		...
		-- end
        if(UI.db.screen.autoScale) then
...
local HookCustomBackdrop_TypeA = function(self)
    if(self.Panel) then
        ...
        local bdSet = UI.Media.backdrop[bgid]
        if(bdSet) then
            ...
            local newBgFile = UI.Media.background[bgid]
            local newBorderFile = UI.Media.border[bgid]
        ...
        local panelColor = UI.Media.color[colorID];
        if(panelColor) then
...
local Cooldown_OnSizeChanged = function(self, width, height)
    ...
    if newSize < 0.5 and not override then
        ...
        frame:Show()
        frame.text:SetFont(UI.Media.font.number, newSize * 15, 'OUTLINE')
...
local SetFrameBorderColor = function(self, r, g, b, setPrevious, reset)
    ...
    elseif(reset) then
        r,g,b = unpack(UI.Media.color[self.Panel.__previous])
    end
...
local ShowAlertFlash = function(self)
    ...
    UI.Animate:Flash(self.Panel.Shadow, 0.75, true)
end

local HideAlertFlash = function(self)
    UI.Animate:StopFlash(self.Panel.Shadow)
...
local function SetCD(button, noSwipe)
    ...
    if(bn) then
        ...
        if(cooldown) then
            if(not UI.db.general or (UI.db.general and (not UI.db.general.cooldown))) then return end
            cooldown:ClearAllPoints()
            cooldown:InsetPoints()
            cooldown:SetDrawEdge(false)
            cooldown:SetDrawBling(false)
            cooldown:SetHideCountdownNumbers(true)
            cooldown.SetHideCountdownNumbers = UI.fubar
            if(not noSwipe) then
...
function lib:APPLY(frame, templateName, underlay, padding, xOffset, yOffset, defaultColor)
    ...
    local bgColor = UI.Media.color[colorName] or UI.Media.color.default;
    local borderColor = UI.Media.bordercolor[panelID] or UI.Media.bordercolor.default;
    ...
    if(panel.Skin) then
        ...
        if(gradientName and UI.Media.gradient[gradientName]) then
            panel.Skin:SetGradient(unpack(UI.Media.gradient[gradientName]))
        else
...
lib.Methods["Button"] = function(self, frame, inverse, xOffset, yOffset, defaultColor)
...
lib.Methods["LiteButton"] = function(self, frame, inverse, xOffset, yOffset, defaultColor)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Methods["ActionSlot"] = function(self, frame, inverse, addChecked)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Methods["CheckButton"] = function(self, frame, inverse, x, y)
    if(not frame or (frame and frame.Panel)) then return end
    ...
    if(frame.SetHighlightTexture) then
        ...
        local color = UI.Media.color.highlight
        frame.hover:SetTexture(color[1], color[2], color[3], 0.5)
        frame:SetHighlightTexture(frame.hover)
    end

    if(frame.SetCheckedTexture) then
        frame:SetCheckedTexture(UI.Media.button.check)
        local ct = frame:GetCheckedTexture()
        ct:SetTexCoord(0, 1, 0, 1)
    end

    if(frame.SetDisabledCheckedTexture) then
        frame:SetDisabledCheckedTexture(UI.Media.button.uncheck)
        local ct = frame:GetDisabledCheckedTexture()
        ct:SetTexCoord(0, 1, 0, 1)
    end
end;

lib.Methods["Editbox"] = function(self, frame, inverse, x, y)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Methods["Icon"] = function(self, frame, inverse, ...)
    if(not frame or (frame and frame.Panel)) then return end
    local underlay = (not inverse)
    self:APPLY(frame, "Icon", underlay, ...)
end;

lib.Methods["DockButton"] = function(self, frame, inverse, template)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Methods["Frame"] = function(self, frame, inverse, styleName, noupdate, overridePadding, xOffset, yOffset, defaultColor)
    if(not frame or (frame and frame.Panel)) then return end
    local padding = false;
...
local SetPanelColor = function(self, ...)
    ...
    if(self.Panel.Skin and self.Panel:GetAttribute("panelGradient")) then
        if(type(arg1) == "string") then
            ...
            elseif(UI.Media.gradient[arg1]) then
                self.Panel:SetAttribute("panelColor", arg1)
                self.Panel.Skin:SetGradient(unpack(UI.Media.gradient[arg1]))
                if(UI.Media.color[arg1]) then
                    local t = UI.Media.color[arg1]
                    self:SetBackdropColor(t[1], t[2], t[3], t[4])
                end
            end
        end
    elseif(type(arg1) == "string" and UI.Media.color[arg1]) then
        local t = UI.Media.color[arg1]
        self.Panel:SetAttribute("panelColor", arg1)
        self:SetBackdropColor(t[1], t[2], t[3], t[4])
...
local SetStyle = function(self, method, ...)
    ...
    local fn = lib.Methods[methodName];
    if(fn) then
        local pass, catch = pcall(fn, lib, self, param, ...)
        if(catch) then
            UI:HandleError("API", "SetStyle", catch);
            return
        end
    end
end
    ...
    for i=1, #LIVE_UPDATE_FRAMES do
        ...
        if(frame) then
            ...
            if(frame.BackdropNeedsUpdate) then
                ...
                if(UI.Media.bordercolor[panelID]) then
                    local borderColor = UI.Media.bordercolor[panelID]
                    frame:SetBackdropBorderColor(borderColor[1], borderColor[2], borderColor[3], borderColor[4] or 1)
                else
            ...
            if(frame.TextureNeedsUpdate and frame.Panel.Skin) then
                local tex = UI.Media.background[panelID]
                if(tex) then
                ...
                if(not frame.NoColorUpdate) then
                    ...
                    if(gradient and UI.Media.gradient[gradient]) then
                        local g = UI.Media.gradient[gradient]
...
UI.Events:On("SHARED_MEDIA_UPDATED", FrameTemplateUpdates, true);
UI.Events:On("REQUEST_TEMPLATE_UPDATED", FrameTemplateUpdates, true);
--[[
##########################################################
CORE FUNCTIONS
##########################################################
]]--
function UI:SetAtlasFunc(atlas, fn)
    ATLAS_HACKS[atlas] = fn
end

function UI:SetAtlasFilter(atlas, fn)
    if(not fn) then
        fn = "default"
    end
...
function UI:UI_SCALE_CHANGED(event)
...
local Button_OnLeave = function(self)
    self:SetBackdropColor(unpack(UI.Media.color.button))
end
...
local _hook_Tooltip_OnShow = function(self)
    self:SetBackdrop(UI.Media.backdrop.tooltip)
end

lib.Concepts["Frame"] = function(self, adjustable, frame, template, noStripping, padding, xOffset, yOffset)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Concepts["Window"] = function(self, adjustable, frame, altStyle, fullStrip, padding, xOffset, yOffset)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Concepts["Button"] = function(self, adjustable, frame)
    if(not frame or (frame and frame.Panel)) then return end
    RemoveTextures(frame)
    self.Methods["Button"](self, frame, adjustable)
end

lib.Concepts["CheckButton"] = function(self, adjustable, frame)
    if(not frame or (frame and frame.Panel)) then return end
    --RemoveTextures(frame)
    self.Methods["CheckButton"](self, frame, adjustable)
end

lib.Concepts["CloseButton"] = function(self, adjustable, frame, targetAnchor)
    if(not frame or (frame and frame.Panel)) then return end
    ...
    frame:SetNormalTexture(UI.Media.icon.close)
...
lib.Concepts["InfoButton"] = function(self, adjustable, frame, targetAnchor, size)
    if(not frame or (frame and frame.Panel)) then return end
    ...
    frame:SetNormalTexture(UI.Media.icon.info)
    --frame:HookScript("OnEnter", ConceptButton_OnEnter)
    --frame:HookScript("OnLeave", ConceptButton_OnLeave)
...
lib.Concepts["ArrowButton"] = function(self, adjustable, frame, direction, targetAnchor)
    if(not frame or (frame and frame.Panel)) then return end
    ...
    frame:SetFrameLevel(frame:GetFrameLevel() + 4)
    frame:SetNormalTexture(UI.Media.icon[iconKey])
    frame:HookScript("OnEnter", ConceptButton_OnEnter)
    frame:HookScript("OnLeave", ConceptButton_OnLeave)
...
lib.Concepts["ItemButton"] = function(self, adjustable, frame, noScript)
    ...
    if(link) then
        ...
        if(iconObject) then
            iconObject:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
        ...
        if(countObject) then
            ...
            countObject:SetFontObject(CUSTOM_UI_Font_Number)
        ...
        if(levelObject) then
            ...
            levelObject:SetFontObject(CUSTOM_UI_Font_Number)
            levelObject:SetDrawLayer("ARTWORK", 7)
        end
    end
end

lib.Concepts["PageButton"] = function(self, adjustable, frame, isVertical, isLeft)
    if(not frame) then return end
    ...
    if not frame.icon then
        ...
        frame.icon:SetTexture(UI.Media.button.radio)
...
lib.Concepts["ScrollBar"] = function(self, adjustable, frame, scale, yOffset)
    if(not frame or (frame and frame.Panel)) then return end
    ...
    if(upButton and downButton) then
        ...
        if(not frame.BG) then
            ...
            frame.BG:SetTexture(UI.Media.background.transparent)

            local fg = CreateFrame("Frame", nil, frame)
            fg:SetPoint("TOPLEFT", frame, "TOPLEFT", 18, -2)
            fg:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -10, 6)
            fg:SetBackdrop(UI.Media.backdrop.outline)
            frame.Brdr = fg
        end
        if(frame.SetThumbTexture) then
            frame:SetThumbTexture(UI.Media.button.knob)
        end
...
lib.Concepts["Tab"] = function(self, adjustable, frame, addBackground, xOffset, yOffset)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Concepts["DropDown"] = function(self, adjustable, frame, width)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Concepts["Tooltip"] = function(self, adjustable, frame, useHook)
    if(not frame or (frame and frame.Panel)) then return end
    ...
    frame:SetBackdrop(UI.Media.backdrop.tooltip)
...
lib.Concepts["EditBox"] = function(self, adjustable, frame, width, height, x, y)
    if(not frame or (frame and frame.Panel)) then return end
...
lib.Concepts["QuestItem"] = function(self, adjustable, frame)
    if(not frame or (frame and frame.Panel)) then return end
    ...
    if(icon) then
        ...
        iconFallback:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
...
lib.Concepts["Skin"] = function(self, style, frame, topX, topY, bottomX, bottomY)
    if(not frame or (frame and frame.Panel)) then return end
    if(not style) then style = "model" end
    local artname = style:lower()
    local texture = UI.Media.background[artname] or UI.Media.background.model
    RemoveTextures(frame, true)
    ...
    borderFrame:SetBackdrop(UI.Media.backdrop.outline)

    local skin = frame:CreateTexture(nil, "BACKGROUND", nil, -1)
    skin:SetAllPoints(borderFrame)
    skin:SetTexture(texture)
    skin:SetGradient(unpack(UI.Media.gradient.special))
end

local ALERT_TEMPLATE = {
    ["typeA"] = {
        COLOR   = {0.8, 0.2, 0.2},
        BG      = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-BG]],
        LEFT    = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-LEFT]],
        RIGHT   = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-RIGHT]],
        TOP     = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-TOP]],
        BOTTOM  = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-BOTTOM]],
    },
    ["typeB"] = {
        COLOR   = {0.08, 0.4, 0},
        BG      = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-BG-2]],
        LEFT    = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-LEFT-2]],
        RIGHT   = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-RIGHT-2]],
        TOP     = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-TOP]],
        BOTTOM  = [[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-BOTTOM]],
    },
};
...
lib.Concepts["Alert"] = function(self, defaultStyle, frame, arg)
    ...
    if(not defaultStyle) then
        ...
        alertpanel.bg:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-FULL]])
        alertpanel.bg:SetGradient('VERTICAL', 0, 0, 0, .37, .32, .29)

        --[[ ICON BG ]]--
        alertpanel.icon = alertpanel:CreateTexture(nil, "BACKGROUND", nil, -2)
        alertpanel.icon:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Alert\ALERT-ICON-BORDER]])
