...
local function GetActiveMissions()
	...
	for key,data in pairs(GarrisonData) do
		...
		if (data.completed) then
			...
			remaining = ("%s %s"):format(data.timeLeft, getColoredString(" ("..UI:ParseSeconds(data.seconds)..")", "lightgrey"))
		end
	...
	for i = 1, #buildings do
		...
		if(isBuilding) then
			...
			if(canActivate or timeLeft < 0) then
				...
				remaining = ("Building %s"):format(getColoredString("("..UI:ParseSeconds(timeLeft)..")", "lightgrey"))
			end
		else
...
local function LoadToolBarGarrison()
	...
	if((not UI.db.Dock.dockTools.garrison) or (not mmButton) or MOD.GarrisonLoaded) then return end
	...
	local garrison = UI.Dock:SetDockButton("BottomLeft", L["Garrison Landing Page"], "CUSTOM_UI_Garrison", UI.Media.dock.garrisonToolIcon, SetGarrisonTooltip, "SecureActionButtonTemplate")
...
function MOD:UpdateGarrisonTool()
	if((not UI.db.Dock.dockTools.garrison) or self.GarrisonLoaded) then return end
	LoadToolBarGarrison()
end

function MOD:LoadGarrisonTool()
	if((not UI.db.Dock.dockTools.garrison) or self.GarrisonLoaded or (not _G.GarrisonLandingPageMinimapButton)) then return end
	UI.Timers:ExecuteTimer(LoadToolBarGarrison, 5)
end
