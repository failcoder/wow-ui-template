...
local function CreateMacroToolButton(proName, proID, itemID)
	local data = UI.Media.dock.professionIconCoords[proID]
	if(not data) then return end

	local globalName = ("CUSTOM_UI_%s"):format(proName)
	local button = UI.Dock:SetDockButton("BottomRight", proName, globalName, UI.Media.dock.professionIconFile, MacroButton_OnEnter, "SecureActionButtonTemplate")
...
local function LoadToolBarProfessions()
	...
	if(prof1 ~= nil and (UI.db.Dock.dockTools.primary)) then
...
function MOD:LoadProfessionTools()
	if(self.ToolBarLoaded) then return end
	UI.Timers:ExecuteTimer(LoadToolBarProfessions, 5)
end
