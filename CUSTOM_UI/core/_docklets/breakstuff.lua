...
local BreakStuff_OnClick = function(self)
	...
	if BreakStuffHandler.ReadyToSmash == true then
		...
		self.icon:SetGradient(unpack(UI.Media.gradient.green))
		if(UI.Inventory and UI.Inventory.BagFrame) then
			if(not UI.Inventory.BagFrame:IsShown()) then
				GameTooltip:Hide()
				UI.Inventory.BagFrame:Show()
				UI.Inventory.BagFrame:RefreshBags()
				if(UI.Tooltip) then
					UI.Tooltip.GameTooltip_SetDefaultAnchor(GameTooltip,self)
				end
			end
		end
	end
...
local function LoadToolBreakStuff()
	...
	BreakStuffButton.icon:SetTexture(UI.Media.dock.breakStuffIcon)
...
function MOD:CloseBreakStuff()
	if((not UI.db.Dock.dockTools.breakstuff) or self.BreakStuffLoaded) then return end
	BreakStuffHandler:MODIFIER_STATE_CHANGED()
	BreakStuffHandler.ReadyToSmash = false
	BreakStuffButton.ttText = "BreakStuff : OFF";
	BreakStuffButton.icon:SetGradient("VERTICAL", 0.5, 0.53, 0.55, 0.8, 0.8, 1)
...
function MOD:LoadBreakStuff()
	if((not UI.db.Dock.dockTools.breakstuff) or self.BreakStuffLoaded) then return end
	UI.Timers:ExecuteTimer(LoadToolBreakStuff, 5)
end