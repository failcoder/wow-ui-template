...
local CUSTOM_UI_AlertFrame = CreateFrame("Frame", "CUSTOM_UI_AlertFrame", UIParent);
CUSTOM_UI_AlertFrame:SetPoint("TOP", CUSTOM_UI_DockTopCenter, "BOTTOM", 0, -115);
CUSTOM_UI_AlertFrame:SetSize(180, 20);
--[[
##########################################################
DEFINITIONS
##########################################################
]]--
UI.SystemAlert["CLIENT_UPDATE_REQUEST"] = {
	text = L["Detected that your CUSTOM_UI Config addon is out of date. Update as soon as possible."],
	button1 = OKAY,
	OnAccept = UI.fubar,
	state1 = 1
};

UI.SystemAlert["CHANGED_MANAGED_UISCALE"] = {
	text = L["You have changed your UIScale, because you have enabled CUSTOM_UI managed scaling, we will have to reload the UI to properly align everything."],
	button1 = OKAY,
...
UI.SystemAlert["TAINT_RL"] = {
	text = L["CUSTOM_UI has lost it's damned mind! I need to reload your UI to fix it."],
	button1 = ACCEPT,
	button2 = CANCEL,
	OnAccept = function()ReloadUI()end,
	timeout = 0,
...
UI.SystemAlert["DISBAND_RAID"] = {
	text = L["Are you sure you want to disband the group?"],
	button1 = ACCEPT,
	button2 = CANCEL,
	OnAccept = function() UI:DisbandRaidGroup() end,
	timeout = 0,
	whileDead = 1,
};
UI.SystemAlert["RESETMOVERS_CHECK"] = {
	text = L["Are you sure you want to reset every mover back to it's default position?"],
	button1 = ACCEPT,
	button2 = CANCEL,
	OnAccept = function(a)UI:ResetUI(true)end,
	timeout = 0,
	whileDead = 1
};
UI.SystemAlert["RESET_UI_CHECK"] = {
	...
	button1 = ACCEPT,
	button2 = CANCEL,
	OnAccept = function(a)UI:ResetAllUI(true)end,
	timeout = 0,
	whileDead = 1
};
UI.SystemAlert["RESETDOCKS_CHECK"] = {
	text = L["Are you sure you want to reset every dock button back to it's default position?"],
	button1 = ACCEPT,
	button2 = CANCEL,
	OnAccept = function(a)UI.Dock:ResetAllButtons()end,
	timeout = 0,
	whileDead = 1
};
UI.SystemAlert["CONFIRM_LOOT_DISTRIBUTION"] = {
...
UI.SystemAlert["RESET_PROFILE_PROMPT"] = {
...
UI.SystemAlert["COPY_PROFILE_PROMPT"] = {
...
UI.SystemAlert["IMPORT_PROFILE_PROMPT"] = {
...
UI.SystemAlert["MASTER_PROFILE_PROMPT"] = {
...
UI.SystemAlert["DELETE_GRAYS"] = {
	text = L["Are you sure you want to delete all your gray items?"],
	button1 = YES,
	button2 = NO,
	OnAccept = function() UI:VendorGrays(true) end,
	OnShow = function(self) MoneyFrame_Update(self.moneyFrame, UI.SystemAlert["DELETE_GRAYS"].Money) end,
	timeout = 0,
	whileDead = 1,
	hideOnEscape = false,
	hasMoneyFrame = 1
...
local SysPop_Event_Show = function(self)
	...
	local dialog = UI.SystemAlert[self.which];
...
local SysBox_Event_KeyEscape = function(self)
	...
	for _, frame in pairs(ACTIVE_ALERTS) do
		if( frame:IsShown() and frame.hideOnEscape ) then
			local standardDialog = UI.SystemAlert[frame.which];
			if ( standardDialog ) then
				...
				UI:StaticPopupSpecial_Hide(frame);
			end
			closed = 1;
		end
	end
...
local SysPop_Move = function(self)
	if ( not tContains(ACTIVE_ALERTS, self) ) then
		...
		if ( lastFrame ) then
			...
			self:SetPoint("TOP", UI.Screen, "TOP", 0, -100);
		end
...
local SysPop_Event_Click = function(self, index)
	...
	local info = UI.SystemAlert[which];
	if ( not info ) then
		return nil;
	end
	local hide = true;
...
local SysPop_Event_Update = function(self, elapsed)
	if ( self.timeleft and self.timeleft > 0 ) then
		...
		if ( timeleft <= 0 ) then
			if ( not UI.SystemAlert[which].timeoutInformationalOnly ) then
				self.timeleft = 0;
				local OnCancel = UI.SystemAlert[which].OnCancel;
				if ( OnCancel ) then
	...
	if ( self.startDelay ) then
		...
		if ( timeleft <= 0 ) then
			...
			text:SetFormattedText(UI.SystemAlert[which].text, text.text_arg1, text.text_arg2);
	...
	local onUpdate = UI.SystemAlert[self.which].OnUpdate;
	if ( onUpdate ) then
...
local SysBox_Event_KeyEnter = function(self)
	...
	if ( not self.autoCompleteParams or not AutoCompleteEditBox_OnEnterPressed(self) ) then
		EditBoxOnEnterPressed = UI.SystemAlert[which].EditBoxOnEnterPressed;
...
local SysBox_Event_KeyEscape = function(self)
	local EditBoxOnEscapePressed = UI.SystemAlert[self:GetParent().which].EditBoxOnEscapePressed;
...
local SysBox_Event_Change = function(self, userInput)
	if ( not self.autoCompleteParams or not AutoCompleteEditBox_OnTextChanged(self, userInput) ) then
		local EditBoxOnTextChanged = UI.SystemAlert[self:GetParent().which].EditBoxOnTextChanged;
...
local SysPop_Find = function(which, data)
	...
	for index = 1, MAX_STATIC_POPUPS, 1 do
		local frame = _G["CUSTOM_UI_SystemAlert"..index];
...
function UI:StaticPopupSpecial_Hide(frame)
	frame:Hide();
	SysPop_Close_Table();
end

function UI:StaticPopup_HideExclusive()
	for _, frame in pairs(ACTIVE_ALERTS) do
...
function UI:StaticPopupSpecial_Show(frame)
...
function UI:StaticPopup_Show(which, text_arg1, text_arg2, data)
	local info = UI.SystemAlert[which];
	if ( not info ) then
		return nil;
	end
	...
	if ( info.cancels ) then
		for index = 1, MAX_STATIC_POPUPS, 1 do
			...
			if ( frame:IsShown() and (frame.which == info.cancels) ) then
				frame:Hide();
				local OnCancel = UI.SystemAlert[frame.which].OnCancel;
				if ( OnCancel ) then
					OnCancel(frame, frame.data, "override");
				end
			end
	...
	if ( not dialog ) then
		...
		for i = index, MAX_STATIC_POPUPS do
			local frame = _G["CUSTOM_UI_SystemAlert"..i];
...
local AlertFramePostMove_Hook = function(forced)
	local b, c = CUSTOM_UI_AlertFrame_MOVE:GetCenter()
	local d = UI.Screen:GetTop()
	if(c > (d  /  2)) then
		POSITION = "TOP"
		ANCHOR_POINT = "BOTTOM"
		YOFFSET = -10;
		CUSTOM_UI_AlertFrame_MOVE:SetText(CUSTOM_UI_AlertFrame_MOVE.textString.." (Grow Down)")
	else
		POSITION = "BOTTOM"
		ANCHOR_POINT = "TOP"
		YOFFSET = 10;
		CUSTOM_UI_AlertFrame_MOVE:SetText(CUSTOM_UI_AlertFrame_MOVE.textString.." (Grow Up)")
	end
	if(UI.RollFrames and UI.RollFrames[1]) then
		local lastFrame = CUSTOM_UI_AlertFrame;
		local newAnchor;
		for index, rollFrame in pairs(UI.RollFrames) do
		...
		if(newAnchor) then
			...
			AlertFrame:SetAllPoints(CUSTOM_UI_AlertFrame)
		end
	else
		AlertFrame:ClearAllPoints()
		AlertFrame:SetAllPoints(CUSTOM_UI_AlertFrame)
...
function UI:StaticPopup_Hide(which, data)
	for index = 1, MAX_STATIC_POPUPS, 1 do
		local dialog = _G["CUSTOM_UI_SystemAlert"..index];
...
function UI:SavedPopup()
	if not _G["CUSTOM_UI_ConfigAlert"] then return end
	local alert = _G["CUSTOM_UI_ConfigAlert"]
...
local function LoadSystemAlerts()
	if not _G["CUSTOM_UI_ConfigAlert"] then
		local configAlert = CreateFrame("Frame", "CUSTOM_UI_ConfigAlert", UIParent)
		...
		bgtex:SetAllPoints()
		bgtex:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Alert\SAVED-BG]])
		...
		fgtex:SetAllPoints()
		fgtex:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\Alert\SAVED-FG]])
		SetConfigAlertAnim(configAlert.fg)

		UI.Animate:Orbit(configAlert.bg, 10, false, true)
	end
	for i = 1, 4 do
		local alert = CreateFrame("Frame", "CUSTOM_UI_SystemAlert"..i, UIParent, "StaticPopupTemplate")
		alert:SetID(i)
		...
		alert.input = _G["CUSTOM_UI_SystemAlert"..i.."EditBox"];
		...
		alert.gold = _G["CUSTOM_UI_SystemAlert"..i.."MoneyInputFrameGold"];
		alert.silver = _G["CUSTOM_UI_SystemAlert"..i.."MoneyInputFrameSilver"];
		alert.copper = _G["CUSTOM_UI_SystemAlert"..i.."MoneyInputFrameCopper"];
		alert.buttons = {}
		for b = 1, 3 do
			local button = _G["CUSTOM_UI_SystemAlert"..i.."Button"..b];
			button:SetScript("OnClick", AlertButton_OnClick)
			alert.buttons[b] = button
		end
		_G["CUSTOM_UI_SystemAlert"..i.."ItemFrameNameFrame"]:Die()
		_G["CUSTOM_UI_SystemAlert"..i.."ItemFrame"]:GetNormalTexture():Die()
		_G["CUSTOM_UI_SystemAlert"..i.."ItemFrame"]:SetStyle("Button")
		_G["CUSTOM_UI_SystemAlert"..i.."ItemFrameIconTexture"]:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
		_G["CUSTOM_UI_SystemAlert"..i.."ItemFrameIconTexture"]:InsetPoints()
	end

	CUSTOM_UI_AlertFrame:SetSize(180, 20);
	CUSTOM_UI_AlertFrame.callbackOnEnter = true;
	UI:NewAnchor(CUSTOM_UI_AlertFrame, L["Loot / Alert Frames"], AlertFramePostMove_Hook)
...
UI.Events:On("LOAD_ALL_ESSENTIALS", LoadSystemAlerts);
