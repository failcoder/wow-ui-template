...
local MOD = UI.Tooltip;
if(not MOD) then return end;

MOD.Holder = CreateFrame("Frame", "CUSTOM_UI_ToolTip", UIParent)
...
function MOD:INSPECT_READY(event, GUID)
	...
	if(UnitExists(unit)) then
		local itemLevel = UI:ParseGearSlots(unit, true)
	...
	if UnitIsPlayer(unit) then
		...
		if(lvlLine) then
			...
			if(creatureType) then
				...
				if(UI.class == "HUNTER" and creatureType == PET_TYPE_SUFFIX[8] and (family and TAMABLE_FAMILIES[family])) then
...
local _hook_GameTooltip_OnTooltipSetItem = function(self)
	...
	if(key and (not self.itemCleared)) then
		...
		if(ITEM_COUNTS and UI.SetLootTooltip) then
			UI:SetLootTooltip(self, key)
		end
...
local _hook_GameTooltip_ShowStatusBar = function(self, ...)
	...
	if bar and not bar.styled then
		bar:RemoveTextures()
		bar:SetStatusBarTexture(UI.Media.statusbar.default)
...
local _hook_GameTooltip_SetDefaultAnchor = function(self, parent)
	...
	if(MOD.Holder.Grip and (not MOD.Holder.Grip:HasMoved())) then
		self:ClearAllPoints()
		if(UI.Inventory and UI.Inventory.BagFrame and UI.Inventory.BagFrame:IsShown()) then
			self:SetPoint("BOTTOMLEFT", UI.Inventory.BagFrame, "TOPLEFT", 0, MOD.DefaultPadding)
		elseif(UI.Dock.BottomRight:GetAlpha() == 1 and UI.Dock.BottomRight:IsShown()) then
			self:SetPoint("BOTTOMLEFT", UI.Dock.BottomRight.Window, "TOPLEFT", 0, MOD.DefaultPadding)
		else
			self:SetPoint("BOTTOMLEFT", UI.Dock.BottomRight.Bar, "TOPLEFT", 0, MOD.DefaultPadding)
		end
	else
...
local function ApplyTooltipSkins()
	...
	for i, tooltip in pairs(tooltips) do
		...
		if(not tooltip.SuperBorder) then
		    ...
		    mask:SetBackdrop(UI.Media.backdrop.tooltip)
...
function MOD:UpdateLocals()
	COMIC_TIPS = UI.db.Tooltip.themed;
	VISIBILITY_COMBAT = UI.db.Tooltip.visibility.combat;
	BAR_HEIGHT = UI.db.Tooltip.healthBar.height;
	SPELL_IDS = UI.db.Tooltip.spellID;
	ON_CURSOR = UI.db.Tooltip.cursorAnchor;
	BAR_TEXT = UI.db.Tooltip.healthBar.text;
	TARGET_INFO = UI.db.Tooltip.targetInfo;
	PLAYER_INFO = UI.db.Tooltip.playerTitles;
	GENDER_INFO = UI.db.Tooltip.playerGender;
	INSPECT_INFO = UI.db.Tooltip.inspectInfo;
	GUILD_INFO = UI.db.Tooltip.guildRanks;
	VISIBILITY_UNITS = UI.db.Tooltip.visibility.unitFrames;
	ITEM_COUNTS = UI.db.Tooltip.itemCount;
end
...
function MOD:Load()
	...
	self.Holder:SetPoint("BOTTOMLEFT", UI.Dock.BottomRight, "TOPLEFT", 0, 56)
	self.Holder:SetSize(130, 20)
	self.Holder:SetFrameLevel(self.Holder:GetFrameLevel() + 50)
	UI:NewAnchor(self.Holder, L["Tooltip"])

	GameTooltipStatusBar:SetHeight(BAR_HEIGHT)
	GameTooltipStatusBar:SetStatusBarTexture(UI.Media.statusbar.default)

	BNToastFrame:ClearAllPoints()
	BNToastFrame:SetPoint("BOTTOMRIGHT", UI.Dock.BottomLeft, "TOPRIGHT", 0, 20)
	--UI:NewAnchor(BNToastFrame, L["BattleNet Frame"], nil, nil, "BattleNetToasts")
	UI:NewAnchor(BNToastFrame, L["BattleNet Frame"])
	...
	GameTooltipStatusBar.text:SetFontObject(CUSTOM_UI_Font_Default)
