...
local cColor = RAID_CLASS_COLORS[UI.class];
...
MOD.MinimapButtons = {}
MOD.Holder = _G["CUSTOM_UI_MinimapFrame"];
MOD.InfoTop = _G["CUSTOM_UI_MinimapInfoTop"];
MOD.InfoBottom = _G["CUSTOM_UI_MinimapInfoBottom"];
local MiniMapCoords = _G["CUSTOM_UI_MiniMapCoords"];
local WorldMapCoords = _G["CUSTOM_UI_WorldMapCoords"];
--local CUSTOM_UI_MinimapFrame = CreateFrame("Frame", "CUSTOM_UI_MinimapFrame", UIParent)
--local WMCoords = CreateFrame('Frame', 'CUSTOM_UI_WorldMapCoords', WorldMapFrame)
--CUSTOM_UI_MinimapFrame:SetSize(MM_WIDTH, MM_HEIGHT);
--[[
##########################################################
GENERAL HELPERS
##########################################################
...
local MMB_OnEnter = function(self)
	if(not UI.db.Maps.minimapbar.mouseover or UI.db.Maps.minimapbar.styleType == "NOANCHOR") then return end
	UIFrameFadeIn(CUSTOM_UI_MiniMapButtonBar, 0.2, CUSTOM_UI_MiniMapButtonBar:GetAlpha(), 1)
	if self:GetName() ~= "CUSTOM_UI_MiniMapButtonBar" then
...
local MMB_OnLeave = function(self)
	if(not UI.db.Maps.minimapbar.mouseover or UI.db.Maps.minimapbar.styleType == "NOANCHOR") then return end
	UIFrameFadeOut(CUSTOM_UI_MiniMapButtonBar, 0.2, CUSTOM_UI_MiniMapButtonBar:GetAlpha(), 0)
	if self:GetName() ~= "CUSTOM_UI_MiniMapButtonBar" then
...
do
	local reserved = {"Node", "Tab", "Pin", "CUSTOM_UI_ConsolidatedBuffs", "GameTimeframe", "HelpOpenTicketButton", "CUSTOM_UI_MinimapFrame", "CUSTOM_UI_EnhancedMinimap", "QueueStatusMinimapButton", "TimeManagerClockButton", "Archy", "GatherMatePin", "GatherNote", "GuildInstance", "HandyNotesPin", "MinimMap", "Spy_MapNoteList_mini", "ZGVMarker", "ZygorGuidesViewerMapIcon"}

	local function UpdateMinimapButtons()
		if(not UI.db.Maps.minimapbar.enable) then return end
		...
		for name,btn in pairs(list) do
			...
			if(UI.db.Maps.minimapbar.styleType == "NOANCHOR") then
				...
				if UI.db.Maps.minimapbar.styleType == "HORIZONTAL"then
		...
		if (UI.db.Maps.minimapbar.styleType ~= "NOANCHOR" and (count > 0)) then
			if UI.db.Maps.minimapbar.styleType == "HORIZONTAL" then
	...
	local StyleMinimapButtons = function()
		...
		if UI.db.Maps.minimapbar.mouseover then
			MMBBar:SetAlpha(0)
		else
			MMBBar:SetAlpha(1)
		end
	...
	function MOD:UpdateMinimapButtonSettings(notimer)
		if(not UI.db.Maps.minimapbar.enable or not MMBBar:IsShown()) then return end
		if(notimer) then
			StyleMinimapButtons()
		else
			UI.Timers:ExecuteTimer(StyleMinimapButtons, 4)
		end
	end
end
...
local function SetLargeWorldMap()
	...
	if UI.db.Maps.tinyWorldMap == true then
		-- WorldMapFrame:SetParent(UI.Screen)
...
local function AdjustMapSize()
	...
	if WORLDMAP_SETTINGS.size == WORLDMAP_FULLMAP_SIZE then
		WorldMapFrame:SetPoint("TOP", UI.Screen, "TOP", 0, 0)
	end
...
local function UpdateWorldMapConfig()
	if(not MM_XY_COORD) then
		if MOD.MMCoordTimer then
			UI.Timers:RemoveLoop(MOD.MMCoordTimer)
		...
		MiniMapCoords.YLabel:SetText("Y:")
		UpdateMiniMapCoords()
		MOD.MMCoordTimer = UI.Timers:ExecuteLoop(UpdateMiniMapCoords, 0.1)
	end

	if((not WMP_XY_COORD) and (not WMM_XY_COORD)) then
		if MOD.WMCoordTimer then
			UI.Timers:RemoveLoop(MOD.WMCoordTimer)
		...
		MOD.WMCoordTimer = UI.Timers:ExecuteLoop(UpdateWorldMapCoords, 0.1)
	end
...
local _hook_WorldMapFrame_OnShow = function()
	...
	if(not UI.db.Maps.tinyWorldMap and not Initialized) then
...
function MOD:RefreshZoneText()
	if(not UI.db.Maps.locationText or UI.db.Maps.locationText == "HIDE") then
		self.InfoTop:Hide();
		self.InfoBottom:Hide();
	else
		if(UI.db.Maps.locationText == "SIMPLE") then
			self.InfoTop:Hide();
...
function MOD:RefreshMiniMap()
	...
	if(self.Holder and self.Holder:IsShown()) then
		...
		if(minimapRotationEnabled or (MM_SHAPE == 'ROUND')) then
			--UI.Dock.TopRight:SetSize(MM_WIDTH, (MM_WIDTH + 4))
			...
			--UI.Dock.TopRight:SetSize(MM_WIDTH, (MM_HEIGHT + 4))
		...
		if(UI.Auras and self.Holder.Grip) then
			UI.Auras:UpdateAuraHolder(MM_HEIGHT, self.Holder.Grip)
		end
	--else
		--UI.Dock.TopRight:SetSize(MM_WIDTH, (MM_HEIGHT + 4))
	end
...
local _hook_BlipTextures = function(self, texture)
	if(UI.db.Maps.customIcons and (texture ~= MOD.Media.customBlips)) then
		self:SetBlipTexture(MOD.Media.customBlips)
	else
		if((not UI.db.Maps.customIcons) and texture ~= MOD.Media.defaultBlips) then
...
function MOD:Load()
	...
	if(UI.db.Maps.customIcons) then
	...
	self.Holder:SetPoint("TOPRIGHT", UI.Screen, "TOPRIGHT", -15, -20)
	self.Holder:SetSize(MM_WIDTH, MM_HEIGHT)
	...
	local mwfont = UI.Media.font.narrator
	...
	UI:NewAnchor(self.Holder, L["Minimap"])

	if(UI.db.Maps.tinyWorldMap) then
		setfenv(WorldMapFrame_OnShow, setmetatable({ UpdateMicroButtons = UI.fubar }, { __index = _G }))
		-- WorldMapFrame:SetParent(UI.Screen)
	...
	--UI:NewAnchor(WorldMapCoords, L["WorldMap Coordinates"])

	DropDownList1:HookScript('OnShow', _hook_DropDownList1)
	WorldFrame:SetAllPoints()

	UI:ManageVisibility(self.Holder)
	...
	if (UI.db.Maps.calendarShortcut) then
		local calendarButton = CreateFrame("Button", "CUSTOM_UI_CalendarButton", MiniMapCoords)
	...
	if (UI.db.Maps.trackingShortcut) then
		local trackingButton = CreateFrame("Button", "CUSTOM_UI_TrackingButton", MiniMapCoords)
		trackingButton:SetSize(22,22)
		if (CUSTOM_UI_CalendarButton) then
			trackingButton:SetPoint("RIGHT", CUSTOM_UI_CalendarButton, "LEFT", -4, 0)
		else
	...
	UI:NewAnchor(MiniMapCoords, L["Minimap ToolBar"])

	if(UI.db.Maps.minimapbar.enable == true) then
		MMBHolder = CreateFrame("Frame", "CUSTOM_UI_MiniMapButtonHolder", self.Holder)
		...
		MMBBar = CreateFrame("Frame", "CUSTOM_UI_MiniMapButtonBar", MMBHolder)
		MMBBar:SetFrameStrata("LOW")
		MMBBar:ClearAllPoints()
		MMBBar:SetPoint("CENTER", MMBHolder, "CENTER", 0, 0)
		MMBBar:SetScript("OnEnter", MMB_OnEnter)
		MMBBar:SetScript("OnLeave", MMB_OnLeave)
		UI:NewAnchor(MMBHolder, L["Minimap Button Bar"])
		self:UpdateMinimapButtonSettings()
	end
	...
	UI.Events:On("CORE_INITIALIZED", MapTriggerFired);
end
