...
local SetButtonSpell = function(self, spellID, spellName, texture)
    ...
    local key = GetBindingKey('CUSTOM_UI_DRAENORZONE')
    if(key) then
...
function MOD:InitializeZoneButton()
	local size = CUSTOM_UI_DraenorButtonHolder:GetHeight()
    local draenor = CreateFrame('Button', "CUSTOM_UI_DraenorZoneAbility", UIParent, 'SecureActionButtonTemplate, SecureHandlerStateTemplate, SecureHandlerAttributeTemplate');
    draenor:SetSize(size,size);
    draenor:SetPoint("CENTER", CUSTOM_UI_DraenorButtonHolder, "CENTER", 0, 0);
    draenor:SetAlpha(0);
  ...
  draenor:SetScript('OnLeave', GameTooltip_Hide);
    ...
    UI:NewAnchor(draenor, L["Zone Ability Button"]);
