...
function MOD:ToggleKeyBindingMode(deactivate, saveRequested)
  if not deactivate then
    Binder.active = true;
    UI:StaticPopupSpecial_Show(CUSTOM_UI_KeyBindPopup)
    MOD:RegisterEvent('PLAYER_REGEN_DISABLED', 'ToggleKeyBindingMode', true, false)
  else
    if saveRequested then 
      SaveBindings(GetCurrentBindingSet())
      UI:AddonMessage(L["Binding Changes Stored"])
    else 
      LoadBindings(GetCurrentBindingSet())
      UI:AddonMessage(L["Binding Changes Discarded"])
    end 
    Binder.active = false;
    Binder:ClearAllPoints()
    Binder:Hide()
    ...
    UI:StaticPopupSpecial_Hide(CUSTOM_UI_KeyBindPopup)
...
local Check_OnClick = function(self)
  if(MOD.bindingsChanged) then 
    UI:StaticPopup_Show("CONFIRM_LOSE_BINDING_CHANGES")
  else 
    if CUSTOM_UI_KeyBindPopupCheckButton:GetChecked() then 
      LoadBindings(2)
      SaveBindings(2)
    else 
      LoadBindings(1)
...
function MOD:LoadKeyBinder()
  ...
  local pop = NewFrame("Frame", "CUSTOM_UI_KeyBindPopup", UIParent)
  ...
  moveText:SetFontObject(CUSTOM_UI_Font_Default)
  ...
  local checkButton = NewFrame("CheckButton", "CUSTOM_UI_KeyBindPopupCheckButton", pop, "OptionsCheckButtonTemplate")
  checkButton:SetStyle("CheckButton")
  _G["CUSTOM_UI_KeyBindPopupCheckButtonText"]:SetText(CHARACTER_SPECIFIC_KEYBINDINGS)
  ...
  local saveButton = NewFrame("Button", "CUSTOM_UI_KeyBindPopupSaveButton", pop, "OptionsButtonTemplate")
  saveButton:SetWidth(150)
  saveButton:SetStyle("Button")
  _G["CUSTOM_UI_KeyBindPopupSaveButtonText"]:SetText(L["Save"])
  saveButton:SetScript("OnClick", Save_OnClick)

  local discardButton = NewFrame("Button", "CUSTOM_UI_KeyBindPopupDiscardButton", pop, "OptionsButtonTemplate")
  discardButton:SetWidth(150)
  discardButton:SetStyle("Button")
  _G["CUSTOM_UI_KeyBindPopupDiscardButtonText"]:SetText(L["Discard"])
  ...
  UI:AddSlashCommand("kb", "Toggle Key Binding Mode", MOD.ToggleKeyBindingMode);
  UI:AddSlashCommand("bind", "Toggle Key Binding Mode", MOD.ToggleKeyBindingMode);

  UI.SystemAlert["KEYBIND_MODE"] = {
  ...
  UI.SystemAlert["CONFIRM_LOSE_BINDING_CHANGES"] = {
