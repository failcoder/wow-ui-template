...
local DEFAULT_MAIN_ANCHOR = _G["CUSTOM_UI_DockBottomCenter"];
MOD.ButtonCache = {};
MOD.MainAnchor = CreateFrame("Frame", "CUSTOM_UI_ActionBarMainAnchor");
--[[
##########################################################
LOCAL VARS
##########################################################
...
local function SaveActionButton(button, noStyle)
	...
	cooldown.SizeOverride = UI.db.ActionBars.cooldownSize
...
local function ModifyActionButton(button, noStyle)
	...
	if count then
		...
		UI:FontManager(count, "number")
	end

	if UI.db.ActionBars.hotkeytext then
		hotkey:ClearAllPoints()
		hotkey:SetAllPoints()
		hotkey:SetFontObject(CUSTOM_UI_Font_Default)
...
do
	...
	function SetSpellFlyoutHook()
		...
		UI.Timers:ExecuteTimer(QualifyFlyouts, 5)
	end
end
--[[
##########################################################
...
function MOD:UpdateBarBindings(pet, stance)
	if stance == true then
		local bar = _G["CUSTOM_UI_StanceBar"]
		local bindText = bar.binding

	  	for i=1,NUM_STANCE_SLOTS do
	  		local name = ("CUSTOM_UI_StanceBarButton%s"):format(i)
	  		local hkname = ("CUSTOM_UI_StanceBarButton%sHotKey"):format(i)
  	...
  	if pet == true then
  		local bar = _G["CUSTOM_UI_PetActionBar"]
		local bindText = bar.binding
...
function MOD:UpdateAllBindings(event)
	...
	for i = 1, TOTAL_BARS do
		local barName = ("CUSTOM_UI_ActionBar%d"):format(i)
...
function MOD:UpdateBarPagingDefaults()
	...
	local mainbar = _G["CUSTOM_UI_ActionBar1"]
	if(mainbar) then
		if UI.db.ActionBars.Bar1.useCustomPaging then
			custom = UI.db.ActionBars.Bar1.customPaging[UI.class];
		else
			custom = ""
		end
	...
	for i=2, TOTAL_BARS do
		...
		local bar = _G["CUSTOM_UI_Action" .. id]
		if(bar and UI.db.ActionBars[id].useCustomPaging) then
			bar.conditions = UI.db.ActionBars[id].customPaging[UI.class];
		end
	end

	if((not UI.db.ActionBars.enable or InCombatLockdown()) or not self.isInitialized) then return end
	...
	if (UI.db.ActionBars.Bar2.enable and not Bar2Option:GetChecked()) or (not UI.db.ActionBars.Bar2.enable and Bar2Option:GetChecked())  then
		Bar2Option:Click()
	end

	if (UI.db.ActionBars.Bar3.enable and not Bar3Option:GetChecked()) or (not UI.db.ActionBars.Bar3.enable and Bar3Option:GetChecked())  then
		Bar3Option:Click()
	end

	if not UI.db.ActionBars.Bar5.enable and not UI.db.ActionBars.Bar4.enable then
	...
	elseif not UI.db.ActionBars.Bar5.enable then
	...
	elseif (UI.db.ActionBars.Bar4.enable and not Bar4Option:GetChecked()) or (not UI.db.ActionBars.Bar4.enable and Bar4Option:GetChecked()) then
		Bar4Option:Click()
	elseif (UI.db.ActionBars.Bar5.enable and not Bar5Option:GetChecked()) or (not UI.db.ActionBars.Bar5.enable and Bar5Option:GetChecked()) then
		Bar5Option:Click()
	end
end
--[[
...
function MOD:RefreshMainAnchor()
	...
	for i = 1, TOTAL_BARS do
		id = ("Bar%d"):format(i);
		bar = _G[("CUSTOM_UI_Action%s"):format(id)];
		if((i < 3) or (i > 5)) then
			local enabled = UI.db.ActionBars[id].enable;
...
do
	...
	local function _getPage(bar, defaultPage, condition)
		local page = UI.db.ActionBars[bar].customPaging[UI.class]
	...
	function MOD:RefreshBar(id)
		...
		local db = UI.db.ActionBars[id]

		if(id == "Pet") then
			bar = _G["CUSTOM_UI_PetActionBar"]
			isPet = true
		elseif(id == "Stance") then
			bar = _G["CUSTOM_UI_StanceBar"]
			isStance = true
		else
			bar = _G[("CUSTOM_UI_Action%s"):format(id)]
		end
		...
		if(isPet or isStance) then
			...
			local moverName = ("CUSTOM_UI_Action%d_MOVE"):format(id);
...
local Vehicle_Updater = function()
	local bar = _G["CUSTOM_UI_ActionBar1"]
	local space = UI.db.ActionBars["Bar1"].buttonspacing
	local total = UI.db.ActionBars["Bar1"].buttons;
	local rows = UI.db.ActionBars["Bar1"].buttonsPerRow;
	local size = UI.db.ActionBars["Bar1"].buttonsize
	local point = UI.db.ActionBars["Bar1"].point;
	local columns = ceil(total / rows)
	if (HasOverrideActionBar() or HasVehicleActionBar()) and total == 12 then
		bar.backdrop:ClearAllPoints()
		bar.backdrop:SetPoint(UI.db.ActionBars["Bar1"].point, bar, UI.db.ActionBars["Bar1"].point)
...
local CUSTOM_UIOptionsPanel_OnEvent = function()
...
local CUSTOM_UIButton_ShowOverlayGlow = function(self)
...
local ResetAllBindings = function(self)
	...
	local bar
	for i = 1, TOTAL_BARS do
		bar = _G[("CUSTOM_UI_ActionBar%d"):format(i)]
		if(bar) then
			ClearOverrideBindings(bar)
		end
	end

	ClearOverrideBindings(_G["CUSTOM_UI_PetActionBar"])
	ClearOverrideBindings(_G["CUSTOM_UI_StanceBar"])
...
local barBindingIndex = {
	...
	"CUSTOM_UIACTIONBAR6BUTTON%d",
	"CUSTOM_UIACTIONBAR7BUTTON%d",
	"CUSTOM_UIACTIONBAR8BUTTON%d",
	"CUSTOM_UIACTIONBAR9BUTTON%d",
	"CUSTOM_UIACTIONBAR10BUTTON%d"
}
...
CreateActionBars = function(self)
	for i = 1, TOTAL_BARS do
		...
		local barName = ("CUSTOM_UI_Action%s"):format(barID)
		local buttonMax = NUM_ACTIONBAR_BUTTONS
		local space = UI.db.ActionBars["Bar"..i].buttonspacing;
		local enabled = UI.db.ActionBars["Bar"..i].enable;
		...
		if(i == 3) then
			thisBar:SetPoint("BOTTOMLEFT", _G["CUSTOM_UI_ActionBar1"], "BOTTOMRIGHT", space, 0)
		elseif(i == 4) then
			thisBar:SetPoint("RIGHT", UI.Screen, "RIGHT", -space, 0)
		elseif(i == 5) then
			thisBar:SetPoint("BOTTOMRIGHT", _G["CUSTOM_UI_ActionBar1"], "BOTTOMLEFT", -space, 0)
		else
...
do
	local function SetStanceBarButtons()
		...
		for i = 1, maxButtons do
			local button = _G["CUSTOM_UI_StanceBarButton"..i]
			local icon = _G["CUSTOM_UI_StanceBarButton"..i.."Icon"]
			if i <= maxForms then
				texture, name, isActive, isCastable = GetShapeshiftFormInfo(i)
				if texture == "Interface\\Icons\\Spell_Nature_WispSplode" and UI.db.ActionBars.Stance.style == "darkenInactive" then
				...
				if isActive then
					...
					if maxForms > 1 and currentForm > 0 then
						...
						if UI.db.ActionBars.Stance.style == "darkenInactive" then
	...
	local function UpdateShapeshiftForms(self, event)
	  if InCombatLockdown() or not _G["CUSTOM_UI_StanceBar"] then return end

	  local stanceBar = _G["CUSTOM_UI_StanceBar"];
	  ...
	  for i = 1, NUM_STANCE_SLOTS do
		if(not stanceBar.buttons[i]) then
		  stanceBar.buttons[i] = CreateFrame("CheckButton", format("CUSTOM_UI_StanceBarButton%d", i), stanceBar, "StanceButtonTemplate")
	...
	local function UpdateShapeshiftCD()
	  ...
	  for i = 1, NUM_STANCE_SLOTS do
		if i  <= maxForms then
		  local cooldown = _G["CUSTOM_UI_StanceBarButton"..i.."Cooldown"]
	...
	CreateStanceBar = function(self)
	  local barID = "Stance";
	  local maxForms = GetNumShapeshiftForms();
	  local stanceBar = NewActionBar("CUSTOM_UI_StanceBar")
	  stanceBar.binding = "CLICK CUSTOM_UI_StanceBarButton%d:LeftButton"
	  ...
	  for i = 1, NUM_STANCE_SLOTS do
		stanceBar.buttons[i] = _G["CUSTOM_UI_StanceBarButton"..i]
	  end
	  ...
	  UI:NewAnchor(stanceBar, L["Stance Bar"])
	  self:RefreshBar("Stance")
	  SetStanceBarButtons()
...
do
	...
	CreatePetBar = function(self)
		local barID = "Pet";
		local petBar = NewActionBar("CUSTOM_UI_PetActionBar")
		...
		UI:NewAnchor(petBar, L["Pet Bar"])
	end
end

local CreateExtraBar = function(self)
	local specialBar = CreateFrame("Frame", "CUSTOM_UI_SpecialAbility", UI.Screen)
	specialBar:SetPoint("BOTTOM", UI.Screen, "BOTTOM", 0, 360)
	...
	UI:NewAnchor(specialBar, L["Extra Action Button"])
end
--[[
##########################################################
DEFAULT REMOVAL
...
local function pushSpellToActionBar(self,spellID,slotIndex,slotPos)
	local slotParent = _G["CUSTOM_UI_Bar1Button"..slotPos];
...
local function UpdateActionBarOptions()
	if InCombatLockdown() or not UI.db.ActionBars.IsLoaded then return end
	if (UI.db.ActionBars.Bar2.enable ~= InterfaceOptionsActionBarsPanelBottomRight:GetChecked()) then
		InterfaceOptionsActionBarsPanelBottomRight:Click()
	end
	if (UI.db.ActionBars.Bar3.enable ~= InterfaceOptionsActionBarsPanelRightTwo:GetChecked()) then
		InterfaceOptionsActionBarsPanelRightTwo:Click()
	end
	if (UI.db.ActionBars.Bar4.enable ~= InterfaceOptionsActionBarsPanelRight:GetChecked()) then
		InterfaceOptionsActionBarsPanelRight:Click()
	end
	if (UI.db.ActionBars.Bar5.enable ~= InterfaceOptionsActionBarsPanelBottomLeft:GetChecked()) then
...
function MOD:Load()
	...
	self.MainAnchor:SetParent(UI.Screen)
	self:UpdateLocals()
	...
	NewHook("BlizzardOptionsPanel_OnEvent", CUSTOM_UIOptionsPanel_OnEvent)
	NewHook("ActionButton_ShowOverlayGlow", CUSTOM_UIButton_ShowOverlayGlow)
	if not GetCVarBool("lockActionBars") then SetCVar("lockActionBars", 1) end
	SetSpellFlyoutHook()

	self.IsLoaded = true

	UI.SystemAlert["BAR6_CONFIRMATION"] = {
		...
		button2 = NO,
		OnAccept = function(a)
			if UI.db.ActionBars["BAR6"].enable ~= true then
				UI.db.ActionBars.Bar6.enable = true;
				UpdateActionBarOptions()
			else
				UI.db.ActionBars.Bar6.enable = false;
				UpdateActionBarOptions()
			end
		end,
		OnCancel = UI.fubar,
