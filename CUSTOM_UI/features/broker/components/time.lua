...
local function ConvertTime(h, m)
	local AmPm
	if UI.db.Reports.time24 == true then
		return h, m, -1
	else
		if h >= 12 then
...
local function CalculateTimeValues(tooltip)
	if (tooltip and UI.db.Reports.localtime) or (not tooltip and not UI.db.Reports.localtime) then
...
Report.OnEnter = function(self)
	...
	Reports.ToolTip:AddLine(" ")
	if AmPm == -1 then
		Reports.ToolTip:AddDoubleLine(UI.db.Reports.localtime and TIMEMANAGER_TOOLTIP_REALMTIME or TIMEMANAGER_TOOLTIP_LOCALTIME,
