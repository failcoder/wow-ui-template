...
local Gold_OnEvent = function(self, event, ...)
	...
	self.text:SetText(UI:FormatCurrency(current, UI.db.Reports.shortGold))
...
Report.OnEnter = function(self)
	...
	Reports.ToolTip:AddDoubleLine(L["Earned:"],UI:FormatCurrency(gains),1,1,1,1,1,1)
	Reports.ToolTip:AddDoubleLine(L["Spent:"],UI:FormatCurrency(loss),1,1,1,1,1,1)
	if gains < loss then
		Reports.ToolTip:AddDoubleLine(L["Deficit:"],UI:FormatCurrency(gains - loss),1,0,0,1,1,1)
	elseif (gains - loss) > 0 then
		Reports.ToolTip:AddDoubleLine(L["Profit:"],UI:FormatCurrency(gains - loss),0,1,0,1,1,1)
	end
	Reports.ToolTip:AddLine(" ")
	local networth = Reports.Accountant["gold"][playerName];
	Reports.ToolTip:AddLine(L[playerName..": "])
	Reports.ToolTip:AddDoubleLine(L["Total: "], UI:FormatCurrency(networth), 1,1,1,1,1,1)
	Reports.ToolTip:AddLine(" ")
	...
	for name,amount in pairs(self.InnerData)do
		if(name ~= playerName and name ~= 'total') then
			...
			Reports.ToolTip:AddDoubleLine(name, UI:FormatCurrency(amount), 1,1,1,1,1,1)
		end
	end
