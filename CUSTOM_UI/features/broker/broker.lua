...
local UI = select(2, ...)
local L = UI.L
local UILib = _G.Librarian("Registry")
local LSM = _G.LibStub("LibSharedMedia-3.0")
local LDB = LibStub("LibDataBroker-1.1", true)
local MOD = UI:NewPackage("Reports", L["Informative Panels"]);

MOD.Sockets = {};
MOD.Plugins = {};
MOD.References = {};
MOD.ToolTip = CreateFrame("GameTooltip", "CUSTOM_UI_Report_ToolTip", UIParent, "GameTooltipTemplate");
...
local UpdateAnchor = function()
	local backdrops, width, height = UI.db.Reports.backdrop
...
local _hook_TooltipOnShow = function(self)
	self:SetBackdrop({
		bgFile = UI.Media.background.default,
...
local function GetDataSlot(parent, index)
	if(not parent.Stats.Slots[index]) then
		...
		if(not UI.db.Reports.backdrop) then
			...
			slot.barframe.bg:SetGradient(unpack(UI.Media.gradient.dark))
			slot.Panel:Hide()
		else
			slot.barframe:SetPoint("TOPLEFT", slot, "TOPLEFT", 24, -2)
			slot.barframe:SetPoint("BOTTOMRIGHT", slot, "BOTTOMRIGHT", -2, 2)
		...
		slot.barframe.bar:SetStatusBarTexture(UI.Media.statusbar.default)

		slot.barframe.bar.extra = CreateFrame("StatusBar", nil, slot.barframe.bar)
		slot.barframe.bar.extra:SetAllPoints()
		slot.barframe.bar.extra:SetStatusBarTexture(UI.Media.statusbar.default)
		...
		UI:FontManager(slot.text, "data")
		if(UI.db.Reports.backdrop) then
...
local Socket_OnClick = function(self, button)
	if IsAltKeyDown() then
		UI.Dropdown:Open(self, self.MenuList, "Select Report");
...
function MOD:EnableReport(socket, name)
	...
	if obj.OnEvent and obj.events then
		...
		Socket_OnEvent(socket, "CUSTOM_UI_FORCE_RUN")
	end
...
do
	...
	local BG_OnClick = function()
		...
		UI:AddonMessage(L["Battleground statistics temporarily hidden, to show type \"/sv bg\" or \"/sv pvp\""])
	end
	...
	local function setMenuLists()
		...
		for reportIndex, parent in ipairs(anchorTable) do
			...
			for i = 1, numPoints do
				...
				tinsert(subList,{text = NONE, func = function() UI.db.REPORT_SLOTS[slotKey][i] = ""; MOD:UpdateAllReports() end});
				for _,name in pairs(statMenu) do
					tinsert(subList,{text = name, func = function() UI.db.REPORT_SLOTS[slotKey][i] = name; MOD:UpdateAllReports() end});
				end
			end
		end
	...
	function MOD:UpdateAllReports()
		...
		local docks = UI.db.REPORT_SLOTS
		local allowPvP = (UI.db.Reports.battleground and not ForceHideBGStats) or false
		...
		local baseWidth, dockHeight = UI.Dock.BottomCenter:GetSize()
...
function MOD:NewHolder(parent, maxCount, tipAnchor, pvpSet, customTemplate, isVertical)
	...
	if(slotKey and (not UI.db.REPORT_SLOTS[slotKey])) then
		UI.db.REPORT_SLOTS[slotKey] = {};
        for i = 1, maxCount do
        	UI.db.REPORT_SLOTS[slotKey][i] = "None"
        end
	end
...
local function SlashPvPStats()
	...
	MOD:UpdateAllReports()
	UI:AddonMessage(L['Battleground statistics will now show again if you are inside a battleground.'])
end
--[[
##########################################################
...
function MOD:ReportAdded(event, dataName, dataObj, noupdate)
	...
	if(t) then
		if(t == "data source" or t == "launcher") then
			...
			UI:HandleError("LibDataBroker", dataName, "Data type (" .. t .. ") is not allowed")
		end
	--else
		--UI:HandleError("LibDataBroker", dataName, "CAUTION: This is a badly written addon")
	end
end
...
function MOD:Load()
	...
	hexHighlight = UI:HexColor("highlight") or "FFFFFF"
	local hexClass = classColor.colorStr
	BGStatString = "|cff" .. hexHighlight .. "%s: |c" .. hexClass .. "%s|r";

	local accountant = UILib:NewGlobal("Accountant")
	accountant[playerRealm] = accountant[playerRealm] or {};
	self.Accountant = accountant[playerRealm];

	local subsettings = UILib:NewGlobal("ReportSubSettings")
	subsettings[playerRealm] = subsettings[playerRealm] or {};
	self.SubSettings = subsettings[playerRealm];

	--BOTTOM CENTER BARS
	local bottomLeft = CreateFrame("Frame", "CUSTOM_UI_ReportsGroup1", UI.Dock.BottomCenter)
	bottomLeft:SetSize(dockWidth, dockHeight)
	bottomLeft:SetPoint("BOTTOMLEFT", UI.Dock.BottomCenter, "BOTTOMLEFT", 0, 0)
	UI:NewAnchor(bottomLeft, L["Data Reports 1"])
	self:NewHolder(bottomLeft, 3, "ANCHOR_CURSOR")

	local bottomRight = CreateFrame("Frame", "CUSTOM_UI_ReportsGroup2", UI.Dock.BottomCenter)
	bottomRight:SetSize(dockWidth, dockHeight)
	bottomRight:SetPoint("BOTTOMRIGHT", UI.Dock.BottomCenter, "BOTTOMRIGHT", 0, 0)
	UI:NewAnchor(bottomRight, L["Data Reports 2"])
	self:NewHolder(bottomRight, 3, "ANCHOR_CURSOR")

	--TOP CENTER BARS
	local topLeft = CreateFrame("Frame", "CUSTOM_UI_ReportsGroup3", UI.Dock.TopCenter)
	topLeft:SetSize(dockWidth, dockHeight)
	topLeft:SetPoint("TOPLEFT", UI.Dock.TopCenter, "TOPLEFT", 0, 0)

	UI:NewAnchor(topLeft, L["Data Reports 3"])
	self:NewHolder(topLeft, 3, "ANCHOR_CURSOR", 1)

	local topRight = CreateFrame("Frame", "CUSTOM_UI_ReportsGroup4", UI.Dock.TopCenter)
	topRight:SetSize(dockWidth, dockHeight)
	topRight:SetPoint("TOPRIGHT", UI.Dock.TopCenter, "TOPRIGHT", 0, 0)

	UI:NewAnchor(topRight, L["Data Reports 4"])
	...
	-- self.ToolTip:SetParent(UI.Screen)
	...
	UI:AddSlashCommand("bg", slashDesc, SlashPvPStats);
	UI:AddSlashCommand("pvp", slashDesc, SlashPvPStats);
	UI.Events:On("DOCKS_UPDATED", MOD.UpdateAllReports, true);
end
