...
local function StyleDXEBar(bar)
	...
	bar.bg:SetTexture("")
	bar.border.Show = UI.fubar
	bar.border:Hide()
	bar.statusbar:SetStatusBarTexture(UI.Media.statusbar.default)
	bar.statusbar:ClearAllPoints()
	bar.statusbar:InsetPoints()
	bar.righticon:SetStyle("!_Frame", "Default")
	bar.righticon.border.Show = UI.fubar
	...
	bar.lefticon.border.Show = UI.fubar
...
local function StyleDXE()
	...
	DXE.LayoutHealthWatchers = function(frame)
		...
		for i,hw in ipairs(frame.HW) do
			if hw:IsShown() then
				...
				hw.border.Show = UI.fubar
				hw.border:Hide()
				hw.healthbar:SetStatusBarTexture(UI.Media.statusbar.default)
			end
		end
	end
	...
	DXEDB["profiles"][playerKey]["Globals"]["BarTexture"] = UI.Media.statusbar.default
	DXEDB["profiles"][playerKey]["Globals"]["Border"] = "None"
	DXEDB["profiles"][playerKey]["Globals"]["Font"] = UI.Media.font.dialog
	DXEDB["profiles"][playerKey]["Globals"]["TimerFont"] = UI.Media.font.dialog
end
