...
local function Widget_ScrollStyle(frame, arg)
	return UI.API:Set("ScrollBar", frame) 
end 
...
local WidgetButton_OnClick = function(self)
	...
	if(obj and obj.pullout and obj.pullout.frame) then
		UI.API:Set("Frame", obj.pullout.frame, "Default", true)
	end
end
...
local function StyleAceGUI(event, addon)
	...
	RegisterAsWidget = function(self, widget)
		...
		if(widgetType == "MultiLineEditBox") then 
			...
			UI.API:Set("!_Frame", widgetFrame, "Default", true)
			UI.API:Set("Frame", widget.scrollBG, "Lite", true) 
			Widget_ButtonStyle(widget.button)
			UI.API:Set("ScrollBar", widget.scrollBar) 
		...
		elseif(widgetType == "CheckBox") then 
			...
			if not widget.styledCheckBG then 
				...
				UI.API:Set("!_Frame", widget.styledCheckBG, "CheckButton")
			end 
		...
		elseif(widgetType == "EditBox") then 
			...
			UI.API:Set("EditBox", widgetEditbox, nil, 15, 2, -2)
		...
		elseif(widgetType == "Slider") then 
			...
			UI.API:Set("!_Frame", widgetSlider, "Bar")
	...
	RegisterAsContainer = function(self, widget)
		...
		elseif widgetType == "Frame" then
			...
			UI.API:Set("Window", widgetParent)
		elseif(ProxyType[widgetType]) then

			if widget.treeframe then 
				UI.API:Set("Frame", widget.treeframe, "Transparent")
				...
				widget.CreateButton = function(self)
					...
					newButton.toggleText:SetFont([[Interface\AddOns\CUSTOM_UI\media\fonts\Default.ttf]], 19)
					newButton.toggleText:SetPoint("CENTER")
					newButton.toggleText:SetText("*")
					return newButton 
				end
			elseif(not widgetParent.Panel) then
				UI.API:Set("Frame", widgetParent, "Lite")
			end
...
--MOD:SaveAddonStyle("CUSTOM_UI_ConfigOMatic", StyleAceGUI, nil, true)
