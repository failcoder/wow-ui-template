...
local function StyleSexyCooldownBar(bar)
	...
	UI.API:Set("Frame", bar)
	UI:ManageVisibility(bar)
	if MOD:IsAddonReady("SexyCooldown_Anchored") then
		if(UI.ActionBars and UI.ActionBars.MainAnchor) then
			bar:ClearAllPoints()
			bar:SetPoint('BOTTOMRIGHT', UI.ActionBars.MainAnchor, 'TOPRIGHT', 0, 4)
			bar:SetPoint("BOTTOMLEFT", UI.ActionBars.MainAnchor, "TOPLEFT", 0, 4)
			bar:SetHeight(UI.ActionBars.MainAnchor:GetHeight())
		end
	end
end
...
local function StyleSexyCooldownIcon(bar, icon)
	if not icon.styled then
		UI.API:Set("Frame", icon, false, true)
		UI.API:Set("Frame", icon.overlay,"Transparent",true)
		icon.styled = true
	end
