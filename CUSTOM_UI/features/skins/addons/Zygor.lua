...
local function StyleZygor()
	...
	UI.API:Set("Window", ZygorGuidesViewerFrame)
	ZygorGuidesViewerFrame_Border:RemoveTextures(true)
	UI.API:Set("Frame", ZygorGuidesViewer_CreatureViewer, 'Model')

	for i = 1, 6 do
		UI.API:Set("Frame", _G['ZygorGuidesViewerFrame_Step'..i], 'Default')
	end
	...
	if(UI.Maps and UI.db.Maps) then 
		if(UI.db.Maps.customIcons) then
			Minimap:SetBlipTexture(UI.Maps.Media.customBlips)
			Minimap.SetBlipTexture = function() return end
		else
			Minimap:SetBlipTexture(UI.Maps.Media.defaultBlips)
			Minimap.SetBlipTexture = function() return end
		end
	end
end
