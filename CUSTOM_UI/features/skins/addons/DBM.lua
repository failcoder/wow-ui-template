	...
	for bar in self:GetBarIterator() do
		if not bar.injected then
			bar.ApplyStyle = function()
				...
				name:SetFont(UI.Media.font.default, 12, 'OUTLINE')
				...
				timer:SetFont(UI.Media.font.default, 12, 'OUTLINE')
...
local StyleBossTitle = function()
	...
	if not anchor.styled then
		...
		if header[1]:IsObjectType('FontString') then
			header[1]:SetFontObject(CUSTOM_UI_Font_Default)
...
local StyleBoss = function()
	...
	while _G[format('DBM_BossHealth_Bar_%d', count)] do
		...
		timer:SetFontObject(CUSTOM_UI_Font_Default)
		count = count + 1
	end
end
