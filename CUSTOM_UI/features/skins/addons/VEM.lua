	...
	for bar in self:GetBarIterator() do
		if not bar.injected then
			bar.ApplyStyle = function()
				...
				texture:SetTexture(UI.Media.statusbar.default)
				...
				name:SetFont(UI.Media.font.dialog, 12, 'OUTLINE')
				timer:SetFont(UI.Media.font.dialog, 12, 'OUTLINE')
...
local StyleBossTitle = function()
	...
	if not anchor.styled then
		...
		if header[1]:IsObjectType('FontString') then
			header[1]:SetFont(UI.Media.font.dialog, 12, 'OUTLINE')
...
local StyleBoss = function()
	...
	while _G[format('VEM_BossHealth_Bar_%d', count)] do
		...
		progress:SetStatusBarTexture(UI.Media.statusbar.default)
...
local function StyleVEM(event, addon)
	...
	if addon == 'VEM-GUI' then
		VEM_GUI_OptionsFrame:HookScript('OnShow', function()
			UI.API:Set("Frame", VEM_GUI_OptionsFrame)
			UI.API:Set("Frame", VEM_GUI_OptionsFrameBossMods)
			UI.API:Set("Frame", VEM_GUI_OptionsFrameVEMOptions)
			UI.API:Set("Frame", VEM_GUI_OptionsFramePanelContainer, 'Transparent', true)
		end)
		UI.API:Set("Tab", VEM_GUI_OptionsFrameTab1)
		UI.API:Set("Tab", VEM_GUI_OptionsFrameTab2)
		VEM_GUI_OptionsFrameOkay:SetStyle("Button")
		VEM_GUI_OptionsFrameWebsiteButton:SetStyle("Button")
		UI.API:Set("ScrollBar", VEM_GUI_OptionsFramePanelContainerFOVScrollBar)
