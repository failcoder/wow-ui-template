...
local function StyleTextureIcon(frame)
	...
	frame.Texture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
...
local function StyleIconElement(frame)
	...
	if(frame.Icon) then
		...
		if(frame.Quantity) then
	    	frame.Quantity:SetFontObject(CUSTOM_UI_Font_Number)
...
local function _hook_GarrisonMissionFrame_SetItemRewardDetails(item)
    ...
    if(item.Icon) then
		...
		item.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
    ...
    if(item.Quantity) then
    	item.Quantity:SetFontObject(CUSTOM_UI_Font_Number)
    ...
    for i = 1, #reagents do
    	if(reagents[i] and (not reagents[i].Panel)) then
        	...
        	if(reagents[i].Icon) then
				reagents[i].Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
			end
		end
    end
end
        ...
        if(index <= numFollowers) then
	        ...
	        if(not button.Panel) then
				...
				if(button.XPBar) then
					button.XPBar:SetTexture(UI.Media.statusbar.default)
...
local _hook_GarrisonFollowerPage_ShowFollower = function(self, followerID)
    ...
    if(not self.XPBar.Panel) then
		...
		self.XPBar:SetStatusBarTexture(UI.Media.statusbar.default)
...
local function LoadGarrisonStyle()
	...
	##############################################################################
	--]]
	UI.API:Set("Window", GarrisonBuildingFrame, true, false, 1, 0, 4)
	...
	--UI.API:Set("Tab", GarrisonBuildingFrame.BuildingList.Tab1)
	...
	--UI.API:Set("Tab", GarrisonBuildingFrame.BuildingList.Tab2)
	...
	--UI.API:Set("Tab", GarrisonBuildingFrame.BuildingList.Tab3)
	...
	UI.API:Set("CloseButton", GarrisonBuildingFrame.CloseButton)
  ...
  hooksecurefunc("GarrisonBuildingInfoBox_ShowFollowerPortrait", _hook_GarrisonBuildingInfoBox_ShowFollowerPortrait)
	...
	##############################################################################
	--]]
	UI.API:Set("Window", GarrisonLandingPage, true, false, 1, 0, 0)
	UI.API:Set("Skin", GarrisonLandingPage.FollowerTab, 12, 0, -2, 30)
	...
	UI.API:Set("Tab", GarrisonLandingPageTab1, nil, 10, 4)
	UI.API:Set("Tab", GarrisonLandingPageTab2, nil, 10, 4)
	UI.API:Set("Tab", GarrisonLandingPageTab3, nil, 10, 4)
	UI.API:Set("ScrollBar", GarrisonLandingPageListScrollFrame)
	UI.API:Set("ScrollBar", GarrisonLandingPageReportListListScrollFrame)
	UI.API:Set("ScrollBar", GarrisonLandingPageShipFollowerListListScrollFrame)
	UI.API:Set("CloseButton", GarrisonLandingPage.CloseButton)
	...
	##############################################################################
	--]]
	UI.API:Set("Window", GarrisonMissionFrame, true, false, 1, 0, 4)
	...
	missionChance:SetFontObject(CUSTOM_UI_Font_Number_Huge)
	local chanceLabel = GarrisonMissionFrame.MissionTab.MissionPage.RewardsFrame.ChanceLabel
	chanceLabel:SetFontObject(CUSTOM_UI_Font_Header)
	...
	UI.API:Set("Tab", GarrisonMissionFrameTab1)
	UI.API:Set("Tab", GarrisonMissionFrameTab2)
	UI.API:Set("Tab", GarrisonMissionFrameMissionsTab1, nil, 10, 4)
	UI.API:Set("Tab", GarrisonMissionFrameMissionsTab2, nil, 10, 4)
	UI.API:Set("ScrollBar", GarrisonMissionFrameMissionsListScrollFrame)
	UI.API:Set("ScrollBar", GarrisonMissionFrameFollowersListScrollFrame)
	UI.API:Set("Skin", GarrisonMissionFrame.FollowerTab)
	UI.API:Set("EditBox", GarrisonMissionFrameFollowers.SearchBox)
	UI.API:Set("CloseButton", GarrisonMissionFrame.CloseButton)
	UI.API:Set("CloseButton", GarrisonMissionFrame.MissionTab.MissionPage.CloseButton)
	--UI.API:Set("Button", GarrisonMissionFrame.MissionTab.MissionPage.MinimizeButton)
	...
	##############################################################################
	--]]
	UI.API:Set("Window", GarrisonCapacitiveDisplayFrame, true, false, 1, 0, 4)
	...
	GarrisonCapacitiveDisplayFrame.CapacitiveDisplay.ShipmentIconFrame.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
  ...
  for i = 1, #reagents do
	...
	if(GarrisonCapacitiveDisplayFrame.CreateAllWorkOrdersButton) then
		...
		UI.API:Set("PageButton", GarrisonCapacitiveDisplayFrame.DecrementButton, false, true)
		UI.API:Set("EditBox", GarrisonCapacitiveDisplayFrame.Count)
		UI.API:Set("PageButton", GarrisonCapacitiveDisplayFrame.IncrementButton)
	end
	...
	UI.NPC:Register(GarrisonCapacitiveDisplayFrame, GarrisonCapacitiveDisplayFrameTitleText)
	GarrisonCapacitiveDisplayFrame.StartWorkOrderButton:HookScript('OnClick', function() UI.NPC:PlayerTalksFirst() end)
	--[[
	##############################################################################
	RECRUITER FRAME
	##############################################################################
	--]]
	UI.API:Set("Window", GarrisonRecruiterFrame, true)
	UI.API:Set("Window", GarrisonRecruitSelectFrame, true)
	...
	UI.API:Set("DropDown", GarrisonRecruiterFramePickThreatDropDown)
	UI.API:Set("CloseButton", GarrisonRecruiterFrame.CloseButton)
	UI.API:Set("CloseButton", GarrisonRecruitSelectFrame.CloseButton)
	UI.API:Set("Button", GarrisonRecruiterFrame.Pick.ChooseRecruits)
	UI.API:Set("Button", GarrisonRecruiterFrame.Random.ChooseRecruits)
	...
	##############################################################################
	--]]
	UI.API:Set("Window", GarrisonShipyardFrame, true)
	GarrisonShipyardFrame.BorderFrame:RemoveTextures()
	UI.API:Set("CloseButton", GarrisonShipyardFrame.BorderFrame.CloseButton2)
	...
	UI.API:Set("CloseButton", GarrisonShipyardFrame.MissionTab.MissionPage.CloseButton)
	...
	UI.API:Set("ScrollBar", GarrisonShipyardFrameFollowersListScrollFrame)
	UI.API:Set("Skin", GarrisonShipyardFrame.FollowerTab, 12, 0, -2, 30)
	UI.API:Set("EditBox", GarrisonShipyardFrameFollowers.SearchBox)

	StyleMissionComplete(GarrisonShipyardFrame)
	UI.API:Set("Tab",GarrisonShipyardFrameTab1)
	UI.API:Set("Tab",GarrisonShipyardFrameTab2)
	--[[
	##############################################################################
	FOLLOWER HOOKS
	##############################################################################
