...
local FAV_ICON = UI.Media.icon.star
...
local function CollectionsJournalStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.mounts ~= true then return end 

	UI.API:Set("Window", CollectionsJournal)

	CollectionsJournalPortrait:Hide()
	UI.API:Set("Tab", CollectionsJournalTab1)
	UI.API:Set("Tab", CollectionsJournalTab2)
	UI.API:Set("CloseButton", CollectionsJournalCloseButton)
	...
	UI.API:Set("ScrollBar", MountJournalListScrollFrameScrollBar)
	...
	for i = 1, #buttons do
		...
		if(button) then
			...
			local bar = _G["CUSTOM_UI_MountSelectBar"..i]
			...
			if(button.favorite) then
				...
				button.favorite:SetTexture(UI.Media.icon.star)
			end
		end
	end
	...
	UI.API:Set("ScrollBar", PetJournalListScrollFrameScrollBar)
	...
	for i = 1, #PetJournal.listScroll.buttons do 
		...
		if(favorite) then
			...
			favorite:SetTexture(UI.Media.icon.star)
	...
	UI.API:Set("!_ItemButton", PetJournalHealPetButton)
	...
	for b = 1, 3 do
		...
		pjPet.pushed = true;
		pjPet.checked = true;
		UI.API:Set("ItemButton", pjPet, true)
		...
		_G['PetJournalLoadoutPet'..b..'HealthFrame'].healthBar:SetStatusBarTexture(UI.Media.statusbar.default)
		_G['PetJournalLoadoutPet'..b..'XPBar']:RemoveTextures()
		_G['PetJournalLoadoutPet'..b..'XPBar']:SetStyle("Frame", 'Default')
		_G['PetJournalLoadoutPet'..b..'XPBar']:SetStatusBarTexture(UI.Media.statusbar.default)
	...
	UI.API:Set("ItemButton", PetJournalPetCard, true)
	PetJournalPetCardInset:RemoveTextures()
	PetJournalPetCardPetInfo.levelBG:SetAlpha(0)
	PetJournalPetCardPetInfoIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	UI.API:Set("ItemButton", PetJournalPetCardPetInfo, true)
	...
	for b = 1, 6 do 
		...
		S.icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	PetJournalPetCardHealthFrame.healthBar:SetStatusBarTexture(UI.Media.statusbar.default)
	PetJournalPetCardXPBar:RemoveTextures()
	PetJournalPetCardXPBar:SetStyle("Frame", 'Default')
	PetJournalPetCardXPBar:SetStatusBarTexture(UI.Media.statusbar.default)

	UI.API:Set("Tab", CollectionsJournalTab3)
	UI.API:Set("Tab", CollectionsJournalTab4)
	...
	UI.API:Set("PageButton", ToyBox.navigationFrame.prevPageButton, false, true)
	UI.API:Set("PageButton", ToyBox.navigationFrame.nextPageButton)
	...
	UI.API:Set("PageButton", HeirloomsJournal.navigationFrame.prevPageButton, false, true)
	UI.API:Set("PageButton", HeirloomsJournal.navigationFrame.nextPageButton)

	UI.API:Set("DropDown", HeirloomsJournalClassDropDown)
	...
	MountJournal.SummonRandomFavoriteButton.texture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
