...
local _hook_AchievementsUpdate = function()
	for i = 1, ACHIEVEMENTUI_MAX_SUMMARY_ACHIEVEMENTS do
		...
		if(summary) then
			...
			if(icontex) then
				icontex:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
				icontex:InsetPoints()
			end
...
local function AchievementStyle()
	if UI.db.Skins.blizzard.enable  ~= true or UI.db.Skins.blizzard.achievement  ~= true then
		return
	end
	...
	UI.API:Set("Window", AchievementFrame, false, false, 1, 4, 8)
	...
	UI.API:Set("CloseButton", AchievementFrameCloseButton, AchievementFrame.Panel)
	UI.API:Set("DropDown", AchievementFrameFilterDropDown)
	AchievementFrameFilterDropDown:SetPoint("TOPRIGHT", AchievementFrame, "TOPRIGHT", -44, 5)

	UI.API:Set("ScrollBar", AchievementFrameCategoriesContainerScrollBar, 5)
	UI.API:Set("ScrollBar", AchievementFrameAchievementsContainerScrollBar, 5)
	UI.API:Set("ScrollBar", AchievementFrameStatsContainerScrollBar, 5)
	UI.API:Set("ScrollBar", AchievementFrameComparisonContainerScrollBar, 5)
	UI.API:Set("ScrollBar", AchievementFrameComparisonStatsContainerScrollBar, 5)
	...
	AchievementFrame:HookScript("OnShow", function(self)
		...
		for i = 1, 20 do
			UI.API:Set("!_ItemButton", _G["AchievementFrameCategoriesContainerButton"..i])
		end
	...
	for i = 1, #AchievementItemButtons do
		...
		if(button) then
			...
			button.bg1:SetTexture(UI.Media.background.button)
			button.bg1:SetVertexColor(unpack(UI.Media.color.default))
			...
			button.bg3:SetBackdrop(UI.Media.backdrop.shadowoutline)
			...
			if(icon) then
				...
				if(tex) then
					tex:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
					tex:InsetPoints()
				end
	...
	for c, v in pairs(u) do
		for f = 1, 9 do
			...
			_G[d].bg1:SetTexture(UI.Media.background.transparent)
			_G[d].bg1:SetVertexColor(unpack(UI.Media.color.default))
			...
			_G[d.."IconTexture"]:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	hooksecurefunc("AchievementButton_GetProgressBar", function(y)
		...
		if d then
			if not d.styled then
				d:RemoveTextures()
				d:SetStatusBarTexture(UI.Media.statusbar.default)
				...
				d.bg1:SetTexture(UI.Media.background.transparent)
				d.bg1:SetVertexColor(unpack(UI.Media.color.default))
				d.bg1:SetAllPoints()
