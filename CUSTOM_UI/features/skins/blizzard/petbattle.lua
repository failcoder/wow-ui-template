...
local PetBattleActionBar = CreateFrame("Frame", "CUSTOM_UI_PetBattleActionBar", UIParent)
...
local _hook_AuraHolderUpdate = function(self)
    ...
    for i=1, C_PetBattles.GetNumAuras(self.petOwner, self.petIndex) do
        ...
        if ( (isBuff and self.displayBuffs) or (not isBuff and self.displayDebuffs) ) then
			...
			frame.Duration:SetFont(UI.Media.font.number, 16, "OUTLINE")
...
local _hook_WeatherFrameUpdate = function(self)
	...
	if auraID then
		...
		self:ClearAllPoints()
		self:SetPoint("TOP", UI.Screen, 0, -15)
	end 
end

local _hook_UpdateDisplay = function(self)
	self.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
...
local _hook_AbilityTooltipShow = function()
	UI:AnchorToCursor(PetBattlePrimaryAbilityTooltip)
end
...
local function PetBattleStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.petbattleui ~= true then
		return 
	end
	...
	UI.API:Set("CloseButton", FloatingBattlePetTooltip.CloseButton)
	...
	for i, frame in pairs(ActiveFramesList) do 
		if(not frame.isStyled) then
			...
			frame.ActualHealthBar:SetTexture(UI.Media.statusbar.default)
			...
			frame.PetTypeFrame.text:SetFont(UI.Media.font.default, 12, "OUTLINE")
	...
	local CUSTOM_UI_DockBottomCenter = _G.CUSTOM_UI_DockBottomCenter;
	if(CUSTOM_UI_DockBottomCenter) then
		PetBattleActionBar:SetPoint("BOTTOM", CUSTOM_UI_DockBottomCenter, "TOP", 0, 4)
	else
		PetBattleActionBar:SetPoint("BOTTOM", UI.Screen, "BOTTOM", 0, 4)
	end
	...
	BottomFrame.TurnTimer:SetPoint("TOP", UI.Screen, "TOP", 0, -140)
	...
	BottomFrame.xpBar:SetStatusBarTexture(UI.Media.statusbar.default)
	...
	BottomFrame.xpBar:SetScript("OnShow", function(self)
		self:RemoveTextures()
		self:SetStatusBarTexture(UI.Media.statusbar.default)
	end)
