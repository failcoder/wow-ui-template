...
local LootHistoryFrame_OnUpdate = function(self)
	...
	for i = 1, numItems do
		...
		if not frame.isStyled then
			...
			frame.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
...
local _hook_LossOfControl = function(self, ...)
	...
	self.AbilityName:SetFont(UI.Media.font.dialog, 20, 'OUTLINE')
	...
	self.TimeLeft.NumberText:SetFont(UI.Media.font.number, 20, 'OUTLINE')
	...
	self.TimeLeft.SecondsText:SetFont(UI.Media.font.default, 20, 'OUTLINE')
...
local function MiscStyles()
	...
	if(UI.db.Skins.blizzard.dressingroom) then
		DressUpFrame:SetSize(500, 600)
		UI.API:Set("Window", DressUpFrame, true, true)
		...
		UI.API:Set("CloseButton", DressUpFrameCloseButton, DressUpFrame.Panel)
	end

	if(UI.db.Skins.blizzard.gossip) then
		UI.API:Set("Window", GossipFrame, true, true)

		ItemTextFrame:RemoveTextures(true)
		ItemTextScrollFrame:RemoveTextures()
		UI.API:Set("CloseButton", GossipFrameCloseButton)
		UI.API:Set("PageButton", ItemTextPrevPageButton, false, true)
		UI.API:Set("PageButton", ItemTextNextPageButton)
		...
		UI.API:Set("ScrollBar", ItemTextScrollFrameScrollBar)
		UI.API:Set("CloseButton", ItemTextFrameCloseButton)
		local r = {"GossipFrameGreetingPanel", "GossipFrameInset", "GossipGreetingScrollFrame"}
		UI.API:Set("ScrollBar", GossipGreetingScrollFrame)
		...
		UI.API:Set("CloseButton", GossipFrameCloseButton, GossipFrame.Panel)

		NPCFriendshipStatusBar:RemoveTextures()
		NPCFriendshipStatusBar:SetStatusBarTexture(UI.Media.statusbar.default)
		...
		UI.NPC:Register(GossipFrame, GossipFrameNpcNameText)
		hooksecurefunc("GossipTitleButton_OnClick", function() UI.NPC:PlayerTalksFirst() end)
	end

	if (UI.db.Skins.blizzard.greeting) then
		_G.QuestFrameGreetingPanel:HookScript("OnShow", _hook_GreetingPanelShow)
	end

	if(UI.db.Skins.blizzard.guildregistrar) then
		UI.API:Set("Window", GuildRegistrarFrame, true, true)
		...
		UI.API:Set("CloseButton", GuildRegistrarFrameCloseButton)
	...
	if(UI.db.Skins.blizzard.loot) then
		local MasterLooterFrame = _G.MasterLooterFrame;
		...
		UI.API:Set("CloseButton", MissingLootFramePassButton)
		hooksecurefunc("MissingLootFrame_Show", MissingLootFrame_OnShow)
		LootHistoryFrame:RemoveTextures()
		UI.API:Set("CloseButton", LootHistoryFrame.CloseButton)
		LootHistoryFrame:RemoveTextures()
		LootHistoryFrame:SetStyle("!_Frame", 'Transparent')
		UI.API:Set("CloseButton", LootHistoryFrame.ResizeButton)
		...
		txt:SetFont(UI.Media.font.default, 14, "NONE")
		txt:SetAllPoints(LootHistoryFrame.ResizeButton)
		txt:SetJustifyH("CENTER")
		txt:SetText("RESIZE")

		LootHistoryFrameScrollFrame:RemoveTextures()
		UI.API:Set("ScrollBar", LootHistoryFrameScrollFrameScrollBar)
		...
		UI.API:Set("Alert", BonusRollFrame)
		BonusRollFrame.PromptFrame.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
		BonusRollFrame.PromptFrame.Timer.Bar:SetTexture(UI.Media.statusbar.default)
		BonusRollFrame.PromptFrame.Timer.Bar:SetVertexColor(0.1, 1, 0.1)
	end

	if(UI.db.Skins.blizzard.losscontrol) then
	...
	if(UI.db.Skins.blizzard.mail) then
		...
		UI.API:Set("Window", MailFrame)
		UI.API:Set("CloseButton", MailFrameCloseButton)
		UI.API:Set("PageButton", InboxPrevPageButton, false, true)
		UI.API:Set("PageButton", InboxNextPageButton)

		UI.API:Set("Tab", MailFrameTab1)
		UI.API:Set("Tab", MailFrameTab2)

		SendMailScrollFrame:RemoveTextures(true)
		SendMailScrollFrame:SetStyle("!_Frame", "Inset")
		UI.API:Set("ScrollBar", SendMailScrollFrameScrollBar)
		...
		UI.API:Set("CloseButton", OpenMailFrameCloseButton)

		OpenMailScrollFrame:RemoveTextures(true)
		OpenMailScrollFrame:SetStyle("!_Frame", "Default")

		UI.API:Set("ScrollBar", OpenMailScrollFrameScrollBar)
		...
		OpenMailLetterButtonIconTexture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
		OpenMailLetterButtonIconTexture:InsetPoints()

		OpenMailMoneyButton:RemoveTextures()
		OpenMailMoneyButton:SetStyle("Button")
		OpenMailMoneyButtonIconTexture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	if(UI.db.Skins.blizzard.merchant) then

		UI.API:Set("Window", MerchantFrame, false, true)
		...
		UI.API:Set("DropDown", MerchantFrameLootFilter)
		UI.API:Set("Tab", _G["MerchantFrameTab1"])
		UI.API:Set("Tab", _G["MerchantFrameTab2"])
		...
		MerchantBuyBackItemItemButton:RemoveTextures()
		MerchantBuyBackItemItemButton:SetStyle("Button")
		MerchantBuyBackItemItemButtonIconTexture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
		...
		UI.API:Set("CloseButton", MerchantFrameCloseButton, MerchantFrame.Panel)
		UI.API:Set("!_PageButton", MerchantNextPageButton)
		UI.API:Set("!_PageButton", MerchantPrevPageButton, false, true)

		UI.NPC:Register(MerchantFrame, MerchantNameText)
	end

	if(UI.db.Skins.blizzard.petition) then
		UI.API:Set("Window", PetitionFrame, nil, true)
		...
		UI.API:Set("CloseButton", PetitionFrameCloseButton)
	...
	if(UI.db.Skins.blizzard.stable) then
		...
		UI.API:Set("CloseButton", PetStableFrameCloseButton)
		PetStablePrevPageButton:SetStyle("Button")
		PetStableNextPageButton:SetStyle("Button")
		UI.API:Set("PageButton", PetStablePrevPageButton)
		UI.API:Set("PageButton", PetStableNextPageButton)
		for i = 1, NUM_PET_ACTIVE_SLOTS do
			 UI.API:Set("!_ItemButton", _G['PetStableActivePet'..i])
		end
		for i = 1, NUM_PET_STABLE_SLOTS do
			 UI.API:Set("!_ItemButton", _G['PetStableStabledPet'..i])
		end
		PetStableSelectedPetIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	end

	if(UI.db.Skins.blizzard.tabard) then
		...
		UI.API:Set("CloseButton", TabardFrameCloseButton)
		...
		for i = 1, 5 do
			...
			if(frame) then
				frame:RemoveTextures()
				UI.API:Set("PageButton", _G[name.."LeftButton"])
				UI.API:Set("PageButton", _G[name.."RightButton"])
	...
	if(UI.db.Skins.blizzard.taxi) then
		UI.API:Set("Window", TaxiFrame)
		UI.API:Set("CloseButton", TaxiFrame.CloseButton)
	end

	if (UI.db.Skins.blizzard.trade) then
		TradeFrameInset:Die()
		...
		UI.API:Set("Window", TradeFrame, true)

		TradeFrameTradeButton:SetStyle("Button")
		TradeFrameCancelButton:SetStyle("Button")
		UI.API:Set("CloseButton", TradeFrameCloseButton, TradeFrame.Panel)
	...
	if(UI.db.Skins.blizzard.bgscore) then
		WorldStateScoreScrollFrame:RemoveTextures()
		WorldStateScoreFrame:RemoveTextures()
		WorldStateScoreFrame:SetStyle("Frame", "Window")
		UI.API:Set("CloseButton", WorldStateScoreFrameCloseButton)
		UI.API:Set("ScrollBar", WorldStateScoreScrollFrameScrollBar)
		WorldStateScoreFrameInset:SetAlpha(0)
		WorldStateScoreFrameLeaveButton:SetStyle("Button")
		UI.API:Set("Tab", _G["WorldStateScoreFrameTab1"])
		UI.API:Set("Tab", _G["WorldStateScoreFrameTab2"])
		UI.API:Set("Tab", _G["WorldStateScoreFrameTab3"])
	end
end
--[[
##########################################################
