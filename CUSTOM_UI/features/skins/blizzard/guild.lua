...
local function GCTabHelper(tab)
	...
	tab.bg1:SetTexture(UI.Media.background.transparent)
	tab.bg1:SetVertexColor(unpack(UI.Media.color.default))
...
local _hook_UIRankOrder = function(self)
	UI.Timers:ExecuteTimer(1, _hook_RankOrder_OnUpdate)
end
...
local _hook_BankTabPermissions = function(self)
	...
	for i = 1, tabs do
		...
		if(tab) then
			if(tab.tabIcon) then tab.tabIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS)) end
...
local function GuildBankStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.gbank ~= true then
		return
	end

	UI.API:Set("Window", GuildBankFrame)

	GuildBankEmblemFrame:RemoveTextures(true)
	GuildBankMoneyFrameBackground:Die()
	UI.API:Set("ScrollBar", GuildBankPopupScrollFrameScrollBar)
	...
	for i = 1, 8 do
		...
		if(tab) then
			...
			if(button) then
				...
				if(texture) then
					texture:InsetPoints()
					texture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
				end
			end
		end
	end
	...
	UI.API:Set("EditBox", GuildItemSearchBox)
	...
	UI.API:Set("ScrollBar", GuildBankTransactionsScrollFrameScrollBar)
	UI.API:Set("ScrollBar", GuildBankInfoScrollFrameScrollBar)
end

local function GuildFrameStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.guild ~= true then
		return
	end

	UI.API:Set("Window", GuildFrame)

	UI.API:Set("CloseButton", GuildMemberDetailCloseButton)
	UI.API:Set("CloseButton", GuildFrameCloseButton)
	...
	GuildFactionBar.progress:SetTexture(UI.Media.statusbar.default)
	...
	UI.API:Set("ScrollBar", GuildRosterContainerScrollBar, 4, -4)
	...
	UI.API:Set("DropDown", GuildRosterViewDropdown, 200)
	...
	UI.API:Set("DropDown", GuildMemberRankDropdown, 182)
	...
	UI.API:Set("CloseButton", GuildNewsFiltersFrameCloseButton)
	...
	UI.API:Set("ScrollBar", GuildNewsContainerScrollBar, 4, 4)
	UI.API:Set("ScrollBar", GuildInfoDetailsFrameScrollBar, 4, 4)
	...
	UI.API:Set("ScrollBar", GuildTextEditScrollFrameScrollBar, 4, 4)
	...
	UI.API:Set("ScrollBar", GuildLogScrollFrameScrollBar, 4, 4)
	...
	UI.API:Set("ScrollBar", GuildRewardsContainerScrollBar, 4, -4)
	UI.API:Set("ScrollBar", GuildPerksContainerScrollBar, 4, 2)

	for i = 1, 8 do
		local button = _G["GuildPerksContainerButton"..i]
		if button then
			button:RemoveTextures()
			UI.API:Set("ItemButton", button, nil, true)
...
local function GuildControlStyle()
	if UI.db.Skins.blizzard.enable~=true or UI.db.Skins.blizzard.guildcontrol~=true then return end
	...
	UI.API:Set("Window", GuildControlUI)

	UI.API:Set("ScrollBar", GuildControlUIRankBankFrameInsetScrollFrameScrollBar)
	...
	UI.API:Set("DropDown", GuildControlUINavigationDropDown)
	UI.API:Set("DropDown", GuildControlUIRankSettingsFrameRankDropDown,180)
	...
	UI.API:Set("DropDown", GuildControlUIRankBankFrameRankDropDown, 180)

	GuildControlUIRankBankFrameRankDropDownButton:SetWidth(20)
end

local function LFGuildFrameStyle()
	if(UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.lfguild ~= true) then return end

	UI.API:Set("Window", LookingForGuildFrame, true)
	...
	UI.API:Set("ScrollBar", LookingForGuildBrowseFrameContainerScrollBar)
	LookingForGuildBrowseButton:SetStyle("Button")
	LookingForGuildRequestButton:SetStyle("Button")

	UI.API:Set("CloseButton", LookingForGuildFrameCloseButton)
