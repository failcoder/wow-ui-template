...
local function AuctionStyle()
	--MOD.Debugging = true
	if(UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.auctionhouse ~= true) then return end

	UI.API:Set("Window", AuctionFrame, false, true, 1, 2, -8)
	...
	UI.API:Set("CloseButton", AuctionFrameCloseButton)
	UI.API:Set("ScrollBar", AuctionsScrollFrameScrollBar)

	UI.API:Set("DropDown", BrowseDropDown)
	UI.API:Set("DropDown", PriceDropDown)
	UI.API:Set("DropDown", DurationDropDown)
	UI.API:Set("ScrollBar", BrowseFilterScrollFrameScrollBar)
	UI.API:Set("ScrollBar", BrowseScrollFrameScrollBar)
	...
	AuctionProgressBar:SetStatusBarTexture(UI.Media.statusbar.default)
	AuctionProgressBar:SetStatusBarColor(1, 1, 0)

	UI.API:Set("PageButton", BrowseNextPageButton)
	UI.API:Set("PageButton", BrowsePrevPageButton)
	...
	AuctionsItemButton:SetScript("OnUpdate", function()
		if AuctionsItemButton:GetNormalTexture()then
			AuctionsItemButton:GetNormalTexture():SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	UI.API:Set("Tab", _G["AuctionFrameTab1"])
	UI.API:Set("Tab", _G["AuctionFrameTab2"])
	UI.API:Set("Tab", _G["AuctionFrameTab3"])
		...
		if(button and (not button.Panel)) then
			...
			if(buttonItem) then
				...
				if(buttonTex) then
					...
					buttonTex:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
