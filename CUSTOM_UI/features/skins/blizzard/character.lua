...
local function SetItemFrame(frame, point)
	...
	point.backdrop:SetTexture(UI.Media.statusbar.default)
	point.backdrop:SetVertexColor(unpack(UI.Media.color.default))
...
local function PaperDollTitlesPane_OnShow()
	for i,btn in pairs(PaperDollTitlesPane.buttons) do
		if(btn) then
			...
			UI:FontManager(btn.text, "default")
		end
	end
...
local function PaperDollEquipmentManagerPane_OnShow()
	for i,btn in pairs(PaperDollEquipmentManagerPane.buttons) do
		if(btn) then
			...
			btn.icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
...
local function CharacterFrameStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.character ~= true then
		 return
	end

	UI.API:Set("Window", CharacterFrame, true, true, 1, 3, 3)

	UI.API:Set("CloseButton", CharacterFrameCloseButton)
	UI.API:Set("ScrollBar", CharacterStatsPane)
	UI.API:Set("ScrollBar", ReputationListScrollFrame)
	UI.API:Set("ScrollBar", TokenFrameContainer)
	UI.API:Set("ScrollBar", GearManagerDialogPopupScrollFrame)
	...
	UI.API:Set("!_PageButton", CharacterFrameExpandButton)
	...
	UI.API:Set("CloseButton", ReputationDetailCloseButton)
	UI.API:Set("CloseButton", TokenFramePopupCloseButton)
	...
	UI.API:Set("ScrollBar", _G["PaperDollTitlesPaneScrollBar"], 5)
	UI.API:Set("ScrollBar", _G["PaperDollEquipmentManagerPaneScrollBar"], 5)
	...
	for i = 1, 4 do
		 UI.API:Set("Tab", _G["CharacterFrameTab"..i])
	end
		...
		for i = 1, GetCurrencyListSize() do
			...
			if(currency) then
				...
				if currency.icon then
					 currency.icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
				end
			end
		end
