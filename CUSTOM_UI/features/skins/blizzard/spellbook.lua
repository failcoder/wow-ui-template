...
local function ChangeTabHelper(tab)
	...
	tab.backdrop:SetBackdrop(UI.Media.backdrop.glow);
...
local function AbilityButtonHelper(index)
	...
	if(button and (not button.Panel)) then
		...
		if(button.iconTexture) then
			button.iconTexture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
...
local function SpellBookStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.spellbook ~= true then return end

	UI:FontManager(_G["SubSpellFont"], "caps", "SYSTEM", 1, "OUTLINE", 0.8, 0.8, 0.8);
	UI:FontManager(_G["CoreAbilityFont"], "zone", "SYSTEM", 10, "OUTLINE", 0, 0, 0);

	UI.API:Set("Window", SpellBookFrame, false, false, 1, 3, 3)
	UI.API:Set("CloseButton", SpellBookFrameCloseButton)
	...
	UI.API:Set("PageButton", SpellBookPrevPageButton)
	UI.API:Set("PageButton", SpellBookNextPageButton)
