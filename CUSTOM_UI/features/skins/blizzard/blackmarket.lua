	...
	for i = 1, #buttons do 
		...
		if(button) then
			...
			if(not button.Panel) then 
				...
				UI.API:Set("ItemButton", button.Item)
			end 
...
local function BlackMarketStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.bmah ~= true then 
		return 
	end 

	UI.API:Set("Window", BlackMarketFrame)

	BlackMarketFrame.Inset:RemoveTextures()
	BlackMarketFrame.Inset:SetStyle("!_Frame", "Inset")

	UI.API:Set("CloseButton", BlackMarketFrame.CloseButton)
	UI.API:Set("ScrollBar", BlackMarketScrollFrameScrollBar, 4)
	...
	UI.API:Set("ItemButton", BlackMarketFrame.HotDeal.Item)
end 
--[[ 
########################################################## 
MOD LOADING
