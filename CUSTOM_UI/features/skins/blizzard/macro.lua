...
local function MacroUIStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.macro ~= true then return end

	UI.API:Set("Window", MacroFrame, true)

	UI.API:Set("CloseButton", MacroFrameCloseButton)
	UI.API:Set("ScrollBar", MacroButtonScrollFrameScrollBar)
	UI.API:Set("ScrollBar", MacroFrameScrollFrameScrollBar)
	UI.API:Set("ScrollBar", MacroPopupScrollFrameScrollBar)
	...
	MacroFrameText:SetFont(UI.Media.font.default, 10, "OUTLINE")
	...
	UI.API:Set("ScrollBar", MacroButtonScrollFrame)
	...
	MacroFrameSelectedMacroButtonIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
		...
		if(button) then
			...
			if(popup) then
				...
				if(popupIcon) then
					...
					popupIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
				end
			end 
		end  
	end 
