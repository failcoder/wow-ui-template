...
local function ChannelList_OnUpdate()
	for i = 1, MAX_DISPLAY_CHANNEL_BUTTONS do 
		...
		if btn then
			...
			_G["ChannelButton"..i.."Text"]:SetFontObject(CUSTOM_UI_Font_Default)
		end 
	end 
end 
--[[ 
...
local function FriendsFrameStyle()
	if UI.db.Skins.blizzard.enable ~= true or UI.db.Skins.blizzard.friends ~= true then
		 return 
	end

	UI.API:Set("Window", FriendsFrame)

	FriendsFrameInset:RemoveTextures()
	WhoFrameListInset:RemoveTextures()
	WhoFrameEditBoxInset:RemoveTextures()
	UI.API:Set("EditBox", WhoFrameEditBoxInset)
	...
	UI.API:Set("ScrollBar", FriendsFrameFriendsScrollFrameScrollBar, 5)
	UI.API:Set("ScrollBar", WhoListScrollFrameScrollBar, 5)
	UI.API:Set("ScrollBar", ChannelRosterScrollFrameScrollBar, 5)
	
	FriendsFrameStatusDropDown:SetPoint('TOPLEFT', FriendsTabHeader, 'TOPLEFT', 0, -27)
	UI.API:Set("DropDown", FriendsFrameStatusDropDown, 70)
	...
	FriendsFrameBattlenetFrame.BroadcastButton:SetScript('OnClick', function()
		UI:StaticPopup_Show("SET_BN_BROADCAST")
	end)
	FriendsFrameBattlenetFrame.Tag:SetFontObject(CUSTOM_UI_Font_Narrator)
	...
	UI.API:Set("ScrollBar", ScrollOfResurrectionSelectionFrameListScrollFrameScrollBar, 4)
	...
	UI.API:Set("CloseButton", ChannelFrameDaughterFrameDetailCloseButton, ChannelFrameDaughterFrame)
	UI.API:Set("CloseButton", FriendsFrameCloseButton, FriendsFrame.Panel)
	UI.API:Set("DropDown", WhoFrameDropDown, 150)

	for i = 1, 4 do
		 UI.API:Set("Tab", _G["FriendsFrameTab"..i])
	end 
	...
	UI.API:Set("DropDown", FriendsFriendsFrameDropDown, 150)
	...
	FriendsTabHeaderSoRButtonIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	FriendsTabHeaderRecruitAFriendButtonIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	FriendsTabHeaderRecruitAFriendButtonIcon:InsetPoints()
	
	FriendsFrameIgnoreScrollFrame:SetStyle("!_Frame", "Model")
	UI.API:Set("ScrollBar", FriendsFrameIgnoreScrollFrameScrollBar, 4)
	FriendsFramePendingScrollFrame:SetStyle("!_Frame", "Inset")
	UI.API:Set("ScrollBar", FriendsFramePendingScrollFrameScrollBar, 4)
