...
local function ListSkinnedAddons(msg, prefix)
	for style,_ in pairs(MOD.SkinnedAddons) do
		...
		UI:AddonMessage(styleMessage:format(style, verb))
	end
end
...
function MOD:Style(style, fn, ...)
	...
	if(catch and self.Debugging) then
		UI:HandleError("SKINS", style, catch);
		return
	end
...
function MOD:IsAddonReady(addon, ...)
	if not UI.db.Skins.addons then return end
	for i = 1, select('#', ...) do
		local a = select(i, ...)
	...
	return UI.db.Skins.addons[addon]
end
...
function MOD:SaveAddonStyle(addon, fn, force, passive, ...)
	...
	if(UI.defaults.Skins.addons and UI.defaults.Skins.addons[addon] == nil) then
		UI.defaults.Skins.addons[addon] = true
	end
...
function MOD:PLAYER_ENTERING_WORLD(event, ...)
	...
	for addonName,fn in pairs(self.AddOnQueue)do
		if(UI.db.Skins.addons[addonName] == nil) then
			UI.db.Skins.addons[addonName] = true
		end
	...
	UI.Events:Trigger("REQUEST_TEMPLATE_UPDATED");
end
...
function MOD:ADDON_LOADED(event, addon)
	...
	-- if(addon == 'CUSTOM_UI_!Options') then
	-- 	self:Style(addon, MOD.StyleCUSTOM_UIOptions)
	-- end
...
function MOD:Load()
	UI.private.Docks = UI.private.Docks or {"None", "None"}
	...
	alert.Text:SetFont(UI.Media.font.dialog, 12);
	...
	alert.Accept.Text:SetFont(UI.Media.font.dialog, 10);
	...
	alert.Close.Text:SetFont(UI.Media.font.dialog, 10);
	...
	self.Docklet = UI.Dock:NewDocklet("BottomRight", "CUSTOM_UI_SkinsDock", "Addon Docklet", [[Interface\AddOns\CUSTOM_UI_Skins\artwork\DOCK-ICON-ADDON]]);
	--self.Docklet:SetVisibilityCallbacks(false, false);
	self.Docklet:SetClickCallbacks(AddonDockletToggle, false, self.GetDockables);

	--UI.Dock.BottomRight.Bar.Button.GetDockOptions = self.GetDockables;

	local dockWidth = self.Docklet:GetWidth()

	self.Docklet.Dock1 = CreateFrame("Frame", "CUSTOM_UI_SkinsDockAddon1", self.Docklet);
	...
	self.Docklet.Dock2 = CreateFrame("Frame", "CUSTOM_UI_SkinsDockAddon2", self.Docklet);
	...
	UI:AddSlashCommand("skinned", "List all addons skinned by CUSTOM_UI", ListSkinnedAddons);

	UI.Events:On("DOCK_FADE_IN", DockFadeInDocklet, true);
	UI.Events:On("DOCK_FADE_OUT", DockFadeOutDocklet, true);
	UI.Events:On("DOCK_EXPANDED", DockExpandDocklet, true);
end
