...
BodyGuard.UF = CreateFrame("Button", "CUSTOM_UI_BodyGuard", UIParent, "SecureActionButtonTemplate")
...
function BodyGuard:UpdateSettings()
  ...
  if(UI.db.UnitFrames.bodyguard.enable) then
    self.UF:SetParent(UI.Screen)
  else
    self.UF:SetParent(UI.Hidden)
  end

  self:HealthUpdate()
  self.UF:SetWidth(UI.db.UnitFrames.bodyguard.width)
  self.UF:SetHeight(UI.db.UnitFrames.bodyguard.height)
end
...
function BodyGuard:HealthUpdate()
  ...
  local r, g, b = unpack(oUF_CUSTOM_UI.colors.health)
  r, g, b = oUF_CUSTOM_UI.ColorGradient(health, maxHealth, 1, 0, 0, 1, 1, 0, r, g, b)
...
local EventListener_OnEvent = function(self, event, ...)
    ...
    if(fn and type(fn) == "function") then
        ...
        if(catch) then
            UI:HandleError("UnitFrames [BodyGuard]", event, catch)
        end
    end
end
...
function MOD:InitializeBodyGuard()
  ...
  BodyGuard.UF:SetPoint("BOTTOMRIGHT", UI.Dock.BottomLeft, "TOPRIGHT", 0, 10)
  BodyGuard.UF:SetWidth(UI.db.UnitFrames.bodyguard.width)
  BodyGuard.UF:SetHeight(UI.db.UnitFrames.bodyguard.height)
  ...
  BodyGuard.UF.Health:SetStatusBarTexture(LSM:Fetch("statusbar", UI.db.UnitFrames.statusbar))

  BodyGuard.UF.Health.bg = BodyGuard.UF.Health:CreateTexture(nil, "BORDER")
  BodyGuard.UF.Health.bg:SetAllPoints()
  BodyGuard.UF.Health.bg:SetTexture(UI.Media.statusbar.gradient)
  BodyGuard.UF.Health.bg:SetVertexColor(0.1, 0.1, 0.1)

  BodyGuard.UF.Name = BodyGuard.UF.Health:CreateFontString(nil, 'OVERLAY')
  UI:FontManager(BodyGuard.UF.Name, "unitsecondary")
  BodyGuard.UF.Name:SetPoint("CENTER", BodyGuard.UF, "CENTER")
  BodyGuard.UF.Name:SetTextColor(unpack(oUF_CUSTOM_UI.colors.reaction[5]))

  UI:NewAnchor(BodyGuard.UF, L["BodyGuard Frame"])
