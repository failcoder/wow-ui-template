...
local groupLayoutPostSizeFunc = function(self, width, height)
    if(not UI.db.UnitFrames[self.___key]) then return end;
...
local Update5ManVisibility = function(token)
    ...
    if(UI.db.UnitFrames.party.useFor5man) then
    ...
    UI.db.UnitFrames.party.visibility = VISIBILITY_OPTIONS.party
    UI.db.UnitFrames.raid.visibility = VISIBILITY_OPTIONS.raid
...
local PartyUnitUpdate = function(self)
    local db = UI.db.UnitFrames.party
    ...
    if self.isChild then
      ...
      if not InCombatLockdown()then
        if altDB.enable then
            ...
            self:ClearAllPoints()
            UI:SetReversePoint(self, altDB.anchorPoint, self.originalParent, altDB.xOffset, altDB.yOffset)
        else
            self:SetParent(UI.Hidden)
        end
      end
      do
...
UpdateTemplates["party"] = function(self)
    if(UI.NeedsFrameAudit) then return end
    ...
    if not groupFrame.positioned then
        ...
        groupFrame:SetPoint("BOTTOMLEFT", UI.Dock.BottomLeft, "TOPLEFT", 0, 80)
        UI:NewAnchor(groupFrame, L['Party Frames']);
        UI:SetAnchorResizing(groupFrame, groupLayoutPostSizeFunc, 10, 500)
...
local RaidUnitUpdate = function(self)
    ...
    local db = UI.db.UnitFrames[token]
    self.colors = oUF_CUSTOM_UI.colors;
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")
...
UpdateTemplates["raid"] = function(self)
    ...
    local db = UI.db.UnitFrames.raid
    local groupFrame = self:GetParent()
    if not self.isForced then
        RegisterStateDriver(groupFrame, "visibility", visibility)
    end
    ...
    if not groupFrame.positioned then
        groupFrame:ClearAllPoints()
        groupFrame:SetPoint("BOTTOMLEFT", UI.Dock.BottomLeft, "TOPLEFT", 0, 80)
        UI:NewAnchor(groupFrame, "Raid Frames")
        UI:SetAnchorResizing(groupFrame, groupLayoutPostSizeFunc, 10, 500)
        groupFrame.positioned = true
    end
...
UpdateTemplates["raidpet"] = function(self)
    if(UI.NeedsFrameAudit) then return end
    local db = UI.db.UnitFrames.raidpet
    local groupFrame = self:GetParent()

    if not groupFrame.positioned then
        groupFrame:ClearAllPoints()
        groupFrame:SetPoint("BOTTOMLEFT", UI.Screen, "BOTTOMLEFT", 4, 433)
        RegisterStateDriver(groupFrame, "visibility", "[group:raid] show;hide")
        UI:NewAnchor(groupFrame, L["Raid Pet Frames"])
        UI:SetAnchorResizing(groupFrame, groupLayoutPostSizeFunc, 10, 500)
        groupFrame.positioned = true;
    end
...
local TankUnitUpdate = function(self)
    local db = UI.db.UnitFrames.tank
    self.colors = oUF_CUSTOM_UI.colors;
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")
    ...
    if self.isChild and self.originalParent then
        ...
        if not InCombatLockdown()then
            if targets.enable then
                ...
                self:SetSize(UNIT_WIDTH, UNIT_HEIGHT)
                self:ClearAllPoints()
                UI:SetReversePoint(self, targets.anchorPoint, self.originalParent, targets.xOffset, targets.yOffset)
            else
                self:SetParent(UI.Hidden)
            end
        end
    ...
    do
        local nametext = self.TextGrip.Name;
        if oUF_CUSTOM_UI.colors.healthclass then
...
UpdateTemplates["tank"] = function(self)
    ...
    if not self.positioned then
        self:ClearAllPoints()
        self:SetPoint("BOTTOMLEFT", UI.Dock.TopLeft, "BOTTOMLEFT", 0, 0)
        UI:NewAnchor(self, L["Tank Frames"])
        UI:SetAnchorResizing(self, groupLayoutPostSizeFunc, 10, 500)
...
local AssistUnitUpdate = function(self)
    local db = UI.db.UnitFrames.assist
    self.colors = oUF_CUSTOM_UI.colors;
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")
...
UpdateTemplates["assist"] = function(self)
    ...
    if not self.positioned then
        self:ClearAllPoints()
        self:SetPoint("TOPLEFT", UI.Dock.TopLeft, "BOTTOMLEFT", 0, -10)
        UI:NewAnchor(self, L["Assist Frames"])
        UI:SetAnchorResizing(self, groupLayoutPostSizeFunc, 10, 500)
        self.Grip.positionOverride = "TOPLEFT"
        self:SetAttribute("minHeight", self.dirtyHeight)
        self:SetAttribute("minWidth", self.dirtyWidth)
...
local HeaderDisableChildren = function(self)
    ...
    for i=1, select("#", self:GetChildren()) do
        local childFrame = select(i, self:GetChildren())
        if(childFrame and childFrame.RegisterForClicks) then
            childFrame:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and 'AnyDown' or 'AnyUp')
...
function MOD:SetGroupHeader(parentFrame, filter, layout, headerName, token, groupTag)
    ...
    if(token == "raidpet") then
        template1 = "CUSTOM_UI_UNITPET"
        template2 = "SecureGroupPetHeaderTemplate"
    elseif(token == "party") then
        template1 = "CUSTOM_UI_UNITPET, CUSTOM_UI_UNITTARGET"
    elseif(token == "tank") then
        filter = "MAINTANK"
        template1 = "CUSTOM_UI_UNITTARGET"
    ...
    local groupHeader = oUF_CUSTOM_UI:SpawnHeader(headerName, template2, nil,
...
local GroupUpdate = function(self)
    local token = self.___groupkey
    if UI.db.UnitFrames[token].enable ~= true then
        UnregisterAttributeDriver(self, "state-visibility")
        self:Hide()
        return
    end
...
local GroupConfigure = function(self)
    ...
    local settings = UI.db.UnitFrames[token]
...
function MOD:GetGroupFrame(token, layout)
    if(not self.Headers[token]) then
        oUF_CUSTOM_UI:RegisterStyle(layout, BuildTemplates[token])
        oUF_CUSTOM_UI:SetActiveStyle(layout)
        local groupFrame = CreateFrame("Frame", layout, _G.CUSTOM_UI_UnitFrameParent, "SecureHandlerStateTemplate")
...
function MOD:SetCustomFrame(token, layout)
    if(not self.Headers[token]) then
        oUF_CUSTOM_UI:RegisterStyle(layout, BuildTemplates[token])
        oUF_CUSTOM_UI:SetActiveStyle(layout)
        local groupFrame = self:SetGroupHeader(_G.CUSTOM_UI_UnitFrameParent, nil, layout, layout, token)
        self.Headers[token] = groupFrame
    end
...
function MOD:SetGroupFrame(token, forceUpdate)
    ...
    local layout = "CUSTOM_UI_"..realName
