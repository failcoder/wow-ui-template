...
local unitLayoutPostSizeFunc = function(self, width, height)
  UI.db.UnitFrames[self.___key].width = width;
  UI.db.UnitFrames[self.___key].height = height;
  self:Update()
end
...
local UpdatePlayerFrame = function(self)
    ...
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")

    MOD.RefreshUnitMedia(self, "player")

    self.colors = oUF_CUSTOM_UI.colors;
    ...
    do
        ...
        if resting then
            if iconDB and iconDB.restIcon and iconDB.restIcon.enable then
                ...
                UI:SetReversePoint(resting, iconDB.restIcon.attachTo, self, iconDB.restIcon.xOffset, iconDB.restIcon.yOffset)
    ...
    do
        ...
        if combat then
            if iconDB and iconDB.combatIcon and iconDB.combatIcon.enable then
                ...
                UI:SetReversePoint(combat, iconDB.combatIcon.attachTo, self, iconDB.combatIcon.xOffset, iconDB.combatIcon.yOffset)
...
CONSTRUCTORS["player"] = function(self, unit)
    ...
    self:SetPoint("BOTTOMRIGHT", UI.Screen, "BOTTOM", -80, 182)
    UI:NewAnchor(self, L["Player Frame"])
    UI:SetAnchorResizing(self, unitLayoutPostSizeFunc, 10, 500)
...
local UpdateTargetFrame = function(self)
    local db = UI.db.UnitFrames["target"]
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height;
...
CONSTRUCTORS["target"] = function(self, unit)
    ...
    local xray = CreateFrame("Button", "CUSTOM_UI_XRayFocus", self, "SecureActionButtonTemplate")
    ...
    xray.icon:SetTexture([[Interface\Addons\CUSTOM_UI\media\textures\Doodads\UNIT-XRAY]])
    ...
    xray:SetScript("OnEnter", function(self)
        ...
        local anchor1, anchor2 = UI:GetScreenXY(self)
    ...
    self:SetPoint("BOTTOMLEFT", UI.Screen, "BOTTOM", 80, 182)
    UI:NewAnchor(self, L["Target Frame"])
    UI:SetAnchorResizing(self, unitLayoutPostSizeFunc, 10, 500)
...
local UpdateTargetTargetFrame = function(self)
    local db = UI.db.UnitFrames["targettarget"]
    local UNIT_WIDTH = db.width
    local UNIT_HEIGHT = db.height
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")
...
CONSTRUCTORS["targettarget"] = function(self, unit)
    ...
    self.RaidIcon = MOD:CreateRaidIcon(self)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("LEFT", CUSTOM_UI_Target, "RIGHT", 4, 0)
    UI:NewAnchor(self, L["TargetTarget Frame"])
    UI:SetAnchorResizing(self, unitLayoutPostSizeFunc, 10, 500)
...
local UpdatePetFrame = function(self)
    ...
    do
        if CUSTOM_UI_Player and not InCombatLockdown()then
            self:SetParent(CUSTOM_UI_Player)
        end
    end
    self:UpdateAllElements()
end
...
CONSTRUCTORS["pet"] = function(self, unit)
    ...
    self.RaidIcon = MOD:CreateRaidIcon(self)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("RIGHT", CUSTOM_UI_Player, "LEFT", -4, 0)
    UI:NewAnchor(self, L["Pet Frame"])
    UI:SetAnchorResizing(self, unitLayoutPostSizeFunc, 10, 500)
    self.MediaUpdate = MOD.RefreshUnitMedia
...
local UpdatePetTargetFrame = function(self)
    local db = UI.db.UnitFrames["pettarget"]
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height;
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")
    ...
    do
        if CUSTOM_UI_Pet and not InCombatLockdown()then
            self:SetParent(CUSTOM_UI_Pet)
        end
    end
    self:UpdateAllElements()
end
...
CONSTRUCTORS["pettarget"] = function(self, unit)
    ...
    MOD:CreateAuraFrames(self, key)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("BOTTOM", CUSTOM_UI_Pet, "TOP", 0, 7)
    self.snapOffset = -7
    UI:NewAnchor(self, L["PetTarget Frame"])
    UI:SetAnchorResizing(self, unitLayoutPostSizeFunc, 10, 500)
...
local UpdateFocusFrame = function(self)
    local db = UI.db.UnitFrames["focus"]
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height;
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")
...
CONSTRUCTORS["focus"] = function(self, unit)
    ...
    local xray = CreateFrame("Button", "CUSTOM_UI_XRayFocusClear", self, "SecureActionButtonTemplate")
    ...
    xray:SetFrameStrata("MEDIUM")
    xray.icon = xray:CreateTexture(nil,"ARTWORK")
    xray.icon:SetTexture([[Interface\Addons\CUSTOM_UI\media\textures\Doodads\UNIT-XRAY-CLOSE]])
    xray.icon:SetAllPoints(xray)
    xray.icon:SetAlpha(0)
    ...
    self:SetPoint("BOTTOMRIGHT", CUSTOM_UI_Target, "TOPRIGHT", 0, 220)
    UI:NewAnchor(self, L["Focus Frame"])
    UI:SetAnchorResizing(self, unitLayoutPostSizeFunc, 10, 500)
...
local UpdateFocusTargetFrame = function(self)
    local db = UI.db.UnitFrames["focustarget"]
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height;
    self:RegisterForClicks(UI.db.UnitFrames.fastClickTarget and "AnyDown" or "AnyUp")
...
CONSTRUCTORS["focustarget"] = function(self, unit)
    ...
    self.RaidIcon = MOD:CreateRaidIcon(self)
    self.Range = { insideAlpha = 1, outsideAlpha = 1 }
    self:SetPoint("LEFT", CUSTOM_UI_Focus, "RIGHT", 12, 0)
    self.snapOffset = -7
    UI:NewAnchor(self, L["FocusTarget Frame"])
    UI:SetAnchorResizing(self, unitLayoutPostSizeFunc, 10, 500)
...
local UpdateBossFrame = function(self)
    local db = UI.db.UnitFrames["boss"]
    local INDEX = self:GetID() or 1;
    local holder = _G['CUSTOM_UI_Boss1_MOVE']
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height;
...
CONSTRUCTORS["boss"] = function(self, unit)
    ...
    if(db.showBy == "UP") then
      if(not lastBossFrame) then
          self:SetPoint("RIGHT", UI.Screen, "RIGHT", -100, -190)
      else
          self:SetPoint("BOTTOMRIGHT", lastBossFrame, "TOPRIGHT", 0, yOffset)
      end
    else
      if(not lastBossFrame) then
          self:SetPoint("RIGHT", UI.Screen, "RIGHT", -85, 190)
      else
          self:SetPoint("TOPRIGHT", lastBossFrame, "BOTTOMRIGHT", 0, -yOffset)
      end
    end
    UI:NewAnchor(self, L["Boss Frame "..selfID])
    UI:SetAnchorResizing(self, enemyLayoutPostSizeFunc, 10, 500)
...
local UpdateArenaFrame = function(self)
    local db = UI.db.UnitFrames["arena"]
    local INDEX = self:GetID() or 1;
    local holder = _G['CUSTOM_UI_Arena1_MOVE'];
    local UNIT_WIDTH = db.width;
    local UNIT_HEIGHT = db.height
...
CONSTRUCTORS["arena"] = function(self, unit)
    ...
    if(not _G[prepName]) then
        ...
        health:SetStatusBarTexture(UI.Media.statusbar.default)
        prep.Health = health
        ...
        icon.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
        icon.Icon:InsetPoints(icon, 2, 2)
        prep.SpecIcon = icon

        local text = prep.Health:CreateFontString(nil, "OVERLAY")
        text:SetFont(UI.Media.font.dialog, 16, "OUTLINE")
    ...
    if(db.showBy == "UP") then
      if(not lastArenaFrame) then
          self:SetPoint("RIGHT", UI.Screen, "RIGHT", -85, -200)
      else
    ...
    UI:NewAnchor(self, L["Arena Frame "..selfID])
    UI:SetAnchorResizing(self, enemyLayoutPostSizeFunc, 10, 500)
...
local ArenaPrepHandler_OnEvent = function(self, event)
    ...
    if(not UI.db or not UI.db.UnitFrames or not UI.db.UnitFrames.arena or not UI.db.UnitFrames.arena.enable or instanceType ~= "arena") then return end
    if event == "PLAYER_LOGIN" then
        for i = 1, 5 do
            prepframe = _G["CUSTOM_UI_Arena"..i.."PrepFrame"]
            if(prepframe) then
                prepframe:SetAllPoints(_G["CUSTOM_UI_Arena"..i])
            end
        end
                ...
                if(prepframe) then
                    if i <= numOpps then
                        ...
                        if classToken and spec then
                            ...
                            if(not UI.db.general.customClassColor) then
...
function MOD:SetUnitFrame(key)
    ...
    local styleName = "CUSTOM_UI_"..realName
    local frame
    if not self.Units[unit] then
        oUF_CUSTOM_UI:RegisterStyle(styleName, CONSTRUCTORS[key])
        oUF_CUSTOM_UI:SetActiveStyle(styleName)
        frame = oUF_CUSTOM_UI:Spawn(unit, styleName)
        self.Units[unit] = frame
    else
        frame = self.Units[unit]
    end
    if frame:GetParent() ~= CUSTOM_UI_UnitFrameParent then
        frame:SetParent(CUSTOM_UI_UnitFrameParent)
    end
    if(UI.db.UnitFrames[key].enable) then
        frame:Enable()
        frame:Update()
    else
        frame:Disable()
