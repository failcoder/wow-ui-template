...
if(UI.class ~= "WARLOCK") then return end 

UI.SpecialFX:Register("affliction", [[Spells\Warlock_bodyofflames_medium_state_shoulder_right_purple.m2]], -12, 12, 12, -12, 0.22, 0, 0.52)
UI.SpecialFX:Register("overlay_demonbar", [[Spells\Warlock_destructioncharge_impact_chest.m2]], -20, -1, 20, -50, 0.9, 0, 0.8)
UI.SpecialFX:Register("underlay_demonbar", [[Spells\Fill_fire_cast_01.m2]], 3, -2, -3, 2, 0.5, -0.45, 1)
...
local FURY_FONT = [[Interface\AddOns\CUSTOM_UI\media\fonts\Numbers.ttf]]
local shardColors = {
...
local shardTextures = {
	[1] = {
		[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\WARLOCK-SHARD]],
		[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\WARLOCK-SHARD-BG]],
		[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\WARLOCK-SHARD-FG]],
		"affliction"
	},
	[2] = {
		UI.NoTexture,
	...
	[3] = {
		[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\WARLOCK-EMBER]],
		[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\WARLOCK-EMBER]],
		[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\WARLOCK-EMBER-FG]], 
		"fire"
	},
}
...
local Reposition = function(self)
    ...
    if(not db.classbar.detachFromFrame) then
    	UI:ResetAnchors(L["Classbar"], true)
    end
...
local ShardUpdate = function(self, value)
	if (value and value == 1) then
		if(self.overlay) then
			self.overlay:Show()
			UI.Animate:Flash(self.overlay,1,true)
		end
		...
		if(self.overlay) then
			UI.Animate:StopFlash(self.overlay)
			self.overlay:Hide()
		end
		self.FX:Hide()
	end
...
function MOD:CreateClassBar(playerFrame)
	...
	for i = 1, max do 
		...
		UI.SpecialFX:SetFXFrame(bar[i], textures[4], true)
	...
	UI.SpecialFX:SetFXFrame(demonicFury, "underlay_demonbar")
	...
	demonBar:SetStatusBarTexture(UI.Media.statusbar.default)
    ...
    UI.SpecialFX:SetFXFrame(demonBar, "overlay_demonbar", true)
