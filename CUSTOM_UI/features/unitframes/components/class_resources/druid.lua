...
if(UI.class ~= "DRUID") then return end
--[[
##########################################################
DRUID ALT MANA
##########################################################
...
local comboTextures = {
	[1]=[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\DRUID-CLAW-UP]],
	[2]=[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\DRUID-CLAW-DOWN]],
	[3]=[[Interface\Addons\CUSTOM_UI_UnitFrames\assets\Class\DRUID-BITE]],
};
...
local UpdateAltPower = function(self, unit, arg1, arg2)
	...
	if(arg1 ~= arg2) then
		local color = oUF_CUSTOM_UI.colors.power.MANA
		color = UI:HexColor(color[1],color[2],color[3])
...
function MOD:CreateClassBar(playerFrame)
	...
	UI.SpecialFX:SetFXFrame(moon, "shadow")
	...
	moon[1]:SetTexture("Interface\\AddOns\\CUSTOM_UI_UnitFrames\\assets\\Class\\VORTEX")
	moon[1]:SetBlendMode("ADD")
	moon[1]:SetVertexColor(0, 0.5, 1, 0.15)
	UI.Animate:Orbit(moon[1], 10, false)

	moon[2] = moon:CreateTexture(nil, "OVERLAY", nil, 2)
	moon[2]:SetSize(40, 40)
	moon[2]:SetPoint("CENTER")
	moon[2]:SetTexture("Interface\\AddOns\\CUSTOM_UI_UnitFrames\\assets\\Class\\DRUID-MOON")
	moon[1]:Hide()
	chicken.Moon = moon;
	...
	lunar:SetStatusBarTexture(UI.Media.statusbar.lazer)
	lunar.noupdate = true;
	...
	solar:SetStatusBarTexture(UI.Media.statusbar.lazer)
	solar.noupdate = true;
	...
	mana.ManaBar:SetStatusBarTexture(UI.Media.statusbar.glow)
	...
	mana.Text:SetFontObject(CUSTOM_UI_Font_Unit)
