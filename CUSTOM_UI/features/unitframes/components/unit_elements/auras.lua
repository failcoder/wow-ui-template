...
local AURA_STATUSBAR = UI.Media.statusbar.default;
local BASIC_TEXTURE = UI.Media.statusbar.default;
local CanSteal = (UI.class == "MAGE");
...
local CUSTOM_UI_Font_UnitAura 		= _G.CUSTOM_UI_Font_UnitAura;
local CUSTOM_UI_Font_UnitAura_Bar 	= _G.CUSTOM_UI_Font_UnitAura_Bar;
--[[
##########################################################
LOCAL FUNCTIONS
##########################################################
...
local FilterAura_OnClick = function(self)
	...
	if name and filterKey then
		UI:AddonMessage((L["The spell '%s' has been added to the BlackList unitframe aura filter."]):format(name))
		UI.db.Filters["BlackList"][filterKey] = {['enable'] = true, ['id'] = spellID}
...
local CreateAuraIcon = function(icons, index)
  ...
  bg:SetBackdrop(UI.Media.backdrop.aura)
  ...
  local fontgroup = "CUSTOM_UI_Font_UnitAura";
  if(baseSize < 18) then
  	fontgroup = "CUSTOM_UI_Font_UnitAura_Small";
  end
  --print(baseSize)
  --print(fontgroup)
...
local PostCreateAuraBars = function(self)
	...
	local barTexture = LSM:Fetch("statusbar", UI.db.UnitFrames.auraBarStatusbar)
	bar:SetStatusBarTexture(barTexture)
	bar.spelltime:SetFontObject(CUSTOM_UI_Font_UnitAura_Bar);
	...
	bar.spellname:SetFontObject(CUSTOM_UI_Font_UnitAura_Bar);
...
local PostBarUpdate = function(self, bar, spellID, isDebuff, debuffType)
	...
	local color;
	if(UI.db.UnitFrames.auraBarByType) then
		local filterKey = tostring(spellID)
		if(UI.db.Filters.AuraBars[filterKey]) then
			color = UI.db.Filters.AuraBars[filterKey]
...
local CommonAuraFilter = function(self, isEnemy, isPlayer, auraName, spellID, debuffType, duration, shouldConsolidate)
	local db = UI.db.UnitFrames[self.___unitkey]
	...
	if(auraDB.filterWhiteList and (not UI.db.Filters.WhiteList[filterKey])) then
		return false;
	elseif(UI.db.Filters.BlackList[filterKey] and UI.db.Filters.BlackList[filterKey].enable) then
		return false;
	else
		...
		if(active and UI.db.Filters[active]) then
			local spellDB = UI.db.Filters[active];
...
local DetailedAuraFilter = function(self, isEnemy, isPlayer, auraName, spellID, debuffType, duration, shouldConsolidate)
	...
	if(filter_test(auraDB.filterAll, isEnemy)) then
		return false
	elseif(filter_test(auraDB.filterWhiteList, isEnemy) and (not UI.db.Filters.WhiteList[filterKey])) then
		return false;
	elseif(UI.db.Filters.BlackList[filterKey] and UI.db.Filters.BlackList[filterKey].enable) then
		return false
	else
...
local PreForcedUpdate = function(self)
	...
	if not UI.db.UnitFrames[unit] then return end
	local db = UI.db.UnitFrames[unit].auraWatch;
	if not db then return end;
	if(unit == "pet" or unit == "raidpet") then
		self.watchFilter = UI.db.Filters.PetBuffWatch
	else
		self.watchFilter = UI.db.Filters.BuffWatch
	end
...
function MOD:CreateAuraWatch(frame, unit)
	...
	if(unit == "pet" or unit == "raidpet") then
		watch.watchFilter = UI.db.Filters.PetBuffWatch
	else
		watch.watchFilter = UI.db.Filters.BuffWatch
	end
...
local function UpdateAuraMediaLocals()
	BUFFS_COLOR = oUF_CUSTOM_UI.colors.buff_bars or DEFAULT_BUFFS_COLOR;
	DEBUFFS_COLOR = oUF_CUSTOM_UI.colors.debuff_bars or DEFAULT_DEBUFFS_COLOR;
	AURA_STATUSBAR = LSM:Fetch("statusbar", UI.db.UnitFrames.auraBarStatusbar);
end
UI.Events:On("UNITFRAME_COLORS_UPDATED", UpdateAuraMediaLocals, true);
