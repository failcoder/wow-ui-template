...
UI.SpecialFX:Register("overlay_castbar", [[Spells\Eastern_plaguelands_beam_effect.m2]], 2, -2, -2, 2, 0.95, -1, 0)
UI.SpecialFX:Register("underlay_castbar", [[Spells\Xplosion_twilight_impact_noflash.m2]], 1, -1, -1, 1, 0.9, 0, 0)
--[[ 
########################################################## 
LOCAL VARIABLES
##########################################################
...
local function SetCastTicks(bar,count,mod)
	...
	for i=1,count do 
		if not ticks[i] then 
			...
			ticks[i]:SetTexture(UI.Media.statusbar.lazer)
...
local function SetCastbarFading(castbar, texture)
	...
	fader.txt:SetFont(UI.Media.font.alert, 16)
...
local CustomInterruptible = function(self, unit, useClass)
	local colors = oUF_CUSTOM_UI.colors
...
function MOD:CreateCastbar(frame, reversed, moverName, ryu, useFader, isBoss, hasModel)
	...
	buttonIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	shieldIcon:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\SHIELD")
	...
	if ryu then
		castbar.Time:SetFontObject(CUSTOM_UI_Font_Aura)
		castbar.Time:SetTextColor(1, 1, 1)
		castbar.Text:SetFontObject(CUSTOM_UI_Font_Caps)
		castbar.Text:SetTextColor(1, 1, 1, 0.75)
		castbar.Text:SetWordWrap(false)

		castbar:SetStatusBarTexture(UI.Media.statusbar.lazer)

		bgFrame:InsetPoints(castbar, -2, 10)
		bgFrame:SetFrameLevel(bgFrame:GetFrameLevel() - 1)

	  	castbar.LatencyTexture:SetTexture(UI.Media.statusbar.lazer)
		...
		if reversed then 
			...
			hadouken[1]:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\HADOUKEN-REVERSED")
			...
			hadouken[1].overlay:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\SKULLS-REVERSED")
			hadouken[1].overlay:SetVertexColor(1, 1, 1)

			UI.Animate:Sprite4(hadouken[1],false,false,true)

			hadouken[2] = hadouken:CreateTexture(nil, "ARTWORK")
			hadouken[2]:InsetPoints(hadouken, 4, 4)
			hadouken[2]:SetBlendMode("ADD")
			hadouken[2]:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\CHANNEL-REVERSED")
			...
			hadouken[2].overlay:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\CHANNEL-REVERSED")
			hadouken[2].overlay:SetVertexColor(1, 1, 1)

			UI.Animate:Sprite4(hadouken[2],false,false,true)
			...
			hadouken[1]:SetAllPoints(hadouken)
			hadouken[1]:SetBlendMode("ADD")
			hadouken[1]:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\HADOUKEN")
			hadouken[1]:SetVertexColor(colors.spark[1],colors.spark[2],colors.spark[3])
			hadouken[1].overlay = hadouken:CreateTexture(nil, "OVERLAY")
			hadouken[1].overlay:SetHeight(50)
			hadouken[1].overlay:SetWidth(50)
			hadouken[1].overlay:SetPoint("CENTER", hadouken)
			hadouken[1].overlay:SetBlendMode("ADD")
			hadouken[1].overlay:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\HADOUKEN")
			hadouken[1].overlay:SetVertexColor(1, 1, 1)
			...
			hadouken[2]:InsetPoints(hadouken, 4, 4)
			hadouken[2]:SetBlendMode("ADD")
			hadouken[2]:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\CHANNEL")
			hadouken[2]:SetVertexColor(colors.spark[1],colors.spark[2],colors.spark[3])
			hadouken[2].overlay = hadouken:CreateTexture(nil, "OVERLAY")
			hadouken[2].overlay:SetHeight(50)
			hadouken[2].overlay:SetWidth(50)
			hadouken[2].overlay:SetPoint("CENTER", hadouken)
			hadouken[2].overlay:SetBlendMode("ADD")
			hadouken[2].overlay:SetTexture("Interface\\Addons\\CUSTOM_UI_UnitFrames\\assets\\Castbar\\CHANNEL")
			hadouken[2].overlay:SetVertexColor(1, 1, 1)
		...
		castbar:SetStatusBarTexture(UI.Media.statusbar.glow)
		...
		castbar.LatencyTexture:SetTexture(UI.Media.statusbar.default)
	...
	if(hasModel) then
		UI.SpecialFX:SetFXFrame(bgFrame, "underlay_castbar")
		bgFrame.FX:SetFrameLevel(0)
  		UI.SpecialFX:SetFXFrame(castbar, "overlay_castbar", nil, bgFrame)
  	end

	castbar.bg = bgFrame:CreateTexture(nil, "BACKGROUND", nil, -2)
	castbar.bg:SetAllPoints(bgFrame)
	castbar.bg:SetTexture(UI.Media.statusbar.default)
	...
	if moverName then
		...
		UI:NewAnchor(castbar.Holder, moverName)
	end 
	
	if useFader then
		SetCastbarFading(castbar, UI.Media.statusbar.lazer)
	end 
...
function MOD:PostCastInterruptible(unit)
	...
	CustomInterruptible(self, unit, UI.db.UnitFrames.castClassColor)
end 
