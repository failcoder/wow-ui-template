...
local AFFLICTED_SKIN = [[Interface\AddOns\CUSTOM_UI_UnitFrames\assets\UNIT-AFFLICTED]];
...
function MOD:CreateRaidDebuffs(frame)
	...
	raidDebuff.icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	raidDebuff.icon:InsetPoints(raidDebuff)
	raidDebuff.count = raidDebuff:CreateFontString(nil, "OVERLAY")
	raidDebuff.count:SetFontObject(CUSTOM_UI_Font_Aura)
	...
	raidDebuff.time:SetFontObject(CUSTOM_UI_Font_Aura)
...
function MOD:CreateGladiator(frame)
	...
	trinket.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))

	trinket.Unavailable = trinket:CreateTexture(nil, "OVERLAY")
	trinket.Unavailable:SetAllPoints(trinket)
	trinket.Unavailable:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	badge.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
...
function MOD:CreateFriendshipBar(frame)
    ...
    buddy:SetStatusBarTexture(UI.Media.statusbar.default)
...
function MOD:CreateExperienceRepBar(frame)
	...
	if db.playerExpBar then
		...
		xp:SetStyle("Frame")
		xp:SetStatusBarTexture(UI.Media.statusbar.default)
		...
		xp.Rested:SetStatusBarTexture(UI.Media.statusbar.default)
		xp.Rested:SetStatusBarColor(1, 0, 1, 0.6)
		xp.Value = xp:CreateFontString(nil, "TOOLTIP")
		xp.Value:SetAllPoints(xp)
		xp.Value:SetFontObject(CUSTOM_UI_Font_Default)
		...
		xp.Rested:SetBackdropColor(unpack(UI.Media.color.default))
	...
	if db.playerRepBar then
		...
		rep:SetStyle("Frame")
		rep:SetStatusBarTexture(UI.Media.statusbar.default)
		...
		rep.Value:SetFontObject(CUSTOM_UI_Font_Default)
...
function MOD:CreateHealPrediction(frame, fullSet)
	...
	absorbBar:SetStatusBarTexture(UI.Media.statusbar.gradient)
	...
	if(fullSet) then
		...
		healAbsorbBar:SetStatusBarTexture(UI.Media.statusbar.gradient)
...
local ResolveBar_OnEvent = function(self, event, unit)
	if(UI.db.UnitFrames.resolveBar) then
...
function MOD:CreateResolveBar(frame)
	...
	bar.text:SetFontObject(CUSTOM_UI_Font_Pixel)
