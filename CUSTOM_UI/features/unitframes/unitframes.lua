...
do
	...
	function oUF_CUSTOM_UI:DisableBlizzard(unit)
...
function MOD:RefreshUnitColors()
	if(not UI.db.general.customClassColor) then
		for eclass, color in next, RAID_CLASS_COLORS do
			oUF_CUSTOM_UI.colors.class[eclass] = {color.r, color.g, color.b}
		end
	else
	...
	local db = UI.Media.extended.unitframes
	for i, setting in pairs(db) do
		if setting and type(setting) == "table" then
			if(setting[1]) then
				oUF_CUSTOM_UI.colors[i] = setting
			else
				local bt = {}
	...
	oUF_CUSTOM_UI.colors.smooth = {1, 0, 0, 1, 1, 0, r, g, b}
	UI.Events:Trigger("UNITFRAME_COLORS_UPDATED");
end

function MOD:RefreshAllUnitMedia()
	if(not UI.db.UnitFrames) then return end
	self:RefreshUnitColors()
	for unit,frame in pairs(self.Units)do
		if UI.db.UnitFrames[frame.___key].enable then
			frame:MediaUpdate()
...
function MOD:RefreshUnitFrames()
	...
	local raidDebuffs = UI.oUF_RaidDebuffs or oUF_RaidDebuffs;
	if raidDebuffs then
		raidDebuffs:ResetDebuffData()
		if groupType == "party" or groupType == "raid" then
		  raidDebuffs:RegisterDebuffs(UI.db.Filters["Raid"])
		else
		  raidDebuffs:RegisterDebuffs(UI.db.Filters["CC"])
		end
	end
	...
	if UI.db.UnitFrames.disableBlizzard then
		oUF_CUSTOM_UI:DisableBlizzard('party')
	end
    ...
    if(unitDB and unitDB.enable) then
        ...
        if(panel) then
          if(panel.Name and unitDB.name) then
          	if(unitDB.grid and unitDB.grid.enable) then
          		panel.Name:SetFont(UI.Media.font.pixel, 8, "MONOCHROMEOUTLINE")
        ...
        if(self.Castbar and (unitDB.castbar)) then
            if(unitDB.castbar.useCustomColor) then
				...
				self.Castbar.CastColor = oUF_CUSTOM_UI.colors.casting
				self.Castbar.SparkColor = oUF_CUSTOM_UI.colors.spark
			end
        end
    end
end
...
function MOD:RefreshUnitLayout(frame, template)
	local db = UI.db.UnitFrames[template]
	...
	if(db.portrait) then
		...
		PORTRAIT_OVERLAY_ANIMATION = (PORTRAIT_OVERLAY) and UI.db.UnitFrames.overlayAnimation or false;
	...
	if(frame.InfoPanelBG) then
		if(UI.db.UnitFrames.infoBackgrounds) then
	...
	if(TEXT_GRIP.Name and db.name) then
		...
		if(GRID_MODE) then
			...
			cY = db.name.yOffset
			nametext:ClearAllPoints()
			UI:SetReversePoint(nametext, point, TEXT_GRIP, cX, cY)
	...
	if(frame.Health and TEXT_GRIP.Health and db.health) then
		if(GRID_MODE) then
			...
			UI:SetReversePoint(healthtext, point, TEXT_GRIP, cX, cY)
	...
	if(POWER_GRIP and TEXT_GRIP.Power and db.power) then
		if(GRID_MODE) then
			...
			if(db.power.tags ~= nil and db.power.tags ~= '') then
				...
				UI:SetReversePoint(powertext, point, TEXT_GRIP, cX, cY)
			end
	...
	do
		...
		health.Smooth = UI.db.UnitFrames.smoothbars;
		...
		if((not GRID_MODE) and PORTRAIT_OVERLAY and UI.db.UnitFrames.forceHealthColor) then
	...
	do
		if(POWER_GRIP) then
			if(POWER_ENABLED) then
				...
				POWER_GRIP.Smooth = UI.db.UnitFrames.smoothbars;
		...
		if(frame.AltPowerBar) then
			...
			if(db.power.enable) then
				...
				altPower.Smooth = UI.db.UnitFrames.smoothbars;
	...
	if(db.castbar and frame.Castbar) then
		...
		if castbar.Spark then
			if(db.castbar.spark) then
				...
				castbar.Spark.SetHeight = UI.fubar
			else
				castbar.Spark:Hide()
			end
		end
		...
		if(db.castbar.useCustomColor) then
			...
			castbar.CastColor = oUF_CUSTOM_UI.colors.casting
			castbar.SparkColor = oUF_CUSTOM_UI.colors.spark
		end
		...
		if((not BUFF_ENABLED) and (not DEBUFF_ENABLED)) then
			...
			if(BUFF_GRIP) then
				if(BUFF_ENABLED) then
					...
					UI:SetReversePoint(BUFF_GRIP, db.buffs.anchorPoint, attachAnchor, db.buffs.xOffset + BOTTOM_MODIFIER, db.buffs.yOffset)
			...
			if(DEBUFF_GRIP) then
				if(DEBUFF_ENABLED) then
					...
					UI:SetReversePoint(DEBUFF_GRIP, db.debuffs.anchorPoint, attachAnchor, db.debuffs.xOffset + BOTTOM_MODIFIER, db.debuffs.yOffset)
		...
		if db.icons then
			...
			if(ico.classIcon and frame.ActionPanel.class) then
				...
				if ico.classIcon.enable then
					...
					UI:SetReversePoint(classIcon, ico.classIcon.attachTo, MASTER_GRIP, ico.classIcon.xOffset, ico.classIcon.yOffset)
				else
					classIcon:Hide()
				end
			end
			...
			if(ico.raidicon and frame.RaidIcon) then
				...
				if ico.raidicon.enable then
					...
					if(GRID_MODE) then
						...
						UI:SetReversePoint(raidIcon, ico.raidicon.attachTo, MASTER_GRIP, ico.raidicon.xOffset, ico.raidicon.yOffset)
					end
				else
			...
			if(ico.roleIcon and frame.LFDRole) then
				...
				if(not MINI_GRID and ico.roleIcon.enable) then
					...
					if(GRID_MODE) then
						...
						UI:SetReversePoint(lfd, ico.roleIcon.attachTo, MASTER_GRIP, ico.roleIcon.xOffset, ico.roleIcon.yOffset)
					end
				else
			...
			if(ico.raidRoleIcons and frame.RaidRoleFramesAnchor) then
				...
				if(not MINI_GRID and ico.raidRoleIcons.enable) then
					...
					if(GRID_MODE) then
						...
						UI:SetReversePoint(roles, ico.raidRoleIcons.attachTo, MASTER_GRIP, ico.raidRoleIcons.xOffset, ico.raidRoleIcons.yOffset)
					end
				else
					roles:Hide()
	...
	if frame.Afflicted then
		if UI.db.UnitFrames.debuffHighlighting then
			...
			-- 	frame.Afflicted.Texture:SetTexture(UI.BaseTexture)
			-- end
	...
	if frame.Range then
		if(template:find("raid") or template:find("party")) then
			frame.Range.outsideAlpha = UI.db.UnitFrames.groupOORAlpha or 1
		else
			frame.Range.outsideAlpha = UI.db.UnitFrames.OORAlpha or 1
		end
	...
	if(frame.XRay) then
        if(UI.db.UnitFrames.xrayFocus) then
            frame.XRay:Show()
        else
            frame.XRay:Hide()
        end
...
function MOD:ADDON_LOADED(event, addon)
	...
	if addon == 'Blizzard_ArenaUI' then
		oUF_CUSTOM_UI:DisableBlizzard('arena')
		self:UnregisterEvent("ADDON_LOADED")
	end
end

function MOD:PLAYER_ENTERING_WORLD()
	if(not UI.NeedsFrameAudit) then
		self:RefreshUnitFrames()
	end
end
...
function MOD:Load()
	...
	local CUSTOM_UI_UnitFrameParent = CreateFrame("Frame", "CUSTOM_UI_UnitFrameParent", UI.Screen, "SecureHandlerStateTemplate")
	RegisterStateDriver(CUSTOM_UI_UnitFrameParent, "visibility", "[petbattle] hide; show")
	...
	UI.Events:On("AURA_FILTER_OPTIONS_CHANGED", UpdateUnitFrames, true);

	self:InitializeBodyGuard()

	local rDebuffs = UI.oUF_RaidDebuffs or oUF_RaidDebuffs;
