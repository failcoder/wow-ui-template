...
MOD.Holder = CreateFrame("Frame", "CUSTOM_UI_AurasAnchor", UIParent)
MOD.HyperBuffFrame = CreateFrame('Frame', 'CUSTOM_UI_ConsolidatedBuffs', UIParent)
--[[
##########################################################
LOCAL VARS
##########################################################
...
do
	local RefreshAuraTime = function(self, elapsed)
		...
		if(self.timeLeft > AURA_FADE_TIME) then
			UI.Animate:StopFlash(self)
		else
			UI.Animate:Flash(self, 1)
		end
	end
	...
	function MOD:CreateIcon(aura, has_attrib)
		...
		aura.texture:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))

		aura.count = aura:CreateFontString(nil, "ARTWORK")
		aura.count:SetPoint("BOTTOMRIGHT", (-1 + UI.db.Auras.countOffsetH), (1 + UI.db.Auras.countOffsetV))
		aura.count:SetFontObject(CUSTOM_UI_Font_Aura)

		aura.time = aura:CreateFontString(nil, "ARTWORK")
		aura.time:SetPoint("TOP", aura, "BOTTOM", 1 + UI.db.Auras.timeOffsetH, 0 + UI.db.Auras.timeOffsetV)
		aura.time:SetFontObject(CUSTOM_UI_Font_Aura)

		aura.highlight = aura:CreateTexture(nil, "HIGHLIGHT")
		aura.highlight:SetTexture(UI.Media.statusbar.default)
		aura.highlight:SetVertexColor(1, 1, 1, 0.45)
		aura.highlight:InsetPoints(aura, 2, 2)

		UI.Animate:Flash(aura)
...
do
	...
	local function PlayerRoleChanged()
		if(UI.db.Auras.hyperBuffsEnabled) then
...
do
	...
	function MOD:Update_ConsolidatedBuffsSettings(event)
		...
		local hideIndex;
		if(UI.db.Auras.hyperBuffsFiltered) then
			if UI.SpecificClassRole == 'CASTER' then
				hideIndex = 3
			else
				hideIndex = 5
			end
...
function MOD:UpdateAuraHeader(auraHeader, auraType)
	...
	local db = UI.db.Auras[auraType]
	local showBy = db.showBy

	if(auraType == "buffs") then
		auraHeader:SetAttribute("consolidateTo", UI.db.Auras.hyperBuffsEnabled == true and 1 or 0)
		auraHeader:SetAttribute("weaponTemplate", ("CUSTOM_UI_AuraTemplate%d"):format(db.size))
	end
	...
	auraHeader:SetAttribute("template", ("CUSTOM_UI_AuraTemplate%d"):format(db.size))
	...
	while(auraChild) do
		...
		if(auraChild.time) then
			...
			auraChild.time:SetPoint("TOP", auraChild, "BOTTOM", 1 + UI.db.Auras.timeOffsetH, UI.db.Auras.timeOffsetV)
			auraChild.count:ClearAllPoints()
			auraChild.count:SetPoint("BOTTOMRIGHT", -1 + UI.db.Auras.countOffsetH, UI.db.Auras.countOffsetV)
		end
...
function MOD:ReLoad()
	...
	local CB_HEIGHT = Minimap:GetHeight() - 50;
	local CB_WIDTH = (CB_HEIGHT / maxShown) + 4;
	if(CUSTOM_UI_MinimapFrame) then
		CB_HEIGHT = CUSTOM_UI_MinimapFrame:GetHeight() - 50;
	end
	self.Holder:SetSize(CB_WIDTH, CB_HEIGHT)
	AURA_FADE_TIME = UI.db.Auras.fadeBy
	self:UpdateAuraHeader(CUSTOM_UI_PlayerBuffs, "buffs");
	self:UpdateAuraHeader(CUSTOM_UI_PlayerDebuffs, "debuffs");
...
function MOD:Load()
	...
	if(UI.db.Auras.hyperBuffsEnabled) then
		...
		UI:ManageVisibility(self.HyperBuffFrame)
		...
		UI.Events:On("PLAYER_ROLE_CHANGED", PlayerRoleChanged)
		self:Update_ConsolidatedBuffsSettings()
	end

	if(UI.db.Auras.aurasEnabled) then
		BuffFrame:Die()
		TemporaryEnchantFrame:Die()
		InterfaceOptionsFrameCategoriesButton12:SetScale(0.0001)

		local buffHeader = CreateFrame("Frame", "CUSTOM_UI_PlayerBuffs", self.Holder, "SecureAuraHeaderTemplate")
		...
		local debuffHeader = CreateFrame("Frame", "CUSTOM_UI_PlayerDebuffs", self.Holder, "SecureAuraHeaderTemplate")
	...
	UI:NewAnchor(self.Holder, L["Auras Frame"])
