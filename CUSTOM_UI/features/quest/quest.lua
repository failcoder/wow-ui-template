...
local OBJECTIVE_GetStatus = function(self)
	if(not self.Status) then
		...
		status.Bar:SetStatusBarTexture(UI.Media.statusbar.default)
		...
		status.Label:SetFontObject(CUSTOM_UI_Font_Quest_Number)
		status.Label:SetTextColor(1,1,1)
		status.Label:SetText('')
...
local OBJECTIVE_HEADER_Get = function(self, index)
	if(not self.Rows[index]) then
		...
		objective.Text:SetFontObject(CUSTOM_UI_Font_Quest);
...
local function ExpandQuestTracker(location)
	...
	UI.Timers:ExecuteTimer(MOD.UpdateDimensions, 0.2)
end
...
function MOD:UpdateLocals()
	...
	local baseWidth = UI.db.QuestTracker.rowHeight;
	local calculated = UI.Media.shared.font.questdialog.size / 0.55;
	...
	UI.Events:Trigger("QUEST_UPVALUES_UPDATED", ROW_WIDTH, ROW_HEIGHT, INNER_HEIGHT, LARGE_ROW_HEIGHT, LARGE_INNER_HEIGHT);
end
...
function MOD:Load()
	self.Docklet = UI.Dock:NewDocklet("BottomRight", "CUSTOM_UI_QuestTracker", "Quest Tracker", MOD.Media.dockIcon);

	local scrollFrame = CreateFrame("ScrollFrame", "CUSTOM_UI_QuestTrackerScrollFrame", self.Docklet);
	...
	local scrollBar = CreateFrame("Slider", "CUSTOM_UI_QuestTrackerScrollFrameScrollBar", scrollFrame);
	...
	scrollBar:SetScript("OnValueChanged", function(self, argValue)
		_G.CUSTOM_UI_QuestTrackerScrollFrame:SetVerticalScroll(argValue)
	end)
	UI.API:Set("ScrollBar", scrollBar)

	local scrollChild = CreateFrame("Frame", "CUSTOM_UI_QuestTrackerScrollFrameScrollChild", scrollFrame);
	...
	ObjectiveTrackerFrame:SetParent(UI.Hidden);

	self.Headers["Popups"]:Refresh()

	UI.Events:On("DOCK_EXPANDED", ExpandQuestTracker, true);
	UI.Events:On("DOCK_FADE_IN", PostFadeInCallback, true);
	UI.Events:On("DOCK_FADE_OUT", PostFadeOutCallback, true);
end
