...
function MOD:InitializeActive()
	...
	block.CloseButton:SetNormalTexture(UI.Media.icon.close)
  ...
  block.CloseButton:HookScript("OnLeave", CloseButton_OnLeave)
	...
	block.Badge.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	block.Header.Level:SetFontObject(CUSTOM_UI_Font_Quest);
	...
	if(UI.AddQuestCompass) then
		block.Details:SetHeight(INNER_HEIGHT - 4);
		UI:AddQuestCompass(block, block.Badge)
	...
	UI.Events:On("QUEST_UPVALUES_UPDATED", UpdateActiveLocals, true);
end
