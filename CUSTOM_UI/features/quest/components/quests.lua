...
local ItemBar = _G["CUSTOM_UI_QuestItemBar"];
ItemBar.Buttons = {};
...
do
    ...
    CreateQuestItemButton = function(index)
    	local buttonName = "CUSTOM_UI_QuestButton" .. index
      ...
      UI:ManageVisibility(itembutton)
...
function ItemBar:Update()
	...
	local isHorizontal = (UI.db.QuestTracker.itemBarDirection == 'HORIZONTAL');
	...
	if(isHorizontal) then
		...
		if(UI.Tooltip and (not self.tipanchorchecked) and UI.Tooltip.Holder and UI.Tooltip.Holder.Grip and (not UI.Tooltip.Holder.Grip:HasMoved())) then
			UI.Tooltip.DefaultPadding = 56
...
local GetQuestRow = function(self, index)
	if(not self.Rows[index]) then
		...
		row.Header.Level:SetFontObject(CUSTOM_UI_Font_Quest_Number);
		...
		row.Header.Text:SetFontObject(CUSTOM_UI_Font_Quest);
		...
		row.Header.Zone:SetFontObject(CUSTOM_UI_Font_Quest);
		...
		row.Timer.Bar:SetStatusBarTexture(UI.Media.statusbar.default)
		...
		bgFrame.bg:SetTexture(UI.Media.statusbar.default)
		...
		row.Timer.TimeLeft:SetFontObject(CUSTOM_UI_Font_Quest_Number);
...
local function UpdateQuestLocals(...)
	...
	ITEM_BUTTON_SIZE = UI.db.QuestTracker.itemButtonSize;
	ITEMS_PER_ROW = UI.db.QuestTracker.itemButtonsPerRow;
	ReAnchorItemBar();
	ItemBar:Update()
end
...
local function PostMoveCallback(buttonName)
	...
	MOD.QuestItemTimer = UI.Timers:ExecuteTimer(ShowItemBarButtons, 1.2);
	ShowItemBarButtons();
end
...
local function PostHideCallback(location, windowName)
	...
	if(MOD.QuestItemTimer) then
		UI.Timers:RemoveTimer(MOD.QuestItemTimer)
...
function MOD:InitializeQuests()
	ReAnchorItemBar()
	UI:NewAnchor(ItemBar, L["Quest Items"]);
	ITEM_BUTTON_SIZE = UI.db.QuestTracker.itemButtonSize;
	ITEMS_PER_ROW = UI.db.QuestTracker.itemButtonsPerRow;
	for i = 1, 5 do
	...
	UI.Events:On("DOCKLET_MOVED", PostMoveCallback, true);
	UI.Events:On("DOCKLET_SHOWN", PostShowCallback, true);
	UI.Events:On("DOCKLET_HIDDEN", PostHideCallback, true);
	...
	quests.Header.Text:SetFontObject(CUSTOM_UI_Font_Quest_Header);
	...
	quests.Header.Divider:SetTexture([[Interface\AddOns\CUSTOM_UI\media\textures\DROPDOWN-DIVIDER]]);
	...
	UI.Events:On("QUEST_UPVALUES_UPDATED", UpdateQuestLocals, true);
end
