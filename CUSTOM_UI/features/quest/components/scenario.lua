...
function MOD:InitializeScenarios()
	...
	block.Icon:SetTexture(LINE_SCENARIO_ICON)
	block.Icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
	...
	block.Header.Stage:SetFontObject(CUSTOM_UI_Font_Quest);
	...
	block.Header.Score:SetFontObject(CUSTOM_UI_Font_Quest);
	...
	block.Header.Text:SetFontObject(CUSTOM_UI_Font_Quest);
	...
	timer.Bar:SetStatusBarTexture(UI.Media.statusbar.default)
	...
	timer.Bar.Wave:SetFontObject(CUSTOM_UI_Font_Quest);
	...
	timer.Bar.TimeLeft:SetFontObject(CUSTOM_UI_Font_Quest_Number);
	...
	UI.Events:On("QUEST_UPVALUES_UPDATED", UpdateScenarioLocals, true);
end
