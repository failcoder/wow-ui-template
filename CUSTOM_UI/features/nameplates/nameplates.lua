...
local BLIZZ_PLATE, CUSTOM_UI_PLATE, PLATE_REF, PLATE_ARGS, PLATE_AURAS, PLATE_AURAICONS, PLATE_GRIP, PLATE_REALNAME;
...
local NPGrip = _G.CUSTOM_UI_PlateParentFrame
local NPGlow = _G.CUSTOM_UI_PlateGlowFrame
...
local function CreatePlateBorder(plate)
	...
	if(not plate.backdrop) then
		...
		plate.backdrop:SetTexture(UI.Media.statusbar.default)
...
local function CheckRaidIcon(plate)
	...
	if PLATE_REF.raidicon:IsShown() then
		...
		CUSTOM_UI_PLATE.raidicon:Show()
		CUSTOM_UI_PLATE.raidicon:SetTexCoord(ULx,ULy,LLx,LLy,URx,URy,LRx,LRy)
	else
		PLATE_REF.raidicontype = nil;
		CUSTOM_UI_PLATE.raidicon:Hide()
	end
end
...
local function CreateAuraIcon(auras, plate)
	...
	button.TimeLeft:SetFontObject(CUSTOM_UI_Font_NamePlate_Aura)
	...
	button.Stacks:SetFontObject(CUSTOM_UI_Font_NamePlate_Aura)
...
do
	...
	local function UpdatePlateUnit()
		...
		if BLIZZ_PLATE:GetAlpha() == 1 and CURRENT_TARGET_NAME and (CURRENT_TARGET_NAME == plateName) then
			...
			CUSTOM_UI_PLATE:SetFrameLevel(2)
			CUSTOM_UI_PLATE.highlight:Hide()
			if(NPUsePointer) then
				NPGlow:SetParent(CUSTOM_UI_PLATE)
				NPGlow:WrapPoints(CUSTOM_UI_PLATE.health,2,2)
		...
		elseif PLATE_REF.highlight:IsShown() and UnitExists("mouseover") and (UnitName("mouseover") == plateName) then
			if(PLATE_ARGS.unit ~= "mouseover" or PLATE_ARGS.allowed) then
				CUSTOM_UI_PLATE:SetFrameLevel(1)
				CUSTOM_UI_PLATE.highlight:Show()
			...
			CUSTOM_UI_PLATE:SetFrameLevel(0)
			CUSTOM_UI_PLATE.highlight:Hide()
			PLATE_ARGS.unit = nil
		end
		CheckRaidIcon()
		UpdatePlateElements(BLIZZ_PLATE,CUSTOM_UI_PLATE)
	end
...
do
	local function HealthBarSizeChanged(self, width, height)
		...
		for index = 1, numAuras do
			if not PLATE_AURAICONS[index] then
				PLATE_AURAICONS[index] = CreateAuraIcon(PLATE_AURAS, CUSTOM_UI_PLATE);
			end
	...
	local function ShowThisPlate(plate)
		...
		if RestrictedPlates[PLATE_REALNAME] then
			CUSTOM_UI_PLATE:Hide()
			PLATE_GRIP:Hide()
			return
		elseif(not CUSTOM_UI_PLATE:IsShown()) then
			PLATE_GRIP:Show()
			CUSTOM_UI_PLATE:Show()
		end

		VisiblePlates[BLIZZ_PLATE] = true

		PLATE_ARGS.tiny = (BLIZZ_PLATE.health:GetEffectiveScale() < 1)
		CUSTOM_UI_PLATE:SetSize(BLIZZ_PLATE:GetSize())

		CUSTOM_UI_PLATE.name:ClearAllPoints()
		if(PLATE_ARGS.tiny) then
			CUSTOM_UI_PLATE.health:SetSize(BLIZZ_PLATE.health:GetWidth() * (BLIZZ_PLATE.health:GetEffectiveScale() * 1.25), HBHeight)
			CUSTOM_UI_PLATE.name:SetPoint("BOTTOM", CUSTOM_UI_PLATE.health, "TOP", 0, 3)
		else
			CUSTOM_UI_PLATE.name:SetPoint("BOTTOMLEFT", CUSTOM_UI_PLATE.health, "TOPLEFT", 0, 3)
			CUSTOM_UI_PLATE.name:SetPoint("BOTTOMRIGHT", CUSTOM_UI_PLATE.level, "BOTTOMLEFT", -2, 0)
		end
	...
	local function HideThisPlate(plate)
		...
		PLATE_ARGS.tiny = nil
		PLATE_ARGS.allowed = nil
		if(NPGlow:GetParent() == CUSTOM_UI_PLATE) then
			NPGlow:Hide()
			if(NPGlow.FX:IsShown()) then
				NPGlow.FX:Hide()
			end
		end
		CUSTOM_UI_PLATE.health.alert:Hide()
		CUSTOM_UI_PLATE.health.icon:Hide()
		if CUSTOM_UI_PLATE.health then
			CUSTOM_UI_PLATE.health:SetSize(HBWidth, HBHeight)
		...
		if MOD.UseCombo then
			for i=1, MAX_COMBO_POINTS do
				CUSTOM_UI_PLATE.combo[i]:Hide()
			end
		end

		CUSTOM_UI_PLATE:SetPoint("BOTTOMLEFT", plate, "BOTTOMLEFT")
	end

	local function UpdateThisPlate(plate)
		if(not ProxyThisPlate(plate, true)) then return; end
		CUSTOM_UI_PLATE.name:SetFontObject(CUSTOM_UI_Font_NamePlate)
		CUSTOM_UI_PLATE.name:SetTextColor(1, 1, 1)
		CUSTOM_UI_PLATE.level:SetFontObject(CUSTOM_UI_Font_NamePlate_Number)
		if not PLATE_ARGS.scaled and not PLATE_ARGS.tiny then
			CUSTOM_UI_PLATE.health:SetSize(HBWidth, HBHeight)
		end
		CUSTOM_UI_PLATE.health:SetStatusBarTexture(NPBarTex)
		CUSTOM_UI_PLATE.health.text:SetFontObject(CUSTOM_UI_Font_NamePlate_Number)
		CUSTOM_UI_PLATE.cast:SetSize(HBWidth, CBHeight)
		CUSTOM_UI_PLATE.cast:SetStatusBarTexture(NPBarTex)
		CUSTOM_UI_PLATE.cast.text:SetFont(UI.Media.font.default, 8, "OUTLINE")
		plate.cast.text:SetFont(UI.Media.font.default, 8, "OUTLINE")
		plate.cast.icon:SetSize(ICON_SIZE, ICON_SIZE)
		PLATE_REF.raidicon:ClearAllPoints()
		UI:SetReversePoint(PLATE_REF.raidicon, RIAnchor, CUSTOM_UI_PLATE.health, RIXoffset, RIYoffset)
		PLATE_REF.raidicon:SetSize(RISize, RISize)
		CUSTOM_UI_PLATE.health.icon:ClearAllPoints()
		UI:SetReversePoint(CUSTOM_UI_PLATE.health.icon, RIAnchor, CUSTOM_UI_PLATE.health, RIXoffset, RIYoffset)
		CUSTOM_UI_PLATE.health.icon:SetSize(RISize, RISize)
		for index = 1, #PLATE_AURAICONS do
			if PLATE_AURAICONS and PLATE_AURAICONS[index] then
				PLATE_AURAICONS[index].TimeLeft:SetFontObject(CUSTOM_UI_Font_NamePlate_Aura)
				PLATE_AURAICONS[index].Stacks:SetFontObject(CUSTOM_UI_Font_NamePlate_Aura)
				PLATE_AURAICONS[index].Icon:SetTexCoord(.07, 0.93, .23, 0.77)
			end
		end

		if(MOD.UseCombo and not CUSTOM_UI_PLATE.combo:IsShown()) then
			CUSTOM_UI_PLATE.combo:Show()
		elseif(CUSTOM_UI_PLATE.combo:IsShown()) then
			CUSTOM_UI_PLATE.combo:Hide()
		end

		ShowThisPlate(plate)
		HealthBarSizeChanged(CUSTOM_UI_PLATE.health, CUSTOM_UI_PLATE.health:GetSize())
	end
	...
	function PlateForge(plate)
		...
		-- 	UI.ScriptError:ShowDebug(MOD.Schema, ref)
		...
		health:SetStatusBarTexture(UI.NoTexture)
		cast:SetStatusBarTexture(UI.NoTexture)
		...
		--ref.eliteicon.SetTexture = UI.fubar
		...
		local frameName = "CUSTOM_UI_PlateHolder"..numChildren
...
function MOD:UpdateLocals()
	...
	AuraFilter = UI.db.Filters[AuraFilterName]

	NPFindHealers = db.findHealers

	local tc = UI.db.NamePlates.threat
	NPUseThreat = tc.enable;
	NPThreatGS = tc.goodScale;
	NPThreatBS = tc.badScale;
	if(UI.ClassRole == 'TANK') then
	...
	if(not db.themed) then
		PLATE_TOP = UI.NoTexture
		PLATE_BOTTOM = UI.NoTexture
		PLATE_RIGHT = UI.NoTexture
		PLATE_LEFT = UI.NoTexture
	else
	...
	if (db.comboPoints and (UI.class == 'ROGUE' or UI.class == 'DRUID')) then
		self.UseCombo = true
...
function MOD:Load()
	--UI.SpecialFX:Register("platepoint", [[Spells\Arcane_missile_lvl1.m2]], -12, 48, 12, -48, 0.25, 0, 0)
	UI.SpecialFX:Register("platepoint", [[Spells\Arrow_state_animated.m2]], -12, 12, 12, -50, 0.75, 0, 0.1)
	--UI.SpecialFX:Register("platepoint", [[Spells\Cast_arcane_01.m2]], -12, 48, 12, -48, 0.25, 0, 0)
	--UI.SpecialFX:Register("platepoint", [[Spells\Cast_arcane_01.m2]], -12, 48, 12, -48, 0.25, 0, 0)
	--UI.SpecialFX:Register("platepoint", [[Spells\Shadow_precast_uber_hand.m2]],  -12, 22, 12, -22, 0.23, -0.1, 0.1)
	UI.SpecialFX:SetFXFrame(NPGlow, "platepoint", true)
	NPGlow.FX:SetParent(UI.Screen)
	...
	UI.Events:On("PLAYER_ROLE_CHANGED", PlayerRoleUpdate, true)
end
