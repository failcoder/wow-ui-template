...
local CUSTOM_UI_Font_Default 	= _G.CUSTOM_UI_Font_Default;
local CUSTOM_UI_Font_Header 		= _G.CUSTOM_UI_Font_Header;
local CUSTOM_UI_Font_Bag 		= _G.CUSTOM_UI_Font_Bag;
local CUSTOM_UI_Font_Bag_Number 	= _G.CUSTOM_UI_Font_Bag_Number;

local BagFilters = _G.CUSTOM_UI_BagFilterMenu;
local BagBar = _G.CUSTOM_UI_BagBar;
--[[
##########################################################
GET ADDON DATA
##########################################################
...
local function StyleBagToolButton(button, iconTex)
	...
	bg:SetTexture(UI.Media.button.roundbg)
	bg:SetVertexColor(unpack(UI.Media.color.default))

	local outer = button:CreateTexture(nil, "OVERLAY")
	outer:WrapPoints(button, 5, 5)
	outer:SetTexture(UI.Media.button.round)
	outer:SetGradient(unpack(UI.Media.gradient.special))

	button:SetNormalTexture(iconTex)
	iconTex = button:GetNormalTexture()
	iconTex:SetGradient(unpack(UI.Media.gradient.medium))
	...
	if button.SetPushedTexture then
		...
		pushed:SetTexture(UI.Media.button.round)
		pushed:SetGradient(unpack(UI.Media.gradient.highlight))
	...
	if button.SetCheckedTexture then
		...
		checked:SetTexture(UI.Media.button.round)
		checked:SetGradient(unpack(UI.Media.gradient.green))
...
local SlotUpdate = function(self, slotID)
  ...
  if(slot.newitemglowAnim) then slot.newitemglowAnim:Stop() end;
	...
	UI:SetGearLabels(slot, bagID, slotID, itemLink, quality, equipSlot)
end
...
local ContainerFrame_UpdateLayout = function(self)
	...
	if(UI.db.Inventory.alignToChat) then
		containerWidth = (isBank and UI.db.Dock.dockLeftWidth or UI.db.Dock.dockRightWidth)
		local avg = 0.08;
		...
		containerWidth = (isBank and UI.db.Inventory.bankWidth) or UI.db.Inventory.bagWidth
		buttonSize = isBank and UI.db.Inventory.bankSize or UI.db.Inventory.bagSize;
	...
	for i, bagID in ipairs(self.BagIDs) do
		if((not isBank and bagID <= 3) or (isBank and bagID ~= -1 and numContainerSlots >= 1 and not (i - 1 > numContainerSlots))) then
			...
			if(not bagSlot) then
				...
				if isBank then
					globalName = "CUSTOM_UI_BankBag" .. bagID - 4;
					bagTemplate = "BankItemButtonBagTemplate";
				else
					globalName = "CUSTOM_UI_MainBag" .. bagID .. "Slot";
				...
				bagSlot.icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
		...
		if numSlots > 0 then
			...
			for slotID = 1, numSlots do
				...
				if not self.Bags[bagID][slotID] then
					...
					-- 	self.Bags[bagID][slotID].flashAnim.Play = UI.fubar
					-- end
					...
					self.Bags[bagID][slotID].icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS));
					...
					self.Bags[bagID][slotID].questIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS));
				...
				if(not self.Bags[bagID][slotID].GearInfo) then
					...
					self.Bags[bagID][slotID].GearInfo:SetFontObject(CUSTOM_UI_Font_Default)
				...
				if(not self.Bags[bagID][slotID].ItemLevel) then
					...
					self.Bags[bagID][slotID].ItemLevel:SetFontObject(CUSTOM_UI_Font_Default)
...
local ReagentFrame_UpdateLayout = function(self)
	...
	for slotID = 1, maxCount do
		...
		if not bag[slotID] then
			...
			slot.icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS));
			...
			slot.questIcon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS));
...
function UI:SetLootTooltip(tooltip, itemKey)
...
function MOD:ModifyBags()
	local docked = UI.db.Inventory.alignToChat
	local anchor, x, y
	if(docked) then
		if self.BagFrame then
			self.BagFrame:ClearAllPoints()
			self.BagFrame:SetPoint("BOTTOMRIGHT", UI.Dock.BottomRight, "BOTTOMRIGHT", 0, 0)
		end
		if self.BankFrame then
			self.BankFrame:ClearAllPoints()
			self.BankFrame:SetPoint("BOTTOMLEFT", UI.Dock.BottomLeft, "BOTTOMLEFT", 0, 0)
		end
	else
		if self.BagFrame then
			local anchor, x, y = UI.db.Inventory.bags.point, UI.db.Inventory.bags.xOffset, UI.db.Inventory.bags.yOffset
			self.BagFrame:ClearAllPoints()
			self.BagFrame:SetPoint(anchor, UI.Screen, anchor, x, y)
		end
		if self.BankFrame then
			local anchor, x, y = UI.db.Inventory.bank.point, UI.db.Inventory.bank.xOffset, UI.db.Inventory.bank.yOffset
			self.BankFrame:ClearAllPoints()
			self.BankFrame:SetPoint(anchor, UI.Screen, anchor, x, y)
		end
	end
end
...
do
	...
	function MOD:PositionBagBar(reset)
		...
		if(BagBar.Grip and (not BagBar.Grip:HasMoved())) then
			...
			if(reset) then
				BagBar:SetPoint("BOTTOMLEFT", UI.Dock.BottomRight.Window, "TOPLEFT", -4, 0)
			else
	...
	local function LoadBagBar()
		local bagFading = UI.db.Inventory.bagBar.mouseover

		BagBar:SetParent(UI.Screen)
		BagBar:ClearAllPoints()
		...
		UI:NewAnchor(BagBar, L["Bags Bar"])

		MOD.BagBarLoaded = true
	end

	function MOD:ModifyBagBar()
		if(not UI.db.Inventory.bagBar.enable) then return end

		if not self.BagBarLoaded then
			LoadBagBar()
		end

		local showBy = UI.db.Inventory.bagBar.showBy
		local showBG = UI.db.Inventory.bagBar.showBackdrop
		local sortDir = UI.db.Inventory.bagBar.sortDirection
		local bagSize = UI.db.Inventory.bagBar.size
		local bagSpacing = UI.db.Inventory.bagBar.spacing
		local bagFading = UI.db.Inventory.bagBar.mouseover
...
do
	...
	local Vendor_OnClick = function(self)
		if(IsShiftKeyDown() or (not MerchantFrame or not MerchantFrame:IsShown())) then
			UI.SystemAlert["DELETE_GRAYS"].Money = UI:VendorGrays(false,true,true)
			UI:StaticPopup_Show('DELETE_GRAYS')
		else
			UI:VendorGrays()
		end
	end
	...
	local TopInfo_OnEnter = function(self)
		...
		local goldData = UI:GetReportData("gold")

		local networth = goldData[nameKey];
		GameTooltip:AddLine(L[nameKey..": "])
		GameTooltip:AddDoubleLine(L["Total: "], UI:FormatCurrency(networth), 1,1,1,1,1,1)
		GameTooltip:AddLine(" ")
		...
		for name,amount in pairs(goldData)do
			if(name ~= nameKey and name ~= 'total') then
				...
				GameTooltip:AddDoubleLine(name, UI:FormatCurrency(amount), 1,1,1,1,1,1)
			end
		end
	...
	local Tooltip_Show = function(self)
		...
		self:GetNormalTexture():SetGradient(unpack(UI.Media.gradient.highlight))
		GameTooltip:Show()
	end

	local Tooltip_Hide = function(self)
		self:GetNormalTexture():SetGradient(unpack(UI.Media.gradient.medium))
		GameTooltip:Hide()
	end
	...
	function MOD:MakeBags()
		local bagName = "CUSTOM_UI_ContainerFrame"
		local uisCount = #UISpecialFrames + 1;
		local frame = CreateFrame("Button", "CUSTOM_UI_ContainerFrame", UI.Screen)
		...
		frame.Bags = {}
		frame.closeButton = CreateFrame("Button", "CUSTOM_UI_ContainerFrameCloseButton", frame, "UIPanelCloseButton")
		frame.closeButton:SetPoint("TOPRIGHT", -4, -4)
		UI.API:Set("CloseButton", frame.closeButton);
		...
		frame.BagMenu = CreateFrame("Button", "CUSTOM_UI_ContainerFrameBagMenu", frame)
		...
		frame.goldText:SetFontObject(CUSTOM_UI_Font_Bag_Number)
		...
		frame.editBox = CreateFrame("EditBox", "CUSTOM_UI_ContainerFrameEditBox", frame)
		...
		frame.editBox:SetFontObject(CUSTOM_UI_Font_Bag)
		...
		searchText:SetFontObject(CUSTOM_UI_Font_Bag)
		...
		for h = 1, MAX_WATCHED_TOKENS do
			...
			frame.currencyButton[h].icon:SetTexCoord(unpack(_G.CUSTOM_UI_ICON_COORDS))
			...
			frame.currencyButton[h].text:SetFontObject(CUSTOM_UI_Font_Bag_Number)
	...
	function MOD:MakeBankOrReagent(isReagent)
		...
		local bagName = isReagent and "CUSTOM_UI_ReagentContainerFrame" or "CUSTOM_UI_BankContainerFrame"
		local uisCount = #UISpecialFrames + 1;

		local frame = CreateFrame("Button", bagName, isReagent and self.BankFrame or UI.Screen)
		frame:SetStyle("Frame", "Pattern")
		frame:SetFrameStrata("HIGH")
		...
		if(not isReagent) then
			...
			local BagBtn_OnClick = function()
				...
				if numSlots  >= 1 then
					...
					UI:StaticPopup_Show("NO_BANK_BAGS")
				end
			end
			frame.bagsButton:SetScript("OnClick", BagBtn_OnClick)
			...
			local PurchaseBtn_OnClick = function()
				...
				if not full then
					UI:StaticPopup_Show("BUY_BANK_SLOT")
				else
					UI:StaticPopup_Show("CANNOT_BUY_BANK_SLOT")
				end
			end
			...
			frame.swapButton:SetScript("OnEnter", function(self)
				...
				if(not IsReagentBankUnlocked()) then
					GameTooltip:AddDoubleLine("Purchase Reagents Bank", UI:FormatCurrency(GetReagentBankCost()), 0.1,1,0.1, 1,1,1)
				else
					GameTooltip:AddLine(self.ttText)
				end
				self:GetNormalTexture():SetGradient(unpack(UI.Media.gradient.highlight))
			...
			frame.swapButton:SetScript("OnClick", function()
				if(not IsReagentBankUnlocked()) then
					UI:StaticPopup_Show("CONFIRM_BUY_REAGENTBANK_TAB");
				else
					PlaySound("igMainMenuOption");
					if(_G["CUSTOM_UI_ReagentContainerFrame"]:IsShown()) then
						_G["CUSTOM_UI_ReagentContainerFrame"]:Hide()
					else
						_G["CUSTOM_UI_ReagentContainerFrame"]:Show()
					end
				end
			end)
...
function MOD:RefreshTokens()
	...
	for i=1,MAX_WATCHED_TOKENS do
		...
		if name then
			...
			if UI.db.Inventory.currencyFormat == 'ICON_TEXT' then
				set.text:SetText(name..': '..count)
			elseif UI.db.Inventory.currencyFormat == 'ICON' then
...
local function _openBags()
	...
	if(not MOD.BagFrame:IsShown()) then
		...
		if(UI.Tooltip) then
			UI.Tooltip.GameTooltip_SetDefaultAnchor(GameTooltip)
		end
...
local function _closeBags()
	...
	if(MOD.BagFrame:IsShown()) then
		...
		if(UI.Dock.CloseBreakStuff) then
			UI.Dock:CloseBreakStuff()
		end
		if(UI.Tooltip) then
			UI.Tooltip.GameTooltip_SetDefaultAnchor(GameTooltip)
		end
...
function MOD:Load()
	...
	UI.Events:On("SPECIAL_FRAMES_CLOSED", _closeSpecialWindows, true);
	UI.Events:On("FULL_UI_RESET", ResetInventoryLogs, true);
	...
	UI.SystemAlert["CONFIRM_BUY_REAGENTBANK_TAB"] = {
	...
	UI.SystemAlert["CANNOT_BUY_BANK_SLOT"] = {
