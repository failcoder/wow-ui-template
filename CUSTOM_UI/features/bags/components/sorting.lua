...
##############################################################################
credit: Kemayo.               original logic from BankStack. Adapted to CUSTOM_UI #
##############################################################################
##########################################################
...
do
	...
	local function Sorter(bags, sorter, reverse)
		...
		local ignoreItems = UI.db.Inventory.ignoreItems
		...
		for i=1, select("#", ...) do
			...
			for bagType, sortedBags in pairs(sortingCache[1]) do
				if bagType ~= 'Normal' then
					...
					SortFiller(sortingCache[1]['Normal'], sortedBags, UI.db.Inventory.sortInverted)
					MOD.Stack(sortingCache[1]['Normal'], IsPartial)
					Sorter(sortedBags, nil, UI.db.Inventory.sortInverted)
					twipe(sortedBags)
				end
			end
			if sortingCache[1]['Normal'] then
				MOD.Stack(sortingCache[1]['Normal'], IsPartial)
				Sorter(sortingCache[1]['Normal'], nil, UI.db.Inventory.sortInverted)
...
do
	...
	function SortUpdateTimer:StopStacking(message)
		...
		self:Hide()
		if(message) then
			UI:SCTMessage(message, 1, 0.35, 0)
		end
	end
