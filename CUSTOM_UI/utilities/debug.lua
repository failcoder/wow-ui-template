...
local L = UI.L;
_G.BINDING_NAME_CUSTOM_UI_FRAMEDEBUGGER = "CUSTOM_UI: Frame Analyzer";
UI.ScriptError = _G["CUSTOM_UI_ScriptError"];
local ScriptErrorDialog = _G["CUSTOM_UI_ScriptErrorDialog"];
local ScriptErrorScrollBar = _G["CUSTOM_UI_ScriptErrorDialogScrollBar"];
...
function UI.ScriptError:DebugOutput(msg)
...
function UI.ScriptError:TableDump(t)
    self:DebugOutput(tprint(t))
end

function UI.ScriptError:ShowDebug(header, ...)
    wipe(DUMPTABLE);
    wipe(CHECKTABLE);
...
_G.DebugThisFrame = function(arg)
    ...
    UI.ScriptError:DebugOutput(outputString)
    --ScriptErrorDialog:SetVerticalScroll(1)
end

_G.SlashCmdList["CUSTOM_UI_FRAME_DEBUG"] = DebugThisFrame;
_G.SLASH_CUSTOM_UI_FRAME_DEBUG1 = "/svdf"

--SetCVar('scriptProfile',1)

local function InitializeScriptError()
    UI.ScriptError:SetParent(UI.Screen)
    UI.ScriptError.Source = "";
    UI.ScriptError:SetStyle("!_Frame", "Transparent")
    UI.ScriptError.Clear:SetStyle("Button")
    UI.ScriptError:SetScript("OnShow", ScriptError_OnShow)
    UI.API:Set("ScrollBar", CUSTOM_UI_ScriptErrorDialogScrollBar)
    ScriptErrorDialog:SetStyle("!_Frame", "Transparent")
    ScriptErrorDialog.Input:SetScript("OnTextChanged", ScriptError_OnTextChanged)
    UI.ScriptError:RegisterForDrag("LeftButton");
end

UI.Events:On("LOAD_ALL_ESSENTIALS", InitializeScriptError);
--[[
##########################################################
...
local function _showErrors(msg)
    if msg then
        if(msg == "off") then
            UI.DebugMode = false
            UI:AddonMessage("Debug Mode |cffFF0000DISABLED|r")
        elseif(msg == "on") then
            UI.DebugMode = true
            UI:AddonMessage("Debug Mode |cff00FF00ENABLED|r")
        end
    end

    if(UI.DebugMode) then
        local ERRORSTRING = table.concat(UI.ERRORLOG, "\n\n");
        UI.ScriptError:DebugOutput(ERRORSTRING)
        wipe(UI.ERRORLOG)
    else
        UI:AddonMessage("Debug Mode Not Enabled! Try using |cff00FF00/showerrors on|r")
    end
end
...
function UI:ResetErrors()
    wipe(UI.ERRORLOG)
end

_G.SlashCmdList["CUSTOM_UI_SHOW_ERRORS"] = _showErrors;
_G.SLASH_CUSTOM_UI_SHOW_ERRORS1 = "/showerrors"
